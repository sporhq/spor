...
3. parses the emitted factory with the runner's own vocabulary
   ...
   `integration:` block (`parseIntegration`), the `rescue:` block
   (`parseRescue`) routed to a strong-model profile in the emitted set, and the
   `implementation:`/`completion:` blocks (`parseImplementation`) — declaring
   `implementation:` is what flips `completion.by` to `controller` on its own;
