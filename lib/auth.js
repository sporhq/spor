...
// ---------------------------------------------------------------------------
// Localhost-loopback login (OAuth 2.1 authorization-code + PKCE, RFC 8252) — the
// browser-local optimization the device grant falls back from. The orchestration
// (the one-shot 127.0.0.1 listener, PKCE generation, opening the browser) lives
// in bin/spor.js; these are the pure front-door calls it drives.
// task-cc-spor-auth-cli-web-loopback.
// ---------------------------------------------------------------------------

// RFC 7591 §3 — anonymous dynamic client registration. Registers a PUBLIC
// (PKCE, no secret) client for the given loopback redirect URIs so the loopback
// flow has a client_id + an exact-match redirect to drive /oauth/authorize.
// Returns the oauthPost result; on ok the json carries { client_id,
// registration_access_token, registration_client_uri, ... }. A 404 means the
// front door exposes no DCR endpoint (caller falls back to the device grant).
function registerClient(server, { redirectUris, clientName = "spor CLI" } = {}) {
  return oauthPost(server, "/oauth/register", {
...
// RFC 6749 §4.1.3 — exchange a captured authorization code (+ the PKCE verifier)
// for the org-scoped access+refresh pair, against the same lenient JSON token
// endpoint the device poll uses.
function exchangeCode(server, { code, codeVerifier, clientId, redirectUri, resource }) {
  return oauthPost(server, "/oauth/token", {
    grant_type: "authorization_code",
    code,
    code_verifier: codeVerifier,
    client_id: clientId,
    redirect_uri: redirectUri,
    // RFC 8707: echo the resource sent at /oauth/authorize so the minted token's
    // `aud` targets it (task-spor-app-api-strict-audience-restriction). Inert when
    // the issuer doesn't allowlist the origin.
    ...(resource ? { resource } : {}),
  });
}

// RFC 7592 §2.3 — best-effort unregister of a throwaway loopback client after a
// successful login, so a public DCR front door does not accrue one dead public
// client per loopback login. SAFE: the grant just minted does NOT depend on the
// client record (refresh looks the grant up by token hash, not client_id), so
// deleting it never invalidates the credential. Never throws; a failure just
// leaves the client for the server's idle GC. The args come from the
// registration 201 (registration_client_uri + registration_access_token).
async function unregisterClient(registrationClientUri, regToken, timeoutMs = 5000) {
  if (!registrationClientUri || !regToken) return false;
  const ctrl = new AbortController();
  const timer = setTimeout(() => ctrl.abort(), timeoutMs);
  try {
    const res = await fetch(registrationClientUri, {
      method: "DELETE",
