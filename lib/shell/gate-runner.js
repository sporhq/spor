...
function buildGateFact({ gate, nodeId, runId, project, verdict, detail, evidence, attempts, escalatedTo, demotion, date, factory, attempt = 0, ledger = null, rescue = 0, rescueNext = null, outage = null, change = null, definition = null }) {
...
async function runGatePipeline({ item, factory, deps, log = () => {} }) {
  ...
  const record = async (gate, outcome, extras = {}) => {
    const fields = gateFactFields(outcome, extras);
    const { verdict, detail, escalatedTo, rescue } = fields;
    let factId = null;
    try {
    ...
    results.push({ gate: gate.id, kind: gate.kind, verdict, detail: detail || null, fact: factId, escalated_to: escalatedTo || null, ...(rescue ? { rescue } : {}),
      ...
      head: change && change.head || null, base: change && change.base || null,
  ...
  const judge = async (rescue, seed) => {
    ...
    for (let gi = 0; gi < factory.gates.length; gi += 1) {
      ...
      if (outcome.passed) {
        ...
        await record(gate, outcome, { attempts: shownAttempts(gate, attempts, rescue), ledger, rescue, startedAt });
  ...
  if (refusal && factory.rescue && deps.rescue) {
    for (;;) {
      ...
      if (resumed) {
        ...
        const fact = await record(gate, outcome, {
          attempts, ledger,
          rescue: n - 1,
...
async function runOneGate({ gate, cycle, factory, item, head = null, changed, changedReason, noCodeRefusal = null, deps, log, ledger = [], lastFix = null, rescue = 0, base = 0 }) {
  if (gate.kind === "command") {
    ...
    return {
      ...
      detail: gates.describeRerunsExhausted(reason, attempt),
