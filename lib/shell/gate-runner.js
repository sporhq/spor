...
async function runGatePipeline({ item, factory, deps, log = () => {} }) {
  ...
  const reportEvidence = async (gate, outcome, { fact, candidate_id = null, head = null, rescue = 0 } = {}) => {
    ...
    try {
      await deps.gateEvidenceRecorded({ gate, verdict: outcome.verdict, fact, candidate_id, head, rescue });
      reportedEvidence.add(key);
  ...
  const judge = async (rescue, seed) => {
    ...
    for (let gi = 0; gi < factory.gates.length; gi += 1) {
      ...
      if (change && change.head && reusable(gate, judged.get(gate.id))) { retainResult(gate); continue; }
      const startedAt = now();
      let outcome = null;
