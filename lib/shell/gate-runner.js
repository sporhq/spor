...
function buildGateFact({ gate, nodeId, runId, project, verdict, detail, evidence, attempts, escalatedTo, demotion, date, factory, attempt = 0, ledger = null, rescue = 0, rescueNext = null, flake = null }) {
...
async function runGatePipeline({ item, factory, deps, log = () => {} }) {
  ...
  const reconcileExisting = async (gate, id, markdown, owed) => {
    let seen = null;
    try {
      seen = deps.readFact ? await deps.readFact({ id, markdown, nodeId, gate }) : { ok: false, reason: "this worker has no door to read it back through" };
    } catch (e) {
    ...
    const has = (issue) => edges.some((e) => e && String(e.to) === issue && String((e && e.type) || "") === FLAKE_EDGE);
  ...
  const payFlakeEdges = async (gate, id, unpaid) => {
    if (!deps.linkFact) return [];
    const paid = [];
    for (const issue of unpaid) {
      let r = null;
      try {
        r = await deps.linkFact({ id, type: FLAKE_EDGE, to: issue, nodeId, gate });
      } catch (e) {
        r = { ok: false, reason: `${(e && e.message) || e}` };
      }
      if (r && r.ok) {
  ...
  const record = async (gate, { verdict, detail, evidence, attempts, escalatedTo, demotion, ledger, rescue = 0, rescueNext = null, flake = null }) => {
    let factId = null;
    let linked = [];
    const owed = flakeEdges(flake);
    try {
      const fact = buildGateFact({
        gate, nodeId, runId, project: item.project || null, verdict, detail, evidence, attempts, escalatedTo, demotion, ledger, flake,
      ...
      if (wrote && wrote.ok) {
        if (!wrote.existing || wrote.identical) {
        ...
        if (factId && unpaid.length) linked = [...linked, ...(await payFlakeEdges(gate, factId, unpaid))];
  ...
  const judge = async (rescue, seed) => {
    for (const gate of factory.gates) {
      ...
      if (outcome.passed) {
        ...
        await record(gate, { verdict: outcome.verdict, detail: outcome.detail, evidence: outcome.evidence || null, attempts: shownAttempts(gate, attempts, rescue), ledger, rescue, flake: outcome.flake || null });
  ...
  await record(gate, {
    ...
    // The flake filings a charged off-diff pass DID land ride the refusal's
    // fact (F7): each is this run's occurrence, and an issue no fact links to
    // has no provenance and no occurrence to its name.
    flake: outcome.flake || null,
    demotion:
      demoted.note ||
      (demoted.reason
