...
async function runGatePipeline({ item, factory, deps, log = () => {} }) {
  ...
  const reconcileExisting = async (gate, id, markdown, owed) => {
    let seen = null;
    try {
      seen = deps.readFact ? await deps.readFact({ id, markdown, nodeId, gate, flakeIssues: owed }) : { ok: false, reason: "this worker has no door to read it back through" };
    } catch (e) {
  ...
  const pipelineEvidenceOrigin = deps.evidenceOrigin ? deps.evidenceOrigin() : null;
    ...
    try {
      ...
      if (wrote && (wrote.ok || wrote.existing)) {
        if (wrote.ok && (!wrote.existing || wrote.identical)) {
          ...
          const seen = await reconcileExisting(gate, fact.id, fact.markdown, owed);
          factId = seen.id;
          linked = seen.linked;
        }
