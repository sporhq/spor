...
async function runGatePipeline({ item, factory, deps, log = () => {} }) {
  ...
  const saveProgress = async (gate, progress, rescue = 0) => {
    ...
    try {
      ...
      const withPre = !rescue && preAttempts.length && gate.id === factory.gates[0].id && !progress.preAttempts ? { ...progress, preAttempts } : progress;
  ...
  const preAttempts = [];
  const shownAttempts = (gate, attempts, rescue = 0) => (!rescue && preAttempts.length && gate.id === factory.gates[0].id ? [...preAttempts, ...attempts] : attempts);
  ...
  if (!rescues.length && changed === null && changedDirty && factory.gates.length && deps.fix) {
  ...
  const rescuable = (refusal) => refusal && !refusal.outcome.escalatedTo && refusal.outcome.verdict !== "blocked";
  let refusal = null;
  let resumed = null;
  if (rescues.length) {
    ...
    for (const e of rescues) if (e.fact && !facts.includes(e.fact)) facts.push(e.fact);
