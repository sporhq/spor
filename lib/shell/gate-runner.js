...
async function runGatePipeline({ item, factory, deps, log = () => {} }) {
  ...
  const roundTrip = async (rescue = 0, seed = null) => {
    if (!(changed === null && changedDirty && factory.gates.length && deps.fix)) return;
    ...
    if (Array.isArray(prior && prior.preAttempts) && prior.preAttempts.length) {
      ...
      log(`work: ${nodeId} is still dirty and its commit-or-discard round-trip was already spent before this pipeline was resumed${pass} — gate ${gate.id} judges the tree as it is`);
      return;
    }
