...
async function runGatePipeline({ item, factory, deps, log = () => {} }) {
  ...
  const saveProgress = async (gate, progress) => {
    ...
    try {
      await deps.saveGateProgress({ gate, item, progress });
    } catch (e) {
  ...
  for (const gate of factory.gates) {
    ...
    if (saved && (startCycle > 0 || ledger.length || attempts.length)) {
      ...
      if (lastFix && !lastFix.toHead) lastFix.toHead = changedHead;
      log(`work: gate ${gate.id} resumed on ${nodeId} at fix cycle ${startCycle}/${gate.cycles} — ${ledger.length} ledger finding(s) carried`);
    }
    for (let cycle = startCycle; ; cycle += 1) {
      ...
      const headBefore = changedHead;
      // Counted BEFORE the dispatch, not after: a worker killed while it waits
      // on the fix run must resume at the review AFTER this fix, or the fix is
      // dispatched twice and the cap is one larger than declared.
      lastFix = { cycle, runId: null, fromHead: headBefore, toHead: null, findings: outcome.findings || [] };
      await saveProgress(gate, { fixes: cycle + 1, attempts, ledger, lastFix });
      let fixed = null;
      try {
        fixed = await deps.fix({ gate, cycle, item, findings: outcome.findings || [], evidence: outcome.evidence || "", detail: outcome.detail, ledger });
      } catch (e) {
      ...
      await readChanged();
      lastFix = { ...lastFix, runId: (fixed && fixed.runId) || null, toHead: changedHead };
      await saveProgress(gate, { fixes: cycle + 1, attempts, ledger, lastFix });
    }

    if (outcome.passed) {
      await record(gate, { verdict: outcome.verdict, detail: outcome.detail, attempts, ledger });
...
async function runOneGate({ gate, cycle, factory, item, changed, changedReason, deps, log, ledger = [], lastFix = null }) {
  ...
  if (gate.kind === "agent-review") {
    ...
    let r = null;
    try {
      r = await deps.review({ gate, cycle, item, factory, prior, ledger, fix: lastFix });
    } catch (e) {
    ...
    const v = gates.parseReviewVerdict(r.text, { prior, cycle });
    const next = gates.applyReviewToLedger(ledger, v, cycle);
    v.findings = gates.withLedgerIds(v.findings, next, cycle);
    if (!v.ok) {
      ...
      // cleared nothing, so the whole prior set stands.
      const answered = Array.isArray(v.prior) && v.prior.some((p) => p.note !== "not answered by this review");
      return {
        passed: false,
        verdict: "failed",
        ...
        findings: answered ? v.findings : prior.map((p) => ({ ...p, origin: "prior", blocking: true, status: "open" })),
        ledger: next,
      };
    }
    if (v.passed) {
