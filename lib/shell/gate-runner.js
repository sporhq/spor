...
function buildGateFact({ gate, nodeId, runId, project, verdict, detail, evidence, attempts, escalatedTo, demotion, date, factory, attempt = 0, ledger = null, rescue = 0, rescueNext = null, outage = null }) {
...
async function runGatePipeline({ item, factory, deps, log = () => {} }) {
  ...
  const judge = async (rescue, seed) => {
    for (const gate of factory.gates) {
      ...
      if (outcome.passed) {
        ...
        await record(gate, outcome, { attempts: shownAttempts(gate, attempts, rescue), ledger, rescue });
  ...
  const escalationRetry = !escalatedTo && outcome.verdict !== "blocked"
    ? {
        ...
        factId: gateFactId(gate.id, nodeId, runId, item.attempt || 0, rescue),
      }
    : null;
  return {
