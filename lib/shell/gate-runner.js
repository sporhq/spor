...
async function runGatePipeline({ item, factory, deps, log = () => {} }) {
  ...
  const rescuable = (refusal) => refusal && !changedGone && !refusal.outcome.noRescue && !refusal.outcome.escalatedTo && refusal.outcome.verdict !== "blocked";
  let refusal = null;
  let resumed = null;
  if (rescues.length) {
