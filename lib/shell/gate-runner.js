...
//
// So an isolation pass runs only when the failing files, and the local files
// they themselves name one hop out (a helper holding the CLI path, a fixture),
// reference nothing the change edited. Reading is bounded and fails CLOSED in
// every direction that leaves the question open: an unreadable seed, a tree
// path we do not have, or a walk that would exceed the budget all read as "not
// demonstrably off-diff" and the failure is charged exactly as it was before
// this existed.
//
...
// the pass would re-RUN) is HARD: anything that stops us reading one stops the
...
// importing anything.
const REF_SCAN_MAX_READS = 64;
const REF_SCAN_MAX_BYTES = 1024 * 1024;
const REF_SCAN_HOPS = 2;
function changeReferencedBy(dir, seeds, changed, { also = [] } = {}) {
  ...
  for (let hop = 0; hop < REF_SCAN_HOPS && queue.length; hop += 1) {
    const next = [];
    for (const { rel, hard } of queue) {
      if (seen.has(rel)) continue;
      seen.add(rel);
      // A hard seed is a file the isolation would RUN, so anything that stops
      // us reading it stops the pass. A soft seed (another file the failure
      // named) and a hop-1 candidate are only guesses at a local file, so one
      // that is not there is simply not a reference.
      const fail = (why) => (hard ? { reached, unknown: why } : null);
      const p = path.resolve(base, rel);
      if (p !== base && !p.startsWith(base + path.sep)) {
        const f = fail(`${rel} is outside the judged tree`);
        if (f) return f;
        continue;
      }
      let src = "";
      try {
        ...
        if (!st.isFile()) {
          const f = fail(`${rel} is not a file in the judged tree`);
          if (f) return f;
          continue;
        }
        if (st.size > REF_SCAN_MAX_BYTES) {
          const f = fail(`${rel} is too large to read`);
          if (f) return f;
          continue;
        }
        ...
        const f = fail(`${rel} could not be read (${(e && e.message) || e})`);
        if (f) return f;
        continue;
      }
      ...
      // imports. Asked on every hop, the last included.
      ...
      if (hop + 1 < REF_SCAN_HOPS) for (const c of cands) next.push({ rel: c, hard: false });
    }
    queue = next;
  }
    ...
    if (isolation && isolation.ok) {
      ...
      if (deps.fileFlakeItem) {
        try {
          ...
          // failing FILES and on nothing else, so a LIVE occupant IS this
          ...
          if (f && f.ok && f.id) {
            filed = f.id;
            if (f.existing) log(`work: the off-diff flake in ${isolation.files.join(", ")} is already filed as ${f.id} — linking that issue rather than rewriting it`);
          } else {
            isolation.fileError = (f && f.reason) || "no response";
          ...
          isolation.fileError = (e && e.message) || String(e);
          log(`work: the off-diff flake in ${isolation.files.join(", ")} could not be filed (${isolation.fileError}) — the suite failure is charged rather than passed with no durable record of why`);
        }
      } else {
        isolation.fileError = "this worker has no door to file it through";
      }
      // Fall through to the charged failure below when nothing landed:
      ...
      // escalation can see the flake that could not be recorded.
      if (filed) {
        const armedBy = armed.classes.length ? ` (armed by ${armed.classes.map((c) => c.class).join(", ")})` : "";
        return {
          passed: true,
