  ...
  const judge = async (rescue, seed) => {
    for (const gate of factory.gates) {
      ...
      if (outcome.passed) {
        ...
        await record(gate, { verdict: outcome.verdict, detail: outcome.detail, evidence: outcome.evidence || null, attempts: shownAttempts(gate, attempts, rescue), ledger, rescue });
