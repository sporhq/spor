...
async function runGatePipeline({ item, factory, deps, log = () => {} }) {
  ...
  // nothing for it to find. The evidence predates the run and is read
  // straight off the graph/git, so unlike the declared route there is nothing
  // for a run to launder by asserting it — it either checks out or it
  // doesn't.
  //
  // Checked BEFORE the declared no-code claim: it costs one graph read and a
  // few git probes, needs no report parsing, and — being independent of
