...
async function runGatePipeline({ item, factory, deps, log = () => {} }) {
  ...
  const judge = async (rescue, seed) => {
    ...
    const reusable = (gate, head) => !!head && declarations.get(gate.id) === declaration(gate) && (head === (change && change.head) || mayRetain(gate));
    ...
    const retainResult = (gate) => {
      ...
      if (result && result.head !== (change && change.head)) Object.assign(result, { retained: true, rejudge_on_repin: false });
    };
    ...
    for (let gi = 0; gi < factory.gates.length; gi += 1) {
      ...
      if (mayRetain(gate) && cached && cached.result && cached.result.head && cached.result.verdict === "passed"
