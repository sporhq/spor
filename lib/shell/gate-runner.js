...
// The EDGES a fact owes those filings — the debt the whole flake payload
...
// twice. A payload carried across a rescue boundary therefore says which fact
// already paid it (`linkedTo`, set by refusalFromEntry from the entry's own
// durable `fact` id): the prose still names the issues, the edge is not
// written a second time. Reconciled against settled state, never acted on
// blindly — the flag guards a debt some other write may already have
// discharged.
function flakeEdges(flake) {
  if (!flake || flake.linkedTo) return [];
  return flakeIssues(flake);
}
  ...
  const lines = [
    ...
    flake && (flake.files || []).length
      ? `Off-diff flake: ${flake.files.join(", ")} failed the whole-suite run and passed alone on the same tree${flakeIssues(flake).length ? `, filed as ${flakeIssues(flake).join(", ")}` : ""}${
          ...
          flake.linkedTo && flakeIssues(flake).length
            ? ` That occurrence is already recorded on ${flake.linkedTo}, so this fact names the issue(s) without linking them a second time.`
            : ""
        }`
      : "",
    ...
    for (;;) {
      ...
      if (resumed) {
        ...
        entry = {
          ...
          // The flake filings this refusal carries (F9), in the SAME stamp as
          // the `fact` id above — which is what says whether the edges they
          // are owed have already been written. Two fields, one write: a crash
          // can never leave the payload persisted beside a stale answer to
          // "was it already linked", and a resumed pipeline that escalates
          // this carried refusal (its rescue could not run, its checkout is
          // gone) can pay a debt the pre-rescue `record` above dropped instead
          // of leaving the issues it filed with no fact naming them.
          flake: outcome.flake || null,
...
// never happened. The payload is RECONCILED against what already discharged
// it rather than replayed blindly — `entry.fact` is the fact that recorded
// this same refusal WITH these filings, so when it landed the escalation fact
// names them without a second edge (`linkedTo`), and only when it did not
// does the escalation actually pay. One occurrence, one edge, either way.
function refusalFromEntry(factory, entry) {
  const gate = factory.gates.find((g) => g.id === entry.gate) || { id: entry.gate, kind: "gate", cycles: 0 };
  const flake = entry.flake ? { ...entry.flake, ...(entry.fact ? { linkedTo: entry.fact } : {}) } : null;
  return {
    gate,
