...
async function runGatePipeline({ item, factory, deps, log = () => {} }) {
  ...
  const record = async (gate, outcome, extras = {}, evidenceChange = change, evidenceDefinition = factory.definition || null, origin = pipelineEvidenceOrigin, evidenceCandidateId = pinnedCandidate && pinnedCandidate.candidate_id || null) => {
    ...
    const persistEvidence = async (paid, receipt = null) => {
      if (!flakeIssues(flake).length) return true;
      const key = `${rescue || 0}:${gate.id}`;
      const previous = passProgress.get(key) || await loadProgress(gate, rescue || 0) || {};
      const progress = { ...previous, evidence: { outcome: { ...fields, flake: paid }, change: evidenceChange, definition: evidenceDefinition, gate, origin, candidate_id: evidenceCandidateId, ...(receipt || {}) } };
      try {
    ...
    for (let gi = 0; gi < factory.gates.length; gi += 1) {
      ...
      for (let cycle = startCycle; ; cycle += 1) {
        if (pendingFix) {
          ...
          outcome = await runOneGate({ gate, cycle, factory, item, head: change && change.head, changed, changedReason, noCodeRefusal, deps, log, ledger, lastFix, rescue, base });
      ...
      if (deps.fileFlakeItem) {
        for (const file of isolation.files) {
          try {
            ...
            const f = await deps.fileFlakeItem({ gate, item, file, files: isolation.files, command: gate.command, isolate: isolation.command });
