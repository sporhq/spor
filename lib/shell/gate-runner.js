...
async function runGatePipeline({ item, factory, deps, log = () => {} }) {
  ...
  for (const gate of factory.gates) {
    ...
    for (let cycle = 0; ; cycle += 1) {
      ...
      await readChanged();
      lastFix = { cycle, runId: (fixed && fixed.runId) || null, fromHead: headBefore, toHead: changedHead, findings: outcome.findings || [] };
    }
  ...
  if (gate.kind === "agent-review") {
    ...
    if (!v.ok) {
      ...
      return {
        ...
        findings: prior.map((p) => ({ ...p, origin: "prior", blocking: true, status: "open" })),
        ledger: next,
      };
    }
    if (v.passed) {
