...
//
// So an isolation pass runs only when the failing TEST FILES, and the local
// files they themselves name one hop out (a helper holding the CLI path, a
// fixture), reference nothing the change edited. Reading is bounded and fails
// CLOSED in every direction that leaves the question open: an unreadable seed,
// a tree path we do not have, or a walk that would exceed the budget all read
// as "not demonstrably off-diff" and the failure is charged exactly as it was
// before this existed.
const REF_SCAN_MAX_READS = 64;
const REF_SCAN_MAX_BYTES = 1024 * 1024;
const REF_SCAN_HOPS = 2;
function changeReferencedBy(dir, seeds, changed) {
  ...
  const seen = new Set();
  let reads = 0;
  let queue = (Array.isArray(seeds) ? seeds : []).map((f) => String(f));
  for (let hop = 0; hop < REF_SCAN_HOPS && queue.length; hop += 1) {
    const next = [];
    for (const rel of queue) {
      if (seen.has(rel)) continue;
      seen.add(rel);
      // A seed is a file the failure named and the isolation would RUN, so
      // anything that stops us reading it stops the pass. A hop-1 candidate is
      // only a guess at a local file, so one that is not there is simply not a
      // reference.
      const fail = (why) => (hop === 0 ? { reached, unknown: why } : null);
      ...
      if (hop + 1 < REF_SCAN_HOPS) next.push(...gates.referencedCandidates(src, rel));
    }
    queue = next;
  }
    ...
    try {
      ...
      if (!(r && r.ok) && !(suite && !suite.ok)) {
        ...
        if (tests.length) {
          ...
          // argument. Fails closed (see changeReferencedBy).
          const ref = changeReferencedBy(dir, tests, changed);
