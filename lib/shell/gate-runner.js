...
// The synthetic gate id the no-code-outcome route files its fact under
// (task-spor-factory-no-code-outcome-convention). Not a declared gate — no
// gate ran — so it needs an id of its own that cannot collide with one a
// factory declares; `art-gate-scoping-…` is what a person greps for.
...
async function runGatePipeline({ item, factory, deps, log = () => {} }) {
  ...
  // outcome and links the item, and the item really moved. A claim that does
  // not check out falls straight through to the empty-diff refusal it would
  // have got anyway, carrying WHY it did not — so this can only ever remove a
  // wrong escalation, never manufacture a pass (dec-spor-gates-enforced-in-
  // code-factory-is-data).
  //
  ...
  if (changed && changed.length === 0 && deps.noCodeClaim) {
    ...
    if (claim) {
      const read = async (id) => {
        if (!id || !deps.node) return null;
        try {
          const r = await deps.node({ id, item });
          return r && r.ok && r.node ? r.node : null;
        } catch (e) {
          log(`work: ${nodeId} — \`${id}\` could not be read from the graph (${(e && e.message) || e})`);
          return null;
        }
      };
      // The verify leg the SUPERSEDED branch above uses, asked here for one
      // fact only: does the item carry a live resolving edge. An unreachable
      // graph answers "no", which is the fail-closed direction — the claim
      // then rests on the re-stamp, or falls through.
      let resolvedNow = false;
      if (deps.resolved) {
        try {
          const r = await deps.resolved({ item });
          resolvedNow = !!(r && r.terminal_state === "resolved");
        } catch (e) {
          log(`work: ${nodeId} — its resolution could not be read from the graph (${(e && e.message) || e}); judging the no-code claim without it`);
        }
      }
      const verdict = gates.verifyNoCodeOutcome({
        nodeId,
        claim,
        resolver: claim.ok ? await read(claim.resolver) : null,
        item: await read(nodeId),
        claimedRepo: item.project || null,
      ...
      // and kind, exactly as the rescue lane's fact does.
      const scopingGate = { id: NO_CODE_GATE_ID, kind: "scoping" };
      if (verdict.ok) {
