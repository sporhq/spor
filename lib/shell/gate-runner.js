...
function buildGateFact({ gate, nodeId, runId, project, verdict, detail, evidence, attempts, escalatedTo, demotion, date, factory, attempt = 0, ledger = null, rescue = 0, rescueNext = null, outage = null, flake = null, change = null, definition = null }) {
...
function outageOf(result) {
  ...
  return { outcome, reason: String((cls && cls.reason) || (result && result.reason) || ""), pool: (cls && cls.pool) || null };
}
...
async function runGatePipeline({ item, factory, deps, log = () => {} }) {
  ...
  if (deps.checkEvidenceOrigins) {
    let origin;
    try { origin = await deps.checkEvidenceOrigins(); } catch (e) { origin = { ok: false, reason: e.message || String(e) }; }
  ...
  // Why a DECLARED no-code outcome did not check out, when one was declared —
  // carried into the empty-diff refusal below so the escalation shows the
  // run's own account beside the check that broke it.
  let noCodeRefusal = null;

  // RE-PIN the candidate (task-spor-factory-candidate-record, §3.3). Every run
  // that commits under the pipeline moves HEAD — a fix cycle commits, a rescue
  // amends — and a candidate left pinned to the first commit would be a record
  // of something the gates are no longer judging. So the pin runs at exactly
  // the point the runner already re-reads the tree, and the fold in
  // kernel/candidate.js decides what a re-pin MEANS: the same tree is the same
  // candidate (the new commit is a relabel and goes to `commits_seen`), a
  // different tree is a new candidate carrying `supersedes`.
  //
  // Fail-soft like every other stamp beside a verdict: a pin that could not be
  // read is logged and the pipeline judges the tree regardless. The candidate
  // is a record OF what was judged, never a precondition for judging it —
  // except for the one thing it exists to guarantee (§3.4): a candidate no
  // reader could obtain is not a candidate, and gate 0 must never start on
  // one (issue-spor-gate-start-not-conditional-on-candidate-submitted). So
  // the SUBMISSION pin (below, before the gate list starts) is checked
  // against `candidateSubmitted`; every later re-pin (a fix cycle, a rescue)
  // stays fail-soft exactly as before — the gate list is already running by
  // then, and a re-pin only ever refines a candidate already judged.
  //
  // `runId` names the run that actually made the commit being pinned — a fix
  // cycle's or a rescue's, not the implementation run whose record the pipeline
  // is stamping. Without it every re-pin would record the implementer's
  // provenance for a tree the implementer never produced, which is a silent
  // misattribution in the one block whose whole job is attribution.
  let pinnedCandidate = null;
  let pinFailureReason = null;
  // Set when a pin's own result is a benign NO-OP rather than evidence about
  // THIS pass's candidate — today, only `change: "refused-settled"`
  // (issue-spor-pin-candidate-settled-record-stamp-race): a late first pin
  // arriving after the run record already settled through a path that never
  // produced a candidate at all (a losing worker adopting an orphan another
  // worker just finished). That settlement is a fact about the RECORD, not a
  // failed submission of this pass's tree, so the submission check below must
  // not read it as "no candidate could be submitted" and escalate over it.
  let pinIgnorable = false;
  const pin = async (submittedBy, { runId = null } = {}) => {
    if (!factory.implementation || !deps.pinCandidate) return;
    try {
      const r = await deps.pinCandidate({ submittedBy, runId });
      if (r && r.ok) {
        ...
        // `publish_pending` names WHY submission is unsettled (§3.4) — a
        // pinned commit with no verified reference, not a pin failure — so it
        // is kept as the reason even though this branch is `ok:true`.
  ...
  //
  // `submittedBy` names which step of the pipeline is re-reading (§3.3): the
  // first read is the implementation stage's submission, and every later one
  // follows a commit some fixer made. It rides on the CANDIDATE the same read
  // pins, so a person reading the chain later can tell which step produced
  // which tree. A factory that declares no `implementation:` block pins
  // nothing and this is byte-identical to before the stage existed.
  const readChanged = async (submittedBy = { stage: "implementation", cycle: 0, rescue: 0 }, pinOpts = {}) => {
    ...
    changed = null;
    changedCwd = null;
    // A failed read after a fix cycle must not leave the PREVIOUS read's head
    // standing as evidence (cross-model review, major finding 4): the tree
    // moved and nobody could read where to, so the judged commit is unknown —
    // every fact and the returned chain say so rather than naming the
    // pre-fix head as if it were what was judged.
    change = null;
    return false;
  };
  ...
  //
  // Returns `{id, linked}`: the id of the fact THIS record is durably on (null
  // when it is on none), and the flake issues whose occurrence edge is durably
  // there with it. Neither is read off the write's bare `ok`, which is the
  // whole of F10: `writeGateNode` reports an already-occupied id as a SUCCESS
  // (`if_exists: skip` remotely, identical-content adoption locally), and an
  // occupied id is NOT this markdown landing.
  //
  // The fact's id is deterministic in this gate, this run and this pass, so
  // the occupant is USUALLY an earlier incarnation of this very record — but
  // "usually" is not the check-then-write race (durable row (c)): a resumed
  // pipeline, a second worker or a heal pass can have written a DIFFERENT
  // verdict under that id between our check and our write, and reporting our
  // verdict as recorded would leave the run, the escalation and the rescue's
  // `derived-from` all pointing at a node that says something else. So an
  // `existing` the write door cannot itself vouch for is reconciled by a READ
  // (`deps.readFact`), which answers both questions at once — is the occupant
  // this record, and which of the owed occurrence edges are on it. The local
  // door CAN vouch for it (it compared the markdown before adopting, and says
  // so with `identical`), and a fresh CREATE needs no read at all.
  //
  // Every answer is a POSITIVE reading or nothing: a read that could not be
  // MADE settles neither question, so the fact is reported unrecorded and the
  // occurrence stays owed — a duplicate edge overcounts one occurrence while a
  // missing one leaves an issue no fact names at all.
  const reconcileExisting = async (gate, id, markdown, owed, origin) => {
    let seen = null;
    try {
      seen = deps.readFact ? await deps.readFact({ id, markdown, nodeId, gate, flakeIssues: owed, origin }) : { ok: false, reason: "this worker has no door to read it back through" };
    } catch (e) {
    ...
    const has = (issue) => edges.some((e) => e && family(e.to) === family(issue) && String((e && e.type) || "") === FLAKE_EDGE);
    const paid = owed.filter(has);
    if (paid.length < owed.length) {
      log(
        `work: gate ${gate.id}'s fact ${id} was already on the graph without ${owed.filter((i) => !has(i)).join(", ")} on it — ` +
  ...
  // Pay what the fact could not carry (F13). An occupied fact id means this
  // markdown — and the occurrence edges in its frontmatter — did not land, and
  // for a PASSING gate or the final refusal there is no later fact of this
  // pass to carry them: the debt would sink there forever. So the edge is
  // written straight onto the fact that IS there, through the idempotent
  // add_edge door, at the moment the debt is discovered rather than deferred
  // to a flag some later pass has to find (durable row (b): there is nothing
  // to clear before owing, and no crash window between two writes). A payment
  // is recorded only from the door's own success (row (a): a stamp that did
  // not land pays nothing), the door dedups so a racing payer cannot
  // double-count (row (c)), and its target is the id the FILING door already
  // reconciled against settled state, refusing a dangling one (row (d)).
  const payFlakeEdges = async (gate, id, unpaid, flake, origin) => {
    if (!deps.linkFact) return [];
    const paid = [];
    for (const issue of unpaid) {
      let r = null;
      try {
        r = await deps.linkFact({ id, type: FLAKE_EDGE, to: issue, nodeId, gate, file: (flake.files || [])[flakeIssues(flake).indexOf(issue)], files: flake.files || [], origin });
      } catch (e) {
        r = { ok: false, reason: `${(e && e.message) || e}` };
      }
      if (r && r.ok) {
  ...
  // Takes the gate's OUTCOME object whole (never a hand-picked copy of it) and
  // the per-site extras — see `gateFactFields`.
  let pipelineEvidenceOrigin = null;
  try { pipelineEvidenceOrigin = deps.evidenceOrigin ? deps.evidenceOrigin() : null; } catch { /* a filing refuses below if its origin cannot be verified */ }
  const reportedEvidence = new Set();
  const reportEvidence = async (gate, outcome, { fact, candidate_id = null, head = null, rescue = 0 } = {}) => {
    if (!fact || !deps.gateEvidenceRecorded) return;
    const key = JSON.stringify([gate.id, rescue, fact, candidate_id, head]);
    if (reportedEvidence.has(key)) return;
    try {
      const reported = await deps.gateEvidenceRecorded({ gate, verdict: outcome.verdict, fact, candidate_id, head, rescue });
      // A failed local write has no outbox. Only an acknowledgement or the
      // reporter's durable outbox receipt makes another adoption redundant.
      if (reported && (reported.ok || reported.deferred)) reportedEvidence.add(key);
    } catch (e) { log(`work: gate ${gate.id} completed evidence could not be reported (${e.message || e})`); }
  };
  // Receipts can be adopted again after a later fix rewinds the gate list.
  // They replace the result for this gate/pass; immutable facts remain in the
  // evidence list, but a stale-head verdict must not survive beside its rerun.
  ...
  const record = async (gate, outcome, extras = {}, evidenceChange = change, evidenceDefinition = factory.definition || null, origin = pipelineEvidenceOrigin, evidenceCandidateId = pinnedCandidate && pinnedCandidate.candidate_id || null, continuation = null) => {
    const fields = gateFactFields(outcome, extras);
    const { verdict, detail, escalatedTo, rescue, flake } = fields;
    let factId = null;
    let linked = [];
    const owed = flakeEdges(flake);
    if (flakeIssues(flake).length && deps.acceptsEvidenceOrigin && !deps.acceptsEvidenceOrigin(origin)) return { id: null, linked: [], pending: true };
    // Write the obligation BEFORE attempting graph publication. A failed
    // payment leaves this exact outcome resumable without re-running the suite.
    const persistEvidence = async (paid, receipt = null) => {
      if (!flake && !flakeIssues(flake).length) return true;
      const key = `${rescue || 0}:${gate.id}`;
      const previous = passProgress.get(key) || await loadProgress(gate, rescue || 0) || {};
      ...
      try {
        if (deps.saveGateProgress) await deps.saveGateProgress({ gate, item, progress, ...(rescue ? { rescue } : {}) });
    ...
    try {
      const fact = buildGateFact({
        // The outcome's own fields FIRST, so the run's identity below can
        // never be displaced by one that happens to share a name.
        ...fields,
      ...
      const wrote = await deps.recordFact({ id: fact.id, markdown: fact.markdown, nodeId, gate, verdict, flakeIssues: owed, origin, candidate_id: evidenceCandidateId, head: evidenceChange && evidenceChange.head || null, ...(rescue ? { rescue } : {}) });
      if (wrote && (wrote.ok || wrote.existing)) {
        if (wrote.ok && (!wrote.existing || wrote.identical)) {
          // A fresh CREATE, or an occupant the write door itself compared
          // against this markdown before adopting it: either way this record
          // is on the graph, edges and all, with nothing left to read.
          factId = fact.id;
        ...
        // Whatever the fact could not carry is paid directly, here, where the
        // debt is known — never left for a later pass that may never come.
    ...
    const paid = flakePaidBy(flake, { id: factId, linked });
    const persisted = await persistEvidence(paid, { complete: flakeEdges(paid).length === 0, fact: factId, result: results[results.length - 1] });
    const pending = !persisted || flakeEdges(paid).length > 0;
    if (!pending) await reportEvidence(gate, fields, { fact: factId, candidate_id: evidenceCandidateId, head: evidenceChange && evidenceChange.head || null, rescue: rescue || 0 });
    return { id: factId, linked, pending };
  };

  // --- the shared INFRASTRUCTURE pool (FACTORY-IMPLEMENTATION-STAGE.md §5.3,
  // task-spor-factory-execution-outcome-classifier) ---
  //
  // ONE pool per PIPELINE, not per dispatch: `implementation.retry.attempts`
  // is the total number of infrastructure re-dispatches this whole pipeline
  // may make — the implementation stage, every review, every fix, every rescue
  // together. That is what keeps the dispatch bound a SUM and not a product
  // (§5.4), so a backend that is 404ing cannot multiply itself by the gate
  // list.
  //
  // It is durable for the same reason the ledger is: a killed worker's
  // pipeline is RESUMED (§10.8), and a pool that lived only in this process
  // would hand every resume a fresh allowance — an outage that outlives a
  // worker is exactly the case the pool exists to bound. So the count rides
  // `deps.saveGatePools` beside `gate_progress`, keyed to the same attempt,
  // and — unlike the ledger's fail-SOFT saves — a charge that does not land
  // REFUSES the retry it was paying for. Owe before you clear: a retry whose
  // charge nobody recorded is a retry the next resume would grant again.
  // A caller with no pool deps therefore gets NO infrastructure retries at
  // all (the fail-closed reading), never unbounded ones.
  let pools = { retry: { spent: 0 } };
  const loadPools = async () => {
    if (!deps.loadGatePools) return;
    try {
      ...
      // A pool we could not READ is treated as spent, not as fresh: the
      // bounded side. Rule 1 of the classifier applied to its own budget.
      log(`work: the infrastructure retry pool could not be read (${(e && e.message) || e}) — no infrastructure retry will be spent on ${nodeId}`);
      pools = { retry: { spent: gates.executionPoolCap(factory.implementation, "retry") } };
    }
  };
  await loadPools();

  // Spend one charge of the infrastructure pool on an OUTAGE, and wait out the
  // declared backoff before the same dispatch is re-made. Answers `true` when
  // the caller should ask again at the SAME cycle (nothing charged to the
  // gate's cycles, nothing folded into its ledger — §4.2 G6), and otherwise
  // the REASON it did not — which is not decoration: "the pool is spent" and
  // "the worker was asked to stop" settle the gate identically but are
  // different stories, and the escalation a person reads must be the true one.
  const spendOutage = async (gate, outage) => {
    // `unroutable` is off the ladder: a dispatch this box REFUSED before any
    // run record (an unsatisfiable profile, a launcher that does not resolve)
    // is a persistent misconfiguration, and waiting buys nothing. It spends
    // neither pool — a refusal is not an attempt (§4.2 I2, F8) — and the
    ...
    // No declared pool (a factory with no `implementation:` block), or a
    // caller with no durable place to record the charge: no retry. An
    // uncounted retry is an unbounded one, and a pool is precisely a loop
    // guard.
    const cap = gates.executionPoolCap(factory.implementation, "retry");
    if (cap <= 0 || !deps.saveGatePools) return "this factory declares no infrastructure retry pool (`implementation.retry.attempts`), so the outage was not waited out";
    ...
    // A worker already asked to stop spends nothing and waits for nothing: it
    // would only hold the process for a backoff and then a fresh dispatch it
    // cannot follow. Reported as its own reason, because the pool still has
    // headroom — the escalation must not tell a person the budget ran out
    // when what happened is that the worker was stopped.
    if (deps.stopping && deps.stopping()) return "the worker was asked to stop before the retry was taken — the pool still has headroom";
    ...
    // Sliced so a stop is answered inside the wait rather than at the end of
    // it, the same way the human gate slices its approval poll.
    const deadline = (deps.now ? deps.now() : Date.now()) + backoffMs;
    while (backoffMs > 0) {
      const at = deps.now ? deps.now() : Date.now();
      if (at >= deadline) break;
      if (deps.stopping && deps.stopping()) return "the worker was asked to stop during the backoff, after the retry had been charged";
      await deps.sleep(Math.min(OUTAGE_BACKOFF_SLICE_MS, Math.max(1, deadline - at)));
    }
    if (deps.stopping && deps.stopping()) return "the worker was asked to stop during the backoff, after the retry had been charged";
    return true;
  };
  ...
  // executing any gate. The production preflight enumerates the whole journal,
  // including later gates and rescue passes, not just the next declared gate.
  ...
  if (!Array.isArray(evidencePreflight && evidencePreflight.pending)) {
    // Custom/in-memory adapters may only expose per-gate progress. Preserve
    // their normal resume behavior while checking every available pass first.
    const passes = [...new Set([0, ...rescues.map((r) => Number(r.n) || 0)])];
    for (const rescue of passes) for (const gate of factory.gates || []) {
      let progress;
      try { progress = deps.loadGateProgress ? await deps.loadGateProgress({ gate, item, ...(rescue ? { rescue } : {}) }) : null; }
      catch (e) { return { state: "interrupted", gates: results, facts, reason: `pending gate evidence could not be inspected: ${e.message || e}` }; }
  ...
  for (const entry of evidenceEntries) {
    ...
    const key = `${rescue}:${gate && gate.id}`;
    if (inspected.has(key)) continue;
    inspected.add(key);
    if (!gate || !(factory.gates || []).some((g) => g.id === gate.id) || (deps.acceptsEvidenceOrigin && !deps.acceptsEvidenceOrigin(debt.origin))) {
    ...
    if (saved.filingIntent && !outcome && debt.draft) outcome = await finishFlakeFiling(debt.draft, { ...deps, filingOrigin: debt.origin }, log);
    ...
    const replay = await record(gate, outcome, saved.filingIntent ? { rescue, attempts: saved.attempts || [], ledger: saved.ledger || [] } : {}, debt.change, debt.definition || null, debt.origin, debt.candidate_id || null, saved.filingIntent ? { phase: "judge", cycle: debt.cycle } : debt.continuation || null);
    ...
    // The normal judge restores this now-completed receipt if it belongs to
    // the current candidate; preflight must not manufacture an extra verdict.
  ...
  // PREMATURE RESOLUTION at submission (task-spor-factory-controller-
  // completion-boundary, FACTORY-IMPLEMENTATION-STAGE.md §4.5 "When"): under
  // `completion.by: controller` the implementer was told to write NO resolving
  // edge; one written anyway is inert under the item's execution hold and is
  // retyped as evidence BEFORE any gate runs, so the first gate fact records
  // it. Optional dep (a factory under agent completion has none) and fail-
  // soft: an unreadable graph leaves the retype owed to the poll-time pass.
  if (deps.premature && factory.completion && factory.completion.by === "controller") {
    try {
      const p = await deps.premature({ item });
  ...
  const roundTrip = async (rescue = 0, seed = null) => {
    ...
    // The round-trip is the IMPLEMENTER's own one chance to commit what it
    // wrote (or a RESCUE's, inside a rescue pass), so the tree it produces is
    // that stage's submission — not a fix cycle's. Labelling it `fix` would put
    // a chain on the record whose first entry claims to fix something that was
    // never submitted.
  ...
  // STALE PREMISE (task-spor-factory-skip-resolved-items-with-empty-diff), a
  // SIBLING of the no-code-outcome route below that needs no declaration: an
  // item's own `commits:` stamps can already be landed on the trusted ref
  // BEFORE this pipeline ever ran anything — typically because a DIFFERENT
  ...
  // unlink named spor 0efea66, landed as part of task-spor-queue-api-offset-
  // paging's own fix) — so the branch this run cut for the item carries
  // nothing to add, and the run made no claim about it because there was
  // nothing for it to find. The evidence has to genuinely PREDATE the run for
  // there to be nothing to launder — so `deps.commitsLanded` checks the
  // item's `commits:` stamps AS CLAIMED at launch (the run record's
  // `item_commits`), never a live re-read: `commits:` is an ordinary editable
  // field, and reading the item's CURRENT stamps would let a run with
  // graph-write access append its own already-landed sha to its own item
  // mid-run and manufacture the very evidence this check exists to require
  // (the same hazard `item_repo` already avoids for the declared route's
  // re-stamp check, below).
  //
  // Checked BEFORE the declared no-code claim: it costs a few git probes,
  // needs no report parsing, and — being independent of
  // anything the run said — settling on it first means a run that also made a
  // (possibly malformed) declaration never has that declaration's failure
  // shown as the reason when the simpler, unconditional evidence already
  // answers the question. A run whose stamps do NOT check out here falls
  // straight through to the declared-claim check, and from there to the
  // ordinary empty-diff refusal exactly as before — this can only ever
  // remove a wrong escalation, never manufacture a pass.
  if (changed && changed.length === 0 && deps.commitsLanded) {
    let commitsLanded = null;
    try {
      commitsLanded = await deps.commitsLanded({ trustedRef: factory.trustedRef });
    } catch (e) {
  ...
  // NO-CODE OUTCOME (task-spor-factory-no-code-outcome-convention). An EMPTY
  // diff reaches the review gate's fail-closed refusal (§10.4) because the
  // gate cannot tell "did nothing" from "correctly found there was nothing to
  // do". A run that DID the second thing can say so — the fixed `SCOPED:` line
  // in its report naming the scoping node it wrote — and the runner checks
  // that claim against the graph before routing it. Everything is verified:
  // the diff really is empty and the tree clean (the read below succeeded, so
  // a dirty tree is already excluded), the named node really declares the
  // outcome and links the item, the item really moved, and any node the claim
  // says it found really exists. A claim that does not check out falls
  // straight through to the empty-diff refusal it would have got anyway,
  // carrying WHY it did not — so this can only ever remove a wrong escalation,
  // never manufacture a pass (dec-spor-gates-enforced-in-code-factory-is-data).
  //
  // Deliberately BEFORE the rescue state and the round-trip: there is no tree
  // to rescue and nothing to commit, so both would only spend a dispatch on a
  // run that is already finished.
  if (changed && changed.length === 0 && deps.noCodeClaim) {
    const claim = await (async () => {
      try {
        return await deps.noCodeClaim({ item });
      } catch (e) {
    ...
    if (claim) {
      // Every node the check consults, keyed by id; a missing entry is the
      // fail-closed reading ("not on the graph"), so a read that throws is
      // logged and simply left out.
      // Prototype-free: a node id can spell `constructor`, and an inherited
      // key would read as a node that exists (and short-circuit its read).
      const nodes = Object.create(null);
      const read = async (id) => {
        if (!id || !deps.node || id in nodes) return;
        nodes[id] = null;
        try {
          const r = await deps.node({ id, item });
      ...
      // Only a well-formed claim is worth reading the graph for: an
      // unreadable `SCOPED:` line is refused on its own terms below, before
      // any node is consulted.
      if (claim.ok) {
        await read(claim.resolver);
        await read(nodeId);
      }
      // ...and, for a FINDING outcome, the nodes the resolver names as what it
      // found: a `derived-from` onto an id nobody ever wrote is a dangling
      // edge, which the graph accepts by design and which demonstrates
      // nothing. Bounded — a resolver naming a dozen things is not naming one.
      ...
      // The synthetic gate this route files its fact under. It is not one of
      // the factory's declared gates — no gate ran — so it carries its own id
      // and kind, exactly as the rescue lane's fact does; `normalizeGate`
      // reserves the id so a declared gate can never mint the same fact.
      const scopingGate = { id: gates.NO_CODE_GATE_ID, kind: "scoping" };
      if (verdict.ok) {
        const reason = `scoped: ${verdict.detail}`;
        await record(scopingGate, { verdict: "scoped", detail: verdict.detail, passed: false });
      ...
      // A claim that did not check out is not silently dropped: the gate that
      // refuses next says the empty diff was CLAIMED as a no-code outcome and
      // what failed, so the person reading the escalation sees the run's own
      // account and the check that broke it.
  ...
  // issue-spor-gate-start-not-conditional-on-candidate-submitted): a factory
  // that declares an `implementation:` block pins — and publishes — a
  // candidate at every readChanged() above, the first of which is the
  // stage's own submission. Submission is not complete until the reference
  // verified: a candidate no reader could obtain is not a candidate, so gate
  // 0 must never start on one. Checked only on a FRESH pass — `rescues.length`
  // is nonzero only when resuming inside a rescue, which cannot exist unless
  // a real gate already ran and refused, so the candidate that gate judged
  // was necessarily submitted — and only once a tree was actually read
  // (`changed !== null`; an unreadable or still-dirty tree is the
  // pre-existing refusal below, unrelated to submission) and the pin's own
  // result was not a benign no-op (`pinIgnorable`). A factory declaring no
  // `implementation:` block pins nothing and never reaches here, so this is
  // byte-identical for every other factory.
  if (!rescues.length && factory.implementation && deps.pinCandidate && changed !== null && !pinIgnorable && !candidate.candidateSubmitted(pinnedCandidate)) {
    ...
    try {
      const esc = await deps.escalate({ gate, item, factory, attempts, detail: outcome.detail, evidence: "", findings: [], ledger: [] });
    ...
    // Mirrors the ordinary gate-refusal path's `escalation_retry` (below): an
    // escalation write that failed here must be as retriable as any other
    // refusal's, not a permanently-unretried exception. `stage` is the
    // discriminator `retryOneEscalation` uses to skip the declared-gate
    // lookup (this id is never a declared gate — it is reserved) and refuse
    // instead when the factory no longer declares an `implementation:` block.
    const escalationRetry = !escalatedTo
      ? {
          ...
          factId: gateFactId(gate.id, nodeId, runId, item.attempt || 0, 0),
        }
      : null;
    return {
      state: "failed",
  ...
  const judge = async (rescue, seed) => {
    ...
    const ancestorOfTip = (head) => {
      ...
      return !!(changedCwd && git(changedCwd, ["merge-base", "--is-ancestor", head, tip]).status === 0);
    };
    const reusable = (gate, head) => declarations.get(gate.id) === declaration(gate)
      && ((head || null) === (change && change.head || null) || (mayRetain(gate) && ancestorOfTip(head)
        && results.some((r) => r.gate === gate.id && (r.rescue || 0) === rescue && r.head === head && r.verdict === "passed" && r.candidate_id)));
    ...
    for (let gi = 0; gi < factory.gates.length; gi += 1) {
      ...
      if (change && change.head && reusable(gate, judged.get(gate.id))) {
        retainResult(gate);
      ...
      if (saved && saved.filingIntent) {
        ...
        if (deps.acceptsEvidenceOrigin && !deps.acceptsEvidenceOrigin(intent.origin)) return { evidencePending: true };
        const recovered = intent.outcome || await finishFlakeFiling(intent.draft, { ...deps, filingOrigin: intent.origin }, log);
        ...
        if ((intent.change && intent.change.head || null) === (change && change.head || null) && JSON.stringify(intent.definition || null) === JSON.stringify(factory.definition || null)) {
          ...
          resumedFilingOutcome = { ...recovered, flake: flakePaidBy(recovered.flake, replay) };
        }
      }
      if (saved && saved.evidence && saved.evidence.outcome) {
        const prior = saved.evidence;
        if (deps.acceptsEvidenceOrigin && !deps.acceptsEvidenceOrigin(prior.origin)) {
        ...
        if (prior.complete) {
          ...
          replay = { id: prior.fact, linked: flakeLinked(prior.payment_receipts || prior.outcome.flake), pending: false };
        ...
        if ((prior.change && prior.change.head || null) === (change && change.head || null) && JSON.stringify(prior.definition || null) === JSON.stringify(factory.definition || null)) {
          ...
          const recovered = { ...prior.outcome, flake: flakePaidBy(prior.outcome.flake, replay) };
          // Payment completion is independent of finishing this judgement.
          // A filing interrupted before its retry decision still has the
          // recorded fix cycles available; consume the outcome once below,
          // without running its suite or filing its occurrence again.
        ...
        // Pay historical debt at its original head before judging a new head.
      }
      const cached = saved && saved.retainedPass;
      if (mayRetain(gate) && cached && cached.result && cached.result.candidate_id && ancestorOfTip(cached.result.head) && cached.result.verdict === "passed"
        && cached.declaration === declaration(gate) && JSON.stringify(cached.scope) === JSON.stringify(cacheScope)
        && (!deps.acceptsEvidenceOrigin || deps.acceptsEvidenceOrigin(cached.origin))
      ...
      const saveFilingIntent = async (draft, cycle) => {
        if (deps.acceptsEvidenceOrigin && !deps.acceptsEvidenceOrigin(pipelineEvidenceOrigin)) throw new Error("flake filing origin could not be verified");
        filingIntent = { draft, gate, cycle, change, definition: factory.definition || null, origin: pipelineEvidenceOrigin, candidate_id: pinnedCandidate && pinnedCandidate.candidate_id || null };
        const p = progress(cycle);
        if (deps.saveGateProgress) await deps.saveGateProgress({ gate, item, progress: p, ...(rescue ? { rescue } : {}) });
      ...
      for (let cycle = startCycle; ; cycle += 1) {
        if (pendingFix) {
          ...
          else outcome = await runOneGate({ gate, cycle, factory, item, head: change && change.head, changed, changedReason, noCodeRefusal, deps: { ...deps, filingOrigin: pipelineEvidenceOrigin, saveFlakeIntent: (draft) => saveFilingIntent(draft, cycle) }, log, ledger, lastFix, rescue, base });
          if (outcome.evidencePending) return { evidencePending: true };
          // An OUTAGE is not a verdict on the change (§5.3, §4.2 G6/G7): the
          // gate's dispatch never answered, so nothing is folded into the
          // ledger, no attempt is recorded and no fix cycle is charged. The
          // shared infrastructure pool pays for asking the SAME gate again at
          // the SAME cycle; an exhausted pool (or a reading waiting cannot
          // help) falls through as the refusal it is, and the escalation then
          // names the outage rather than the code.
          while (outcome.outage) {
            const again = await spendOutage(gate, outcome.outage);
            if (again !== true) {
              // Why the retry was NOT taken rides the refusal, so the fact and
              // the escalation tell a person the true story rather than
              // "the budget ran out" for every reason a retry did not happen.
              outcome = { ...outcome, outage: { ...outcome.outage, notRetried: again } };
              break;
            }
            outcome = await runOneGate({ gate, cycle, factory, item, head: change && change.head, changed, changedReason, noCodeRefusal, deps: { ...deps, filingOrigin: pipelineEvidenceOrigin, saveFlakeIntent: (draft) => saveFilingIntent(draft, cycle) }, log, ledger, lastFix, rescue, base });
          }
          if (filingIntent) {
            ...
            try {
              if (deps.saveGateProgress) await deps.saveGateProgress({ gate, item, progress: p, ...(rescue ? { rescue } : {}) });
              passProgress.set(`${rescue}:${gate.id}`, JSON.parse(JSON.stringify(p)));
            } catch (e) { return { evidencePending: true }; }
          }
        ...
        if (!fixed || !fixed.ok) {
          // The fixer's dispatch is read through the same classifier (§5.3):
          // a fix REFUSED before any run record — an unsatisfiable lane
          // profile, a launcher that does not resolve — is `unroutable`, and
          // it says nothing about the change. It spends neither pool (a
          // refusal is not an attempt) and, carried here, it keeps the
          // refusal out of the rescue lane: a fixer that never ran left no
          // defect to diagnose, and rescuing one spends a strong-model
          // dispatch on nothing.
          const outage = outageOf(fixed);
          outcome = {
            passed: false,
            verdict: "failed",
            ...
            noRetry: true,
            // Carried so the refusal is not handed to the rescue lane: a fixer
            // that never ran left no defect to diagnose.
            ...(outage ? { outage } : {}),
          };
        ...
        await saveProgress(gate, progress(cycle + 1), rescue);
        // The FIXER's run is read through the same classifier as the review's
        // (§5.3), and what its class means depends on what it left behind. A
        // fixer that died on the environment and left HEAD exactly where it
        // was produced nothing to judge: re-reviewing an unchanged tree
        // re-raises the same finding and spends the rest of this gate's cycles
        // — and then the whole refusal — on an outage. One that COMMITTED
        // before it died did produce work, and that work is judged like any
        // other fix; the class says how the run ENDED, not whether its output
        // is worth looking at. Only a POSITIVE reading of "did not move" stops
        // the gate: an unreadable head is not evidence the tree stood still,
        // and falls through to the behaviour it always had.
        const fixOutage = outageOf(fixed);
        if (fixOutage && changedHead && fixFromHead && changedHead === fixFromHead) {
          ...
          // The attempt entry is the refusal's own record for the fact, and it
          // is deliberately NOT saved: the save above already left the shape
          // the resume logic expects for a cycle whose fix has launched
          // (`attempts.length === fixes - base`), and a second save would add
          // an entry the resume reads as a stray one — rolling it back,
          // re-running the cycle and dispatching a second fixer at the same
          // unmoved tree. This is exactly how the `!fixed.ok` refusal above
          // leaves its own progress, so the two refusal paths resume the same
          // way: the last cycle is asked again (a resumed worker is a fresh
          // ask, and the backend may be back), and if it is not, the review's
          // own pool-guarded outage path stops it.
          attempts.push({ verdict: outcome.verdict, detail: outcome.detail, passed: false });
          break;
        }
      }
      ...
      if (outcome.passed) {
        // A pass carries evidence only when a command gate passed on a RERUN
        // (the first attempt's failure) — every other passing outcome has
        // none, so the fact is unchanged for them.
        ...
        // All other steps may also opt out, so no rewind is guaranteed to
        // visit earlier passes. Mark retained evidence before deciding whether
        // a current-tip gate still needs to run.
        for (const g of factory.gates) if (reusable(g, judged.get(g.id))) retainResult(g);
        const moved = factory.gates.some((g) => judged.has(g.id) && !reusable(g, judged.get(g.id)));
        if (moved) gi = -1;
  ...
  // ...nor an EMPTY-diff refusal (`noRescue`, see `emptyDiffRefusal`): the
  // tree exists but carries no change for a rescue to work on, and the
  // question it leaves — is the graph write the deliverable? — is a person's.
  // ...nor an OUTAGE (§5.3): the rescue lane diagnoses a defect and fixes it in
  // the checkout, and a reviewer whose harness never answered found no defect
  // to diagnose. Rescuing one spends a strong-model dispatch on nothing and
  // then re-runs the whole gate list into the same dead backend.
    ...
    for (;;) {
      ...
      if (resumed) {
        ...
        if (refusal && changedGone && !refusal.outcome.escalatedTo && refusal.outcome.verdict !== "blocked" && rescues.length < factory.rescue.attempts) {
          ...
          log(`work: gate ${refusal.gate.id} refused ${nodeId} on an empty diff — a rescue has no change to work on; escalating straight to a person`);
        }
        ...
        const rec = refusal.recordedEvidence && refusal.recordedEvidence.outcome.rescueNext ? refusal.recordedEvidence : await record(gate, outcome, {
          ...
          // A charged off-diff pass still filed what it could (F7): the fact
          // that records the refusal is the one that links those issues.
          flake: outcome.flake || null,
        });
        // …and what it OBSERVABLY linked is folded straight back into the
        // refusal (F12). `outcome` IS `refusal.outcome`, which the escalation
        // below re-reads: without this, a rescue that cannot be dispatched
        // breaks to that escalation and its fact links the same occurrence a
        // second time — an issue two facts of one run name is an occurrence
        // that happened twice. One value, stamped into the in-memory refusal
        // and the durable entry alike, so an uninterrupted pass and a resumed
        // one settle the debt identically.
        if (rec.pending) return evidenceWaiting();
        if (outcome.flake) outcome.flake = flakePaidBy(outcome.flake, rec);
        entry = {
          ...
          // The flake filings this refusal carries (F9), carrying their own
          // per-issue discharge state (`linked`/`linkedBy`, F10/F11) — one
          // value, one write, alongside the `fact` id. A crash can never leave
          // the payload persisted beside a stale answer to "was it already
          // linked", and a resumed pipeline that escalates this carried
          // refusal (its rescue could not run, its checkout is gone) pays
          // exactly the edges the pre-rescue `record` above did not, instead
          // of either leaving the issues it filed with no fact naming them or
          // linking again what is already linked. What is NOT written here is
          // an inference from `fact` being non-null: a fact id says a node is
          // there, never that this markdown — and so these edges — landed.
          flake: outcome.flake || null,
  ...
  async function recordRescue(entry) {
    try {
      ...
      const wrote = await deps.recordFact({ id: fact.id, markdown: fact.markdown, nodeId, gate, verdict: entry.error ? "unrun" : entry.category || "unknown", rescue: entry.n });
      if (wrote && wrote.ok) {
        // The same reading a gate fact's write gets (F10): an occupied id is
        // not this markdown landing, and the rescue fact is what the tasks a
        // rescue files point at with `derived-from` — an anchor on a record
        // that is not there, or on another actor's, is worse than none. It
        // carries no occurrence edges, so there is no debt here to owe.
  ...
  // The non-gate, non-item, non-factory args this attempt handed
  // `deps.escalate` — captured up front so a failed write leaves behind
  // everything a LATER retry needs to make the identical call again
  // (task-spor-gate-escalation-bounded-auto-retry), keyed only by `gate.id`
  // and `item.attempt` (a retry reconstructs `gate`/`item`/`factory` from the
  // worker it runs in; `item.attempt` still has to travel with the payload
  // since it keys the escalation's deterministic id via gateRunKey/
  // shortRunAttempt — get it wrong and a retry mints a SECOND escalation
  // instead of finishing the first one).
  const escalateExtras = {
    ...
    // An OUTAGE refusal (§5.3): the escalation must say the reviewer never
    // answered, not that the gate refused the code after spending its fix
    // cycles — the misreading issue-spor-review-gate-reviewer-outage-read-as-
    // rejection is about. It rides the replayed payload so a RETRY of a failed
    // escalation write files the same text.
    ...(outcome.outage ? { outage: outcome.outage } : {}),
  };
  if (!escalatedTo && outcome.verdict !== "blocked") {
    try {
      const esc = await deps.escalate({ gate, item, factory, ...escalateExtras });
      if (esc && esc.ok) escalatedTo = esc.id;
      else log(`work: gate ${gate.id} escalation could not be filed (${(esc && esc.reason) || "no response"})`);
    } catch (e) {
      log(`work: gate ${gate.id} escalation could not be filed (${(e && e.message) || e})`);
  ...
  // The same condition that gated the `deps.escalate` attempt above: a
  // payload rides along exactly when that attempt ran and failed, never for a
  // human gate genuinely `blocked` on its own already-filed approval item
  // (escalatedTo is that item's id there, so this is never reached). Evidence
  // is capped the same way the escalation body itself caps it
  // (fenceSafe(...).slice(0, 3000)), so a retry payload never holds more than
  // the escalation could ever show.
  // `factId` is the refusal's OWN fact — the `art-gate-*` node written just
  // above, which states that no escalation could be filed. It is the
  // deterministic id (gateFactId), not `record()`'s return: the fact's write
  // usually fails for the same reason the escalation's did, and the closing
  // artifact a landed retry writes over it (bin/spor.js retryOneEscalation)
  // must name the node the fact WILL occupy either way — an edge onto an id
  // not yet on the graph is skipped by the compiler, never refused.
  const escalationRetry = !escalatedTo && outcome.verdict !== "blocked"
    ? {
        ...
        factId: gateFactId(gate.id, nodeId, runId, item.attempt || 0, rescue, change && change.head),
      }
    : null;
  return {
...
function refusalFromEntry(factory, entry) {
  ...
  return {
    gate,
    outcome: { passed: false, verdict: entry.verdict || "failed", detail: entry.detail || "", evidence: entry.evidence || "", findings: entry.findings || [], escalatedTo: null, flake },
  ...
  const detail =
    gate.kind === "command"
      ...
      : `the branch carries no committed change against ${factory.trustedRef} — an empty diff has nothing to review,` +
        ...
        declared;
  return { passed: false, verdict: "failed", noRetry: true, noRescue: true, detail };
}

async function runOneGate({ gate, cycle, factory, item, head = null, changed, changedReason, noCodeRefusal = null, deps, log, ledger = [], lastFix = null, rescue = 0, base = 0 }) {
  if (gate.kind === "command") {
    ...
    let isolation = null;
    const outputs = [];
    const suiteArgs = { gate, factory, item, trustedRef: factory.trustedRef, protectedPaths: factory.protectedPaths, armed: armed.classes };
    try {
      if (deps.openSuite) {
        try {
      ...
      for (;;) {
        attempt += 1;
        try {
          if (suite && !suite.ok) r = { ok: false, reason: suite.reason || "the gate tree could not be prepared" };
    ...
    if (r && r.ok) {
      ...
      if (firstFailure) {
        return {
          passed: true,
          verdict: "passed",
          detail: `${gates.describeRerun(gate.command, attempt, firstFailure.reason)}; judged against ${factory.trustedRef}'s copy of the protected paths${armedBy}`,
          evidence: failureEvidence(firstFailure.output || ""),
        };
      }
      return { passed: true, verdict: "passed", detail: `\`${gate.command}\` passed against ${factory.trustedRef}'s copy of the protected paths${armedBy}`, evidence: null };
    }
    ...
    return finishFlakeFiling(intent, deps, log);
  }
