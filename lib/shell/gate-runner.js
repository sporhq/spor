...
async function runGatePipeline({ item, factory, deps, log = () => {} }) {
  ...
  const payFlakeEdges = async (gate, id, unpaid, flake) => {
    if (!deps.linkFact) return [];
    const paid = [];
    for (const issue of unpaid) {
      let r = null;
      try {
        r = await deps.linkFact({ id, type: FLAKE_EDGE, to: issue, nodeId, gate, file: (flake.files || [])[flakeIssues(flake).indexOf(issue)], files: flake.files || [] });
      } catch (e) {
        r = { ok: false, reason: `${(e && e.message) || e}` };
      }
      if (r && r.ok) {
  ...
  const record = async (gate, outcome, extras = {}, evidenceChange = change, evidenceDefinition = factory.definition || null) => {
    ...
    const persistEvidence = async (paid, receipt = null) => {
      ...
      const progress = { ...previous, evidence: { outcome: { ...fields, flake: paid }, change: evidenceChange, definition: evidenceDefinition, ...(receipt || {}) } };
      try {
    ...
    try {
      ...
      const wrote = await deps.recordFact({ id: fact.id, markdown: fact.markdown, nodeId, gate, verdict, flakeIssues: owed, ...(rescue ? { rescue } : {}) });
      if (wrote && (wrote.ok || wrote.existing)) {
        ...
        if (factId && unpaid.length) linked = [...linked, ...(await payFlakeEdges(gate, factId, unpaid, flake))];
  ...
  const judge = async (rescue, seed) => {
    ...
    for (let gi = 0; gi < factory.gates.length; gi += 1) {
      ...
      if (saved && saved.evidence && saved.evidence.outcome) {
        ...
        } else replay = await record(gate, prior.outcome, {}, prior.change, prior.definition || null);
      ...
      for (let cycle = startCycle; ; cycle += 1) {
        if (pendingFix) {
          ...
          outcome = await runOneGate({ gate, cycle, factory, item, changed, changedReason, noCodeRefusal, deps, log, ledger, lastFix, rescue, base });
...
async function runOneGate({ gate, cycle, factory, item, changed, changedReason, noCodeRefusal = null, deps, log, ledger = [], lastFix = null, rescue = 0, base = 0 }) {
