...
function gateFactId(gateId, nodeId, runId) {
  const g = String(gateId || "gate").replace(/[^a-z0-9]+/gi, "-").toLowerCase().slice(0, 24).replace(/^-+|-+$/g, "");
  return `art-gate-${g || "gate"}-${stemOf(nodeId)}-${shortRun(runId)}-${gateIdSuffix("fact", gateId, nodeId, runId)}`;
}
...
function buildGateFact({ gate, nodeId, runId, project, verdict, detail, evidence, attempts, escalatedTo, date, factory }) {
  const id = gateFactId(gate.id, nodeId, runId);
  ...
  const lines = [
    ...
    `${passed ? "passed" : verdict === "blocked" ? "is blocking" : "failed"} for dispatched run \`${runId}\` on ${nodeId}${factory ? `, under factory \`${factory}\`` : ""}.`,
    "",
...
function prepareGateTree(change, { trustedRef, protectedPaths }) {
  let parent = null;
  try {
  ...
  if ((protectedPaths || []).length) {
    const ls = git(change.top, ["ls-tree", "-r", "--name-only", trustedRef], { maxBuffer: GIT_MAX_BUFFER });
    if (ls.status !== 0) {
      cleanup();
  ...
  return { ok: true, dir, restored, cleanup };
}
...
function runGateCommand(gate, dir) {
  ...
  return new Promise((resolve) => {
    ...
    try {
      child = spawn(gate.command, {
        ...
        detached: group,
        env: { ...process.env, CI: "1", SPOR_GATE: gate.id },
...
//
// Returns {state, gates: [...], facts: [...], reason}:
//   passed   every gate passed (or was not armed) — the item's resolution stands
//   failed   a gate failed; the work is not done, and an escalation names why
//   blocked  a human gate is filed and unanswered — the runner is WAITING, and
//            deliberately does not decide on the person's behalf
async function runGatePipeline({ item, factory, deps, log = () => {} }) {
  ...
  const record = async (gate, { verdict, detail, evidence, attempts, escalatedTo }) => {
    let factId = null;
    try {
      const fact = buildGateFact({
        gate, nodeId, runId, project: item.project || null, verdict, detail, evidence, attempts, escalatedTo,
  ...
  for (const gate of factory.gates) {
    ...
    if (outcome.passed) {
      await record(gate, { verdict: outcome.verdict, detail: outcome.detail, attempts });
    ...
    await record(gate, { verdict: outcome.verdict, detail: outcome.detail, evidence: outcome.evidence, attempts, escalatedTo });
    const state = outcome.verdict === "blocked" ? "blocked" : "failed";
    log(`work: gate ${gate.id} ${state} on ${nodeId} — ${outcome.detail || "no detail"}${escalatedTo ? ` (escalated to ${escalatedTo})` : ""}`);
    return { state, gates: results, facts, reason: `gate '${gate.id}' ${state}: ${outcome.detail || ""}`.trim(), escalated_to: escalatedTo };
  }
...
async function runOneGate({ gate, cycle, factory, item, changed, changedReason, deps, log }) {
  if (gate.kind === "command") {
    ...
    let r = null;
    try {
      r = await deps.runSuite({ gate, factory, item, trustedRef: factory.trustedRef, protectedPaths: factory.protectedPaths });
    } catch (e) {
      r = { ok: false, reason: `the gate command could not be run: ${(e && e.message) || e}` };
    }
    if (r && r.ok) {
      return { passed: true, verdict: "passed", detail: `\`${gate.command}\` passed against ${factory.trustedRef}'s copy of the protected paths`, evidence: null };
    }
    return {
      passed: false,
      verdict: "failed",
      detail: (r && r.reason) || (r && r.code != null ? `\`${gate.command}\` exited ${r.code}` : "the gate command failed"),
      evidence: tailBytes((r && r.output) || ""),
    };
  }
