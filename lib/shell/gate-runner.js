...
async function runGatePipeline({ item, factory, deps, log = () => {} }) {
  ...
  const record = async (gate, outcome, extras = {}, evidenceChange = change, evidenceDefinition = factory.definition || null, origin = pipelineEvidenceOrigin, evidenceCandidateId = pinnedCandidate && pinnedCandidate.candidate_id || null) => {
    ...
    const persistEvidence = async (paid, receipt = null) => {
      ...
      const progress = { ...rest, evidence: { outcome: fields, payment_receipts: paid, change: evidenceChange, definition: evidenceDefinition, gate, origin, candidate_id: evidenceCandidateId, ...(receipt || {}) } };
      try {
    ...
    return { id: factId, linked, pending: !persisted || flakeEdges(paid).length > 0 };
  };
  ...
  for (const entry of evidenceEntries) {
    ...
    const replay = await record(gate, outcome, saved.filingIntent ? { rescue, attempts: saved.attempts || [], ledger: saved.ledger || [] } : {}, debt.change, debt.definition || null, debt.origin, debt.candidate_id || null);
  ...
  const judge = async (rescue, seed) => {
    ...
    for (let gi = 0; gi < factory.gates.length; gi += 1) {
      ...
      if (saved && saved.filingIntent) {
        ...
        const replay = await record(intent.gate || gate, recovered, { rescue, attempts: saved.attempts || [], ledger: saved.ledger || [] }, intent.change, intent.definition, intent.origin, intent.candidate_id);
      ...
      if (saved && saved.evidence && saved.evidence.outcome) {
        ...
        if (prior.complete) {
          ...
          if (prior.result) results.push(prior.result);
          replay = { id: prior.fact, linked: flakeLinked(prior.payment_receipts || prior.outcome.flake), pending: false };
        } else replay = await record(prior.gate || gate, prior.outcome, {}, prior.change, prior.definition || null, prior.origin, prior.candidate_id || null);
        if (replay.pending) return { evidencePending: true };
        if ((prior.change && prior.change.head || null) === (change && change.head || null) && JSON.stringify(prior.definition || null) === JSON.stringify(factory.definition || null)) {
          ...
          return { gate, outcome: { ...prior.outcome, flake: flakePaidBy(prior.outcome.flake, replay) }, attempts: prior.outcome.attempts || [], ledger: prior.outcome.ledger || [], recordedEvidence: { ...replay, outcome: prior.outcome } };
        }
      ...
      if (mayRetain(gate) && cached && cached.result && cached.result.candidate_id && ancestorOfTip(cached.result.head) && cached.result.verdict === "passed"
        ...
        const index = results.findIndex((r) => r.gate === gate.id && (r.rescue || 0) === rescue);
      ...
      for (let cycle = startCycle; ; cycle += 1) {
        if (pendingFix) {
          ...
          if (retry && filingIntent) {
            const rec = await record(gate, outcome, { attempts: shownAttempts(gate, attempts, rescue), ledger, rescue, startedAt });
      ...
      if (outcome.passed) {
        ...
        const prior = results.findIndex((r) => r.gate === gate.id && (r.rescue || 0) === rescue);
        if (prior >= 0) results[prior] = current; else results.push(current);
        remember(gate, change && change.head || null);
        if (mayRetain(gate)) {
