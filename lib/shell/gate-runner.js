  ...
  if (gate.kind === "command") {
    ...
    try {
      for (;;) {
        attempt += 1;
        try {
          r = await deps.runSuite({ gate, factory, item, trustedRef: factory.trustedRef, protectedPaths: factory.protectedPaths, armed: armed.classes, attempt });
        } catch (e) {
    ...
    return {
      ...
      detail: attempt > 1 ? `${reason} — on every one of ${attempt} runs of the same tree (${attempt - 1} rerun${attempt - 1 === 1 ? "" : "s"} declared)` : reason,
