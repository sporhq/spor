...
//
// COMMIT-BOUND as well (task-spor-factory-gate-attestation, review finding 1):
// a fix cycle advances the implementer's HEAD, and the earlier gates are then
// re-run against the new head — so one gate can legitimately record TWO facts
// in one run, one per head it judged. Folding the head into the identity keeps
// them distinct (and keeps a retried write of the SAME judgement idempotent);
// a fact whose head could not be read keeps the bare run key.
function gateFactId(gateId, nodeId, runId, attempt = 0, head = null) {
  const g = String(gateId || "gate").replace(/[^a-z0-9]+/gi, "-").toLowerCase().slice(0, 24).replace(/^-+|-+$/g, "");
  const key = head ? `${gateRunKey(runId, attempt)}@${String(head)}` : gateRunKey(runId, attempt);
  return `art-gate-${g || "gate"}-${stemOf(nodeId)}-${shortRunAttempt(runId, attempt)}-${gateIdSuffix("fact", gateId, nodeId, key)}`;
}
...
function buildGateFact({ gate, nodeId, runId, project, verdict, detail, evidence, attempts, escalatedTo, demotion, date, factory, attempt = 0, change = null, definition = null }) {
  ...
  const id = gateFactId(gate.id, nodeId, runId, attempt, head);
...
async function runGatePipeline({ item, factory, deps, log = () => {} }) {
  ...
  // The record slot per gate INDEX (not appended): a gate re-run against a
  // moved head REPLACES its earlier verdict in the result, while the earlier
  // fact stays on the graph (and in `facts`) as the record of what was judged
  // at the old head.
  const slot = (gi, r) => {
    results[gi] = r;
  };

  // Every gate must have judged the SAME head (review finding 1). Gate A passes
  // at H1; gate B fails and its fix cycle commits H2; B passes at H2 — A never
  // saw H2, and an attestation reading "H2, all passed" would be a lie. So a
  // fix cycle that moves the head sends the pipeline back to the first gate,
  // and a gate whose recorded pass already judged the CURRENT head stands
  // (never re-dispatching a review at a head it already approved). Bounded:
  // a restart only ever follows a fix cycle, and fix cycles are charged
  // against each gate's cap CUMULATIVELY across restarts (`spent`), so the
  // whole pipeline runs at most sum(cycles) fixes before it settles.
  const spent = new Map();
  const headOf = (r) => (r && r.head) || null;
  const currentHead = () => (change ? change.head : null);
  const alreadyJudged = (gi) => {
    const r = results[gi];
    const head = currentHead();
    return !!(r && (r.verdict === "passed" || r.verdict === "skipped") && head && headOf(r) === head);
  };
  const headMoved = () => {
    const head = currentHead();
    return results.some((r) => r && headOf(r) !== head);
  };

  let gi = 0;
  while (gi < factory.gates.length) {
    ...
    const startedAt = now();
    let moved = false;
    for (let cycle = spent.get(gate.id) || 0; ; cycle += 1) {
      ...
      await readChanged();
      if (currentHead() !== before) moved = true;
    }
    ...
    if (outcome.passed) {
      ...
      if (moved && headMoved()) {
        log(`work: the fix cycle for gate ${gate.id} moved ${nodeId}'s head to ${String(currentHead() || "unknown").slice(0, 12)} — re-running the earlier gates against it`);
        gi = 0;
        continue;
      }
      gi += 1;
    ...
    return {
      state, gates: results.filter(Boolean), facts,
  ...
  return { state: "passed", gates: results.filter(Boolean), facts, reason: `${results.length} gate(s) passed`, ...chain() };
}
...
async function runOneGate({ gate, cycle, factory, item, changed, changedReason, deps, log }) {
  ...
  if (gate.kind === "agent-review") {
    // An UNREADABLE change is not a reviewable one either (review finding 3):
    // the command and human gates already refuse it, and a review that passed
    // on a change nobody could read would mint a passing fact with no head —
    // which the attestation cannot bind to a commit and the integration
    // stage's head-equality check cannot compare against. Fail closed and
    // unretried: no fix cycle makes an unreadable tree readable.
    if (!changed) {
      return { passed: false, verdict: "failed", detail: changedReason || "the change under judgement could not be read, so there is nothing a review can be bound to", noRetry: true };
    }
