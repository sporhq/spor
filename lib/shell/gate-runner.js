...
function buildGateFact({ gate, nodeId, runId, project, verdict, detail, evidence, attempts, escalatedTo, demotion, date, factory, attempt = 0, ledger = null, rescue = 0, rescueNext = null, flake = null }) {
  ...
  const lines = [
    ...
    flake && (flake.files || []).length
      ? `Off-diff flake: ${flake.files.join(", ")} failed the whole-suite run and passed alone on the same tree${flakeIssues(flake).length ? `, filed as ${flakeIssues(flake).join(", ")}` : " (the flake could not be filed as its own issue)"}.`
      : "",
