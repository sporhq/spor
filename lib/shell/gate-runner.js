...
function buildGateFact({ gate, nodeId, runId, project, verdict, detail, evidence, attempts, escalatedTo, demotion, date, factory, attempt = 0, ledger = null, rescue = 0, rescueNext = null, flake = null }) {
  ...
  const lines = [
    ...
    // The flake issue an off-diff pass filed. The edge is what makes the
    // telemetry aggregatable BY FILE: the issue node is keyed on the failing
    // files, so every gate fact that ever tripped over the same flake points
    // at the one node, and its inbound edges are the occurrence count.
    ...(flake && flake.issue ? [`  - {type: relates-to, to: ${flake.issue}}`] : []),
    "---",
    "",
    ...
    flake && (flake.files || []).length
      ? `Off-diff flake: ${flake.files.join(", ")} failed the whole-suite run and passed alone on the same tree${flake.issue ? `, filed as ${flake.issue}` : " (the flake could not be filed as its own issue)"}.`
      : "",
...
async function runOneGate({ gate, cycle, factory, item, changed, changedReason, deps, log, ledger = [], lastFix = null, rescue = 0, base = 0 }) {
  if (gate.kind === "command") {
    ...
    try {
      ...
      //       when the gate declares `isolate`, the failure is demonstrably
      //       off the change's diff, and the named test files are few.
      //
      ...
      if (!(r && r.ok) && !(suite && !suite.ok)) {
        const failed = firstFailure || r || {};
        classified = gates.offDiffFailure(gates.failingFiles((r && r.output) || failed.output || "", { dir: (suite && suite.dir) || "" }), changed);
        const tests = gate.isolate && classified.offDiff ? gates.isolatableTests(classified.files) : [];
        if (tests.length) {
    ...
    // change's diff, and those files passed on their own on that same tree. The
    // gate passes — charging this would spend the item's fix cycles, its rescue
    // and finally a person on work that was never wrong — but it is never
    // laundered into a clean run: the whole-suite failure rides the fact as
    // evidence, the outcome names the flake, and the flake is filed as its own
    // issue so the flaky FILE is what gets fixed. Filing is best-effort in the
    // sense every gate-filed node is: it is not a debt anything later owes,
    // because the finding is already durable on the gate fact this pass writes
    // — a failed filing costs the convergent per-file issue, not the record.
    if (isolation && isolation.ok) {
      ...
      if (deps.fileFlakeItem) {
        try {
          ...
          const f = await deps.fileFlakeItem({ gate, item, files: isolation.files, command: gate.command, isolate: isolation.command });
          if (f && f.ok) filed = f.id;
          // An id already OCCUPIED by different content is a refusal for every
          // other node a gate files — adopting a stranger's approval item would
          // pass a gate nobody looked at. Here it is the opposite: the id is
          // keyed on the failing FILES and on nothing else, so the occupant IS
          // this flake's issue, and the difference is a person having triaged
          // it. Link it; do not rewrite it, and do not report it unfiled.
          else if (f && f.existing && f.id) {
            filed = f.id;
            log(`work: the off-diff flake in ${isolation.files.join(", ")} is already filed as ${f.id}, since edited — linking that issue rather than rewriting it`);
          } else log(`work: the off-diff flake in ${isolation.files.join(", ")} could not be filed (${(f && f.reason) || "no response"}) — it stands on the gate fact regardless`);
        } catch (e) {
          log(`work: the off-diff flake in ${isolation.files.join(", ")} could not be filed (${(e && e.message) || e}) — it stands on the gate fact regardless`);
        }
      }
      const armedBy = armed.classes.length ? ` (armed by ${armed.classes.map((c) => c.class).join(", ")})` : "";
      return {
        ...
        evidence: failureEvidence(firstFailure.output || ""),
        flake: { files: isolation.files, issue: filed },
      };
    }
