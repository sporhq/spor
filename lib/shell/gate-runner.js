...
async function runGatePipeline({ item, factory, deps, log = () => {} }) {
  ...
  //
  // Returns `{id, linked}`: the fact's id (the node is there, whoever wrote
  // it — that is what a `derived-from` from the rescue's filings needs), and
  // the flake issues this write is KNOWN to have linked. The two are not the
  // same answer, which is the whole of F10: `writeGateNode` reports an
  // already-occupied id as a SUCCESS (`if_exists: skip` remotely,
  // identical-content adoption locally), and an occupied id is NOT this
  // markdown landing. For the fact itself that is immaterial — the id is
  // deterministic in this gate, this run and this pass, so the occupant is an
  // earlier incarnation of this very record. For the occurrence EDGES it
  // carries it is the whole question, because that earlier incarnation's flake
  // set may have been smaller (a filing that failed then and lands now; a
  // different set of files failing under load). So `existing` is sent through
  // a READ once — the same reconciliation the flake filing door makes for the
  // same reason — and only the issues the occupant actually names count as
  // paid. A read that could not be MADE settles nothing: the debt stays owed
  // and the refusal's next fact pays it, since a duplicate edge overcounts one
  // occurrence while a missing one leaves an issue no fact names at all.
  const confirmFlakeEdges = async (gate, id, owed, wrote) => {
    if (!owed.length) return [];
    if (!wrote.existing) return owed;
    let seen = null;
    try {
      seen = deps.factEdges ? await deps.factEdges({ id, nodeId, gate }) : { ok: false, reason: "this worker has no door to read it back through" };
    } catch (e) {
    ...
    if (!seen || !seen.ok) {
      log(
        `work: gate ${gate.id}'s fact ${id} was already on the graph and could not be read back (${(seen && seen.reason) || "no response"}) — ` +
          `the occurrence of ${owed.join(", ")} stays owed rather than assumed recorded`
      );
      return [];
    }
    const targets = new Set((seen.targets || []).filter(Boolean).map(String));
    const paid = owed.filter((i) => targets.has(i));
    if (paid.length < owed.length) {
      log(
        `work: gate ${gate.id}'s fact ${id} was already on the graph without ${owed.filter((i) => !targets.has(i)).join(", ")} on it — ` +
  ...
  const record = async (gate, { verdict, detail, evidence, attempts, escalatedTo, demotion, ledger, rescue = 0, rescueNext = null, flake = null }) => {
    ...
    try {
      ...
      if (wrote && wrote.ok) {
        factId = fact.id;
        linked = await confirmFlakeEdges(gate, fact.id, owed, wrote);
