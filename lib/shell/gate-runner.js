...
// How many nodes a FINDING outcome's "what it found" edges are read back from
// the graph before the check gives up (task-spor-factory-no-code-outcome-
// convention). A resolver naming a dozen things is not naming one, and each
// entry is a graph round trip on a path that only ever runs for an empty diff.
  ...
  if (changed && changed.length === 0 && deps.noCodeClaim) {
    ...
    if (claim) {
      ...
      if (resolver && (claim.outcome === "premise-stale" || claim.outcome === "duplicate")) {
        for (const cand of gates.noCodeFoundCandidates({ nodeId, resolver }).slice(0, NO_CODE_FOUND_READS)) await read(cand);
      }
