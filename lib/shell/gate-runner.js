    ...
    if (changed && changed.length === 0) {
      return {
        ...
        detail:
          ...
          // A run that DECLARED a no-code outcome (§10.11) and failed the
          // check gets the same refusal, plus what broke — otherwise the
          // escalation reads as if the run said nothing, and a person has to
          // re-derive from the report what the runner already worked out.
          (noCodeRefusal ? `. The run declared a no-code outcome, but it does not check out: ${noCodeRefusal}` : ""),
      };
    }
