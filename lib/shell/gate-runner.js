...
function changeReferencedBy(dir, seeds, changed, { also = [] } = {}) {
  ...
  for (let i = 0; i < queue.length; i += 1) {
    ...
    while ((spec = packageImport.exec(src)) !== null) {
      if (!require("module").isBuiltin(spec[2])) return { reached, unknown: `${rel} uses package entry ${spec[2]}, whose references were not resolved` };
    }
...
async function runGatePipeline({ item, factory, deps, log = () => {} }) {
  ...
  const results = [];
  const facts = [];
  // Refuse an unrelated graph before even pinning or repairing a dirty tree.
    ...
    for (let gi = 0; gi < factory.gates.length; gi += 1) {
      ...
      if (saved && saved.evidence && saved.evidence.outcome) {
        ...
        if (prior.complete) {
          ...
          replay = { id: prior.fact, linked: [], pending: false };
