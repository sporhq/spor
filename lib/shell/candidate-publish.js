...
const { networkGit } = require("./git-network.js");
...
// Built per call by the shared helper: preserve transport settings and the
// injected git seam, with publication's existing five-minute budget.
const netGit = (git, cwd, args, transportCwd = null) => networkGit(git, cwd, args, { timeout: GIT_NET_TIMEOUT_MS, transportCwd });
...
function verifyPublished(cand, { kind, locator, source, base, git = defaultGit }) {
  ...
  try {
    ...
    if (kind === "branch") fetched = netGit(git, scratch, ["fetch", "--no-tags", "-q", "--depth=1", locator, spec], source);
    if (fetched.status !== 0) fetched = kind === "branch" ? netGit(git, scratch, ["fetch", "--no-tags", "-q", locator, spec], source) : git(scratch, ["fetch", "--no-tags", "-q", locator, spec]);
    if (fetched.status !== 0) {
