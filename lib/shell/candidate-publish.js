...
const NET_OPTS = {
  ...
  env: { ...process.env, GIT_TERMINAL_PROMPT: "0", GIT_ASKPASS: "echo", SSH_ASKPASS: "echo", GIT_SSH_COMMAND: "ssh -oBatchMode=yes" },
};
const netGit = (git, cwd, args) => git(cwd, args, NET_OPTS);
...
function putFileStore(store, key, bytes) {
  ...
  try {
    ...
    try {
      ...
      // finished file over the reservation.
    ...
    // OCCUPIED, not yet a conflict. WHAT is under the id is settled by
    // fetching it and asking whether it resolves to the pinned commit and
    // tree — never by comparing bytes, because `git bundle create` is not
    // byte-reproducible (threaded delta search alone changes the packfile run
    // to run), so a legitimate retry after a landed put produces different
    // bytes for the same content and a byte comparison would call it
    // corruption forever.
...
// under the id is decided by fetching it and checking the commit and tree it
// resolves to, for the same non-reproducibility reason the file arm gives.
  ...
  try {
    ...
    if (kind === "bundle") {
      ...
      if (verified.status !== 0) {
        const why = String(verified.stderr || "");
