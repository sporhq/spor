...
async function personForceRelease({ home, store, origin, nodeId, executionId, reason, clearHold, requestId = crypto.randomUUID() }) {
  ...
  // A successful HTTP exchange with an unreadable/empty body is ambiguous:
  // the release may already be committed. Only a recognized definitive
  // refusal can retire a new intent without authoritative receipt readback.
  const definitiveCodes = new Set(["unauthorized", "forbidden", "not_found", "conflict", "invalid_event", "invalid_node", "execution_terminal"]);
  const definitive = !released.ok && !released.transport && !released.transient && definitiveCodes.has(released.code)
    && (released.status == null || (released.status >= 400 && released.status < 500));
  if (definitive) {
