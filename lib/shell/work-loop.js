...
// against it first, and a failed or blocked gate cools the node off instead of
// counting it done. The loop itself decides nothing about a gate — it holds the
// slot, folds the verdict into the status surface, and cools the node; the
// pipeline owns the enforcement. Adoption therefore has no cliff in either
// direction: no factory, no behavior change at all.
//
...
async function runWorkLoop({ opts = {}, deps, control = {} }) {
  ...
  for (;;) {
    ...
    // 1b. Settle finished gate pipelines. A pipeline that PASSED leaves the
    //     item as the run left it (resolved, out of the queue); one that failed
    //     or is blocked on a person cools the node off — the work is not done,
    //     and this worker re-dispatching it on the next poll would just race
    //     its own escalation. The verdict is folded into the run's own recent
    //     entry rather than a second one: one run, one line, two dimensions.
    ...
    //    it; the loop says so rather than leaving a half-gated item looking
    //    judged, and the pipeline's facts are idempotent, so re-gating that run
    //    on a later worker picks up where it left off.
    if (stopping()) {
      if (status.gating.length) {
        log(`work: ${status.gating.length} gate pipeline(s) abandoned by the stop — their items stay uncleared ('spor work' re-gates on the next run)`);
      }
      break;
    }
