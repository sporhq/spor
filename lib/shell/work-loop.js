...
//
//   - a slot — ACTIVE or GATING — held by a worker that is NOT live. `active`
//     counts too: a worker killed with runs in flight never reaches the harvest
//     that would have started their gates, and those runs go terminal anyway;
...
function orphanedGateRuns(statuses, { records = new Map(), terminalStates = null, now = Date.now, maxAgeMs = 0 } = {}) {
  ...
  for (const w of statuses || []) {
    if (!w || w.live) continue;
    for (const slot of [...(w.gating || []), ...(w.active || [])]) {
