...
function selectWorkCandidates(items, { skipped = new Map(), active = new Set(), now = Date.now(), accept = WORK_DEFAULTS.accept, repos = null, graph = null, onSkip = null } = {}) {
  const out = [];
  ...
  for (const it of items || []) {
    ...
    const verdict = classifyWorkItem(it, { accept, repos, scope, graph });
    if (verdict) {
...
// {reason, kind}.
function classifyWorkItem(it, { accept = WORK_DEFAULTS.accept, repos = null, scope = null, graph = null } = {}) {
...
async function runWorkLoop({ opts = {}, deps, control = {} }) {
  ...
  const status = {
    ...
    ...(deps.gate ? { gates: { passed: 0, failed: 0, blocked: 0, parked: 0, superseded: 0, scoped: 0 }, factory: opts.factory || null, ...(repos.length ? { repos } : {}) } : {}),
    active: [],
    gating: [],
    recent: [],
    skipped: [],
  ...
  const settleGates = () => {
    ...
    for (const g of status.gating) {
      ...
      // loop for as long as the graph stayed unwritable. The recorded door is
      // `spor work --regate <run>` instead — it judges the run RECORD, which a
      // failed escalation leaves untouched, so the whole pipeline can be
      // re-run by hand once the graph is writable (WORKERS.md §10.7).
      markGate(g.run_id, {
        gate_state: state,
