...
function readWorkerStatuses(home, { alive = () => true, now = Date.now, maxAgeMs = WORK_DEFAULTS.statusRetentionMs } = {}) {
  ...
  for (const name of names) {
    ...
    const live = !stopped && rec.pid != null && alive(rec.pid);
    rec.live = live;
...
// A one-line reason from a dispatch refusal: its FIRST stderr line, which is
// the refusal itself ("cannot dispatch X here: …"); the lines after it are
// remediation the operator already saw on the console. Bounded so a runaway
// message can't bloat the status file.
function refusalReason(lines, fallback = "dispatch refused") {
  const first = (lines || []).map((l) => String(l || "").trim()).find(Boolean);
  return (first || fallback).slice(0, 300);
}

// Fold one terminal run record into the worker's counters. The outcome
// dimension is the run's (WORKERS.md §8) — read, never recomputed — and an
// UNENFORCED verdict is counted separately rather than tallied with the
// verified ones, so a box whose server was unreachable can't read as a box
// that resolved things (dec-spor-dispatch-terminal-states-supervised-first).
    ...
    if (status.active.length) {
      ...
      for (const a of status.active) {
        ...
        if (a.node_id && outcome.terminal_state !== "resolved") {
          skipped.set(a.node_id, {
            reason: `last run here ended ${outcome.terminal_state || outcome.state || "without a verdict"}${outcome.report_node_id ? ` (report ${outcome.report_node_id})` : ""}`,
    ...
    if (free > 0 && !quotaReached && !draining) {
      ...
      for (const item of cands) {
        ...
        if (res && res.ok && res.run && res.run.run_id) {
          ...
          skipped.set(item.id, { reason, at: new Date(now()).toISOString(), until: now() + retryAfterMs });
          while (skipped.size > SKIP_CAP) skipped.delete(skipped.keys().next().value);
