...
function selectWorkCandidates(items, { skipped = new Map(), active = new Set(), now = Date.now(), accept = WORK_DEFAULTS.accept, onSkip = null } = {}) {
  const out = [];
  for (const it of items || []) {
    ...
    if (accept !== "open" && it.readiness !== "agent") {
      if (onSkip) onSkip(it, `not agent-ready; work.accept ${accept}`);
      continue;
    }
    out.push(it);
  }
...
async function runWorkLoop({ opts = {}, deps, control = {} }) {
  ...
  const status = {
    ...
    ...(deps.gate ? { gates: { passed: 0, failed: 0, blocked: 0, parked: 0 }, factory: opts.factory || null } : {}),
    active: [],
    gating: [],
    recent: [],
    skipped: [],
