...
function selectWorkCandidates(items, { skipped = new Map(), active = new Set(), now = Date.now(), accept = WORK_DEFAULTS.accept, repos = null, onSkip = null } = {}) {
  const out = [];
  // Empty/absent scope => unscoped, and every check below is a no-op, so a
  // bare loop and a factory that declares no repos stay byte-identical.
  ...
  for (const it of items || []) {
    ...
    // Scope before policy: an item this factory may not judge at all is out of
    // scope, not merely un-consented, and the reason should say the useful
    // thing. Like the policy check it runs AFTER the cooldown, so a cooling
    // out-of-scope item is not re-reported every poll.
    ...
    if (!gates.inRepoScope(stamp, scope)) {
      if (onSkip) onSkip(it, `outside the factory's repo scope (${stamp ? `repo ${stamp}` : "no project stamp"}; this factory judges ${scopeLabel()})`, "scope");
      continue;
    }
    if (accept !== "open" && it.readiness !== "agent") {
      if (onSkip) onSkip(it, `not agent-ready; work.accept ${accept}`, "policy");
      continue;
    }
    out.push(it);
  }
      ...
      if (free > 0 && !stopping()) {
        ...
        const cands = selectWorkCandidates(items || [], {
          ...
          onSkip: (it, reason, kind) => {
            ...
            cool(it.id, reason);
            log(`work: skipping ${it.id} — ${reason}`);
          },
        });
