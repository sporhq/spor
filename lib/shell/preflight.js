...
// custom harness (dispatch.harness.<id>) without an optional posture still
// leaves it to the operator-bound argv. With a posture, its adapter exposes
// that fixed meaning through the same surface as the built-in harnesses.
// Foreign flags remain refused by validateOptions in either case.
...
//   `allowAttended` — the caller has ALREADY read this posture, translated it
//       by meaning (dispatchHarnesses.postureMeaning) and deliberately chosen
//       to run it here rather than widen it — the rescue lane's own posture
//       translation (issue-spor-rescue-posture-attended-translation-hard-
//       refuses), never a general escape hatch: there is still no plain
//       `--force` for an ordinary worker with no posture at all. An ATTENDED
//       reading then proceeds rather than refuses, accepting the same stall
//       risk a pre-preflight rescue always ran (WORKERS.md §10.10) — a hard
//       refusal here would lose the rescue's diagnosis entirely, which a
//       stall does not.
function checkWritePosture({ adapter, options, unattended = false, readOnly = false, harnessId = null, allowAttended = false }) {
  ...
  if (meaning === "attended" && allowAttended) return base;
...
function liveWorkspaceWriters(records, {
  ...
  // the silence-past-staleMs fallback for exactly the unverifiable case — passed
  // this function's own `staleMs` (NATIVE_STALE_MS by default) rather than left
  // to default to `supervisorStillWatching`'s own 24h, so an unverifiable
  // supervised writer is bounded by the SAME 1h horizon the native-background
  // fallback below already uses for the identical reason: one exported
  // constant, two consumers, not a second stale-off-Linux literal
  // (issue-spor-preflight-live-writers-24h-stale-off-linux).
  const stillWatching = watching || ((r) => dispatchRuns.supervisorStillWatching(r, { staleMs }).watching);
  const out = [];
  for (const r of records || []) {
    ...
    if (r.runner_pid != null) {
      ...
      // `nativeAgents` now carries `state: "done"` entries too (they are
      // `reconcileRuns`'s own evidence to reap and settle by,
      // issue-spor-native-run-done-state-unreachable-and-contract-pending-invisible),
      // but a done agent is not a WRITER — it has stopped touching this
      // checkout, whatever the identity match says — so occupancy filters it
      // out here rather than trusting `isRunLive`'s bare identity match.
      const writing = (nativeAgents || []).filter((a) => String((a && a.state) || "").toLowerCase() !== "done");
      if (!dispatchRuns.isRunLive(r, writing)) continue;
    } else {
