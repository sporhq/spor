...
//   - a native-background record has no supervisor pid to probe, so it counts
//     while it is non-terminal AND young enough to still plausibly be running;
//   - a `read_only` run is not a writer and never occupies anything.
function liveWorkspaceWriters(records, { dir, watching = null, now = Date.now, staleMs = NATIVE_STALE_MS, excludeRunId = null } = {}) {
