...
function readRecord(home, tenant, id) {
  ...
  return readJsonFile(recordPath(home, tenant, id), "execution record");
}

function readJsonl(abs, label) {
  let text;
  try {
    text = fs.readFileSync(abs, "utf8");
  } catch (e) {
  ...
  for (const line of text.split("\n")) {
    ...
    try {
      ...
      // a torn tail line (a crash mid-append); everything before it is intact
    }
  }
  return out;
}
...
function readEvents(home, tenant, id) {
  ...
  return readJsonl(eventsPath(home, tenant, id), `execution event log for ${id}`);
}
...
function openExecutionFor(home, tenant, nodeId) {
  ...
  return readRecord(home, tenant, ptr.open);
}
...
function listExecutions(home, tenant, { nodeId = null, stage = null, limit = 100 } = {}) {
...
function rebuildFromEvents(home, tenant, id) {
  return kernel.rebuildFromEvents(readEvents(home, tenant, id), { sha256 });
}
...
function localExecutionEngine({ home, tenant = "local", worker = "local", machine = null, now = nowIso, ttlMs = DEFAULT_LEASE_TTL_MS, pinRead = null }) {
  ...
  const pin = ({ nodeId, factoryId, gates }) => {
    ...
    return {
      ...
      gates: (gates || []).map((g) => {
        ...
        return { id: String(g.id), node_id: g && g.node_id != null ? String(g.node_id) : null, revision: gNode && gNode.revision != null ? String(gNode.revision) : null };
      }),
    };
  };
  ...
  async function open(args) {
    ...
    return withItemLock(`${t}/${nodeId}`, async () => {
      ...
      if (existing) {
        ...
        const mine = existing.owner && existing.owner.worker === worker;
      ...
      const claimed = kernel.claim(record, { worker, machine: args.machine != null ? args.machine : machine, lease_expires_at: expiryFrom(at, clampTtl(args.ttl_ms || ttlMs)), now: at });
  ...
  const ownershipEvent = (record, at, owner) => ({ type: "ownership.changed", execution_id: record.execution_id, seq: record.seq, at, owner });
  ...
  return {
    ...
    worker,
    open,
    claim: (id, { takeover = false, ttl_ms = null, machine: m = null } = {}) =>
      withExecution(id, async ({ record, at }) => {
        const r = kernel.claim(record, { worker, machine: m != null ? m : machine, lease_expires_at: expiryFrom(at, clampTtl(ttl_ms || ttlMs)), now: at, takeover: takeover === true });
        if (!r.ok) return r;
    ...
    event: (id, { fence, event } = {}) =>
      withExecution(id, async ({ record, at }) => {
        ...
        if (kernel.isTerminal(r.record)) writeItemPointer(home, t, record.item.node_id, { open: null, add: record.execution_id });
...
const EVENT_REJECTED = new Set(["invalid_event", "unknown_gate", "candidate_conflict", "no_candidate", "gates_unsettled", "boundary_not_reached", "invalid_node"]);
...
function remoteExecutionAdapter(cfg, remote, { home, tenant = "remote", timeoutMs = DEFAULT_TIMEOUT_MS, log = () => {} }) {
  ...
  const cached = (id) => {
    for (const part of [t, ...tenants(home).filter((x) => x !== t)]) {
      try {
        const rec = readRecord(home, part, id);
        if (rec) return rec;
      } catch {
  ...
  const answer = (r) => {
    ...
    return fail(e.code, e.message, { status: r.status, holder: e.holder, ...(r.status >= 500 || unserved ? { transient: true } : {}), ...(unserved || absent ? { unserved: true } : {}) });
  };

  const post = (p, body) => remote.post(cfg, p, body, { timeoutMs });
  ...
  return {
    ...
    list: async ({ node_id = null, stage = null, limit = 50 } = {}) => {
      ...
      if (r.ok || !r.transport) return r;
      const rows = [];
      for (const part of [t, ...tenants(home).filter((x) => x !== t)]) rows.push(...listExecutions(home, part, { nodeId: node_id, stage, limit: 200 }));
...
function openExecutionStore(cfg, { home, mode = null, tenant = null, worker = null, machine = null, ttlMs = null, timeoutMs = null, pinRead = null, log = () => {}, remote = null } = {}) {
  ...
  if (resolvedMode === "remote") {
    ...
    const adapter = remoteExecutionAdapter(cfg, rem, { home, tenant: tn, timeoutMs: timeout, log });
...
function withOwnershipCheck(store) {
  return {
    ...store,
    // The box's name rides every open unless the caller names one; the
    // worker principal is the identity's (remote) or the engine's (local).
    ...
    confirmOwnership: async (id, fence) => {
      ...
      return { ok: false, confirmed: false, reason: `${r.code}: ${r.message}`, ownership: !OWNERSHIP_LOST.has(r.code) && r.code !== "execution_terminal" && r.code !== "not_found", holder: r.holder || null, code: r.code, transport: !!r.transport };
    },
    ...
    terminate: async (id, { fence, attempt = 1, outcome = "cancelled", reason = null } = {}) =>
      store.event(id, { fence, event: { type: "stage.observed", attempt, state: "exhausted", outcome, ...(reason ? { reason } : {}) } }),
  };
}
