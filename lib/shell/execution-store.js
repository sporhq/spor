...
function localExecutionEngine({ home, tenant = "local", worker = "local", machine = null, now = nowIso, ttlMs = DEFAULT_LEASE_TTL_MS, pinRead = null, person = null }) {
  ...
  async function open(args) {
    ...
    return withLocalItemLock(home, t, nodeId, () => {
      ...
      if (existing) {
        ...
        if (!existing.owner) {
          const claimed = kernel.claim(existing, { worker, machine, lease_expires_at: expiryFrom(at, clampTtl(args.ttl_ms || ttlMs)), now: at });
          if (!claimed.ok) return claimed;
          existing = claimed.record;
          writeRecord(home, t, existing, ownershipEvent(existing, at, existing.owner));
        }
        const mine = owns(existing);
...
function openExecutionStore(cfg, { home, mode = null, tenant = null, worker = null, machine = require("node:os").hostname(), ttlMs = null, timeoutMs = null, pinRead = null, person = null, log = () => {}, remote = null } = {}) {
  ...
  const engine = localExecutionEngine({ home, tenant: tenant || "local", worker: worker || "local", machine, ttlMs: ttl, pinRead, person });
