...
//   now()             -> ms
//   execution         -> OPTIONAL (task-spor-client-execution-store-adapter):
//                         the execution store's doors for this record —
//                         {confirm() -> {ok, confirmed, reason},
//                          completed(resolverId), ended(why)}. `confirm` runs
//                         BEFORE the resolving edge is written and a
//                         non-confirmation keeps the debt owed (EXECUTION-
//                         STATE.md §8 rule 3: never the edge on the strength of
//                         local state while the fence cannot be confirmed);
//                         `completed`/`ended` are reports, fail-soft. Absent
//                         (a legacy record, a pre-adapter claim): byte-
//                         identical to before.
"use strict";
...
async function withdrawCompletion({ record, deps, log = () => {}, why, touchItem = true }) {
  ...
  deps.stamp({ completion_debt: null, completion_withdrawn_at: isoNow(deps), completion_note: why, ...(deps.execution?.ended ? { completion_execution_end: { execution_id: record.impl_claim.execution_id, reason: why } } : {}) });
  log(`work: ${nodeId} — completion withdrawn: ${why}`);
  await reportExecution(deps, "ended", why);
  return { ok: true, settled: "withdrawn" };
}

// A report to the execution store, fail-soft: `completed` after the CAS
// landed, `ended` when the pipeline settles without a completion. Never
// changes the outcome the caller already reached.
async function reportExecution(deps, door, arg) {
  const ex = deps && deps.execution;
  if (!ex || typeof ex[door] !== "function") return;
  try {
    await ex[door](arg);
  } catch {
    /* the store's copy is owed; the record is the client's evidence */
  }
}
...
async function writeCompletion({ record, deps, log = () => {}, facts = [], boundary = null, date = null }) {
  ...
  for (let attempt = 0; attempt < 4; attempt += 1) {
    ...
    if (!holder) {
      ...
      deps.stamp({ completion_debt: null, completion_withdrawn_at: isoNow(deps), completion_note: "the execution was released by a person before the completion landed; nothing written", ...(deps.execution?.ended ? { completion_execution_end: { execution_id: executionId, reason: "the execution was released by a person before the completion landed" } } : {}) });
      log(`work: ${nodeId} — execution ${executionId} was released before the completion landed; nothing written`);
      await reportExecution(deps, "ended", "the execution was released by a person before the completion landed");
    ...
    // The FENCE, before the edge (EXECUTION-STATE.md §8 rule 3). The item's
    // hold says the item is ours on the graph; the execution store says
    // whether this worker still OWNS the execution — flushing what it owes
    // the store (gate verdicts a partition kept back) and renewing under the
    // fence. Not confirmed — the store unreachable, a fence taken over, a
    // lease expired — leaves the debt owed for a later pass rather than
    // writing the one edge that releases dependents on local state alone.
    if (deps.execution && typeof deps.execution.confirm === "function") {
      let confirmed;
      try {
        confirmed = await deps.execution.confirm();
      } catch (e) {
        confirmed = { ok: false, confirmed: false, reason: String((e && e.message) || e) };
      }
      if (!confirmed || !confirmed.confirmed) {
        return { ok: false, retry: true, reason: `${nodeId}: the execution fence could not be confirmed (${(confirmed && confirmed.reason) || "no answer"}) — the completion stays owed` };
      }
    }
    ...
    if (wrote && wrote.ok) {
      ...
      await reportExecution(deps, "completed", resolverId);
...
async function consume({ record, deps, log = () => {}, why }) {
  deps.stamp({ completion_debt: null, completion_consumed_at: isoNow(deps), completion_note: why, ...(deps.execution?.ended ? { completion_execution_end: { execution_id: record.impl_claim.execution_id, reason: why } } : {}) });
