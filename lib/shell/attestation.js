...
function buildUnboundAttestation({ item, factory, gate, integration = null, environment = {}, now = Date.now }) {
  ...
  const declaredRetention = (id) => factory && factory.completion && factory.completion.by === "controller" && (factory.gates || []).some((g) => g.id === id && g.kind === "command" && g.rejudgeOnRepin === false);
  ...
  const steps = latest.map((s) => ({
    ...
    ...(s.retained === true ? { retained: true, rejudge_on_repin: s.rejudge_on_repin } : {}),
    base: s.base || null,
  ...
  const policyConsistent = !!gatedHead && steps.every((s) => s.head === gatedHead || retainedStepAllowed({ configIntegrity, factory: def.factory }, s));
...
function coreSteps(g) {
  ...
  return (g.steps || []).map((s) => ({ id: nz(s.id), kind: nz(s.kind), verdict: nz(s.verdict), passed: s.passed === true, head: nz(s.head), digest: nz(s.digest), fact: nz(s.fact), ...(s.candidate_id ? { candidate_id: s.candidate_id } : {}), ...(s.retained !== undefined ? { retained: s.retained, rejudge_on_repin: s.rejudge_on_repin } : {}) }));
}
