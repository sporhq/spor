...
function attestationCore(att) {
  ...
  return {
    ...
    integration: i ? { mode: nz(i.mode), state: nz(i.state), head: nz(i.head), gated_head: nz(i.gated_head), head_matches_gated: nz(i.head_matches_gated), landed_sha: nz(i.landed_sha) } : null,
    configIntegrity: {
      ...
      protected_paths: Array.isArray(ci.protected_paths) ? [...ci.protected_paths] : [],
    },
...
function bindAttestation(att, signing = null) {
  ...
  att.configIntegrity = { ...(att.configIntegrity || {}), gates_count: ((att.configIntegrity && att.configIntegrity.gates) || []).length, gates_digest: gatesDigest(att.configIntegrity || {}) };
...
//             id): same id, same digest — the PR body is a copy of the graph's
...
function verifyAttestation(att, { key = null, keyId = null, now = Date.now, maxAgeMs = null, commit = null, factoryDigest = null, trusted = null, requireSignature = false, requireTrusted = false } = {}) {
  const checks = [];
  ...
  if (trusted) {
    ...
    add("trusted", sameId && sameDigest, !sameId ? `the graph artifact is ${trusted.id || "unknown"}, not ${att.id || "unknown"}` : sameDigest ? `bound to graph artifact ${trusted.id}` : `the graph artifact ${trusted.id} carries digest ${trusted.digest || "none"}, this copy ${att.digest || "none"}`);
...
function renderPrBody({ attestation, branch, base, intro = null }) {
  ...
  return [
    ...
    "",
    `A CI job can validate this instead of re-running the suite — but this text is not evidence by itself (anyone who can edit the PR can edit it). The evidence is the graph artifact \`${attestation.id}\` the runner wrote, which the PR author does not write through: fetch it (\`spor get ${attestation.id}\`) and require its \`digest\` to equal this copy's (\`${attestation.digest || "unbound"}\`)${attestation.signature ? `, and/or verify the \`signature\` (${attestation.signature.alg}, key \`${attestation.signature.key_id}\`, over the canonical JSON of the bound core) with the key CI holds` : ""}. Then \`subject.commit\` must equal the PR head, \`gate.allPassed\` must be true, \`issued_at\` must be fresh, and \`configIntegrity\` digests must match the factory as it stands (\`spor get ${attestation.factory.id || "<factory>"}\`; sha256 over the canonical JSON of the normalized definition). \`spor attestation verify --pr-body <file> --commit <sha> --max-age <ms>\` runs every check.`,
    "",
    PR_BEGIN,
    "```json",
