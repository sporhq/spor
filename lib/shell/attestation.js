...
function buildUnboundAttestation({ item, factory, gate, integration = null, environment = {}, now = Date.now }) {
  ...
  // Rescue runs retain historical refusals in gates[] as well as facts[];
  // only the latest pass judges the current candidate. Historical facts stay
  // linked on the artifact, but are not verdicts on the rescued head.
  const history = gate && gate.gates ? gate.gates : [];
  const pass = history.reduce((n, s) => Math.max(n, Number(s.rescue) || 0), 0);
  const latest = history.filter((s) => (Number(s.rescue) || 0) === pass);
  const effective = gatesKernel.describeDefinition(factory);
  const pinnedPolicy = !!(def.factory && effective.factory.digest === def.factory.digest);
  const declaredRetention = (id) => pinnedPolicy && factory && factory.completion && factory.completion.by === "controller" && (factory.gates || []).some((g) => g.id === id && g.kind === "command" && g.rejudgeOnRepin === false);
  const configIntegrity = {
    factory: { id: (def.factory && def.factory.id) || null, revision: (def.factory && def.factory.revision) || null, digest: (def.factory && def.factory.digest) || null,
      ...(latest.some((s) => s.retained) && factory && factory.completion && factory.completion.by === "controller" ? { completion_by: "controller" } : {}) },
    gates: (def.gates || []).map((g) => ({ id: g.id, source: g.source || "inline", revision: g.revision || null, digest: g.digest || null,
      ...(latest.some((s) => s.gate === g.id && s.retained) && declaredRetention(g.id) ? { kind: "command", rejudge_on_repin: false } : {}) })),
  ...
  const steps = latest.map((s) => ({
    ...
    head: s.head || null,
    ...(s.candidate_id ? { candidate_id: s.candidate_id } : {}),
    ...(s.retained === true ? { retained: true, rejudge_on_repin: s.rejudge_on_repin, retained_for: s.retained_for, ancestry_verified: s.ancestry_verified } : {}),
    base: s.base || null,
  ...
  // Each step judges the current head unless a pinned controller command
  // explicitly accepts a verified ancestor result. Keep head_consistent honest
  // and sign the separate policy exception, including its original candidate.
  // Ordinarily every step must have judged the SAME head, which must be known
  ...
  const policyConsistent = !!gatedHead && steps.every((s) => s.head === gatedHead || retainedStepAllowed({ configIntegrity, factory: def.factory, gate: { head: gatedHead } }, s));
  const allPassed = gateState === "passed" && steps.every((s) => s.passed) && policyConsistent;
  ...
  return {
    ...
    gate: {
      ...
      ...(!headConsistent && policyConsistent ? { policy_consistent: true } : {}),
...
function coreSteps(g) {
  ...
  return (g.steps || []).map((s) => ({ id: nz(s.id), kind: nz(s.kind), verdict: nz(s.verdict), passed: s.passed === true, head: nz(s.head), digest: nz(s.digest), fact: nz(s.fact), ...(s.candidate_id ? { candidate_id: s.candidate_id } : {}), ...(s.retained !== undefined ? { retained: s.retained, rejudge_on_repin: s.rejudge_on_repin, retained_for: s.retained_for, ancestry_verified: s.ancestry_verified } : {}) }));
}
function coreGates(ci) {
  const nz = (v) => (v === undefined ? null : v);
  return (ci.gates || []).map((x) => ({ id: nz(x.id), source: nz(x.source), revision: nz(x.revision), digest: nz(x.digest), ...(x.rejudge_on_repin !== undefined ? { kind: x.kind, rejudge_on_repin: x.rejudge_on_repin } : {}) }));
}
...
function attestationCore(att) {
  ...
  return {
    ...
    gate: {
      ...
      ...(g.policy_consistent !== undefined ? { policy_consistent: g.policy_consistent === true } : {}),
...
function retainedStepAllowed(att, step) {
  const ci = att.configIntegrity || {};
  const pinned = (ci.gates || []).find((g) => g.id === step.id);
  return !!(step.kind === "command" && step.retained === true && step.rejudge_on_repin === false && step.head && step.candidate_id
    && step.verdict === "passed" && step.ancestry_verified === true && step.retained_for === (att.gate && att.gate.head)
    && ci.factory && ci.factory.completion_by === "controller" && ci.factory.digest && att.factory && ci.factory.digest === att.factory.digest
    && pinned && pinned.kind === "command" && pinned.rejudge_on_repin === false && pinned.digest && pinned.digest === step.digest);
}
...
function evidenceFailures(att) {
  ...
  if (g.head_consistent !== true && g.policy_consistent !== true) out.push(`gate.head_consistent is ${JSON.stringify(g.head_consistent)}`);
  ...
  if (steps) {
    ...
    for (const st of steps) {
      ...
      if ((!st.head || st.head !== g.head) && !retainedStepAllowed(att, st)) out.push(`gate '${st.id}' judged ${st.head || "an unknown head"}, not the gated head ${g.head || "unknown"}`);
    }
...
function compactAttestation(att, level) {
  ...
  if (level < 3) return a;
  a.gate.steps = coreSteps(att.gate);
...
// Replace only the block we own; retain human and other automation text.
function mergePrBody(current, generated) {
  const outerBegin = "<!-- spor-proposal:begin -->";
  const outerEnd = "<!-- spor-proposal:end -->";
  const begin = generated.includes(outerBegin) ? outerBegin : PR_BEGIN;
  const end = begin === outerBegin ? outerEnd : PR_END;
  const from = generated.indexOf(begin), to = generated.indexOf(end, from + begin.length);
  if (from < 0 || to < 0) throw new Error("generated PR body has no complete attestation block");
  const replacement = generated.slice(from, to + end.length);
  const oldBegin = current.includes(outerBegin) ? outerBegin : PR_BEGIN;
  const oldEnd = oldBegin === outerBegin ? outerEnd : PR_END;
  const a = current.indexOf(oldBegin), b = current.indexOf(oldEnd, a + oldBegin.length);
  if (a < 0 && b < 0) return current + (current.endsWith("\n\n") ? "" : "\n\n") + replacement + "\n";
  if (a < 0 || b < a || current.indexOf(oldBegin, a + oldBegin.length) >= 0 || current.indexOf(oldEnd, b + oldEnd.length) >= 0) throw new Error("PR contains ambiguous attestation markers; body left unchanged");
  return current.slice(0, a) + replacement + current.slice(b + oldEnd.length);
}
