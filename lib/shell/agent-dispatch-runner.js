...
// the meantime). And release is checked, never blind: the holder wrote a random
// token into its lock and unlinks the lock path only while that token is still
// what the path holds — a lock re-taken under the same name by someone else is
// theirs to release. Both halves are exercised in test/gate-pipeline.test.js.
...
// caller may go on to acquire), false when there was nothing to break or what
// was taken turned out to be live and was handed back.
function breakStaleLock(lock, { staleMs, now }) {
  const taken = `${lock}.stale-${lockToken().replace(/:/g, "-")}`;
  try {
    fs.renameSync(lock, taken);
  } catch {
    return false; // another breaker won the rename, or the holder released — retry the acquire
  }
  let age = null;
  try {
    age = now() - fs.statSync(taken).mtimeMs;
  } catch {
    return false;
  }
  if (age > staleMs) {
  ...
  // Live: put it back for its holder. `link` never replaces — if the path is
  // already taken again, the holder's checked release simply finds a token
  // that is not its own and leaves the newcomer's lock alone.
  try { fs.linkSync(taken, lock); } catch { /* the lock path is already someone else's */ }
  ...
  for (let i = 0; i < attempts; i += 1) {
    ...
    try {
      ...
      try {
        ...
        continue; // the holder released between our open and stat — retry at once
      }
      if (age > staleMs) {
        breakStaleLock(lock, { staleMs, now });
        continue;
      }
      sleepSync(waitMs);
      continue;
...
function stampGateState(home, runId, patch, { verifyAttempts = 3, readBack = readJson, force = false, own = null, lock = withRecordLock } = {}) {
  ...
  try {
    ...
    const held = lock(file, () => {
      ...
      for (let attempt = 0; ; attempt += 1) {
        ...
        } else if (!force && record.gate_state && gatesKernel.SETTLED_GATE_STATES.has(record.gate_state)) return record;
...
const JUDGE_ONLY_ENV = Object.freeze(["SPOR_ATTESTATION_KEY", "SPOR_ADMIN_TOKEN", "SPOR_REFRESH_TOKEN"]);
...
async function runJob(jobFile) {
  ...
  return new Promise((resolve) => {
    ...
    child.on("close", async (code, signal) => {
      // A safety net for a `close` with neither `spawn` nor `error` observed
...
module.exports = {
  ...
  supervisorAliveProbe, isSameSupervisor, supervisorStillWatching,
  terminalOutcomeBackfill, mergeTerminalOutcome, stampGateState, claimGateRecord, withRecordLock, breakStaleLock, recordLockPath, RECORD_LOCK_STALE_MS, RECORD_LOCK_ATTEMPTS, RECORD_LOCK_WAIT_MS, writeRecordCarryingGate,
