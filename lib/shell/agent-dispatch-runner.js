...
function closeRun(recordFile, patch, fromState = null) {
  try {
    ...
    const held = withRecordLock(recordFile, step);
    return held.ok ? held.value : step();
  } catch {
    return null;
  }
}
...
//
// Fail-soft like every journal write: a lock that cannot be taken in the
// bounded wait still writes (unlocked, carrying whatever is on disk at that
// instant) — a supervisor's terminal outcome must land even if a settler died
// holding the lock for the 30s it takes to go stale. The window that leaves
// is the lock's own failure, not the ordinary path.
function writeRecordCarryingGate(file, next) {
  const write = () => atomicJson(file, carryGateFields(file, next));
  const held = withRecordLock(file, write);
  if (!held.ok) write();
}
...
// atomic on every filesystem Node runs on, including Windows). A lock left by
// a worker that died mid-stamp is broken once it is older than
// RECORD_LOCK_STALE_MS — a stamp is a few milliseconds, so a lock that old is
// a corpse, not a writer. A lock that cannot be taken within the bounded wait
// is a stamp that DID NOT LAND (null), the same fail-soft answer as any other
// write here: the resume scan re-offers the run; nothing is ever double-owned.
...
function withRecordLock(file, fn, { attempts = RECORD_LOCK_ATTEMPTS, waitMs = RECORD_LOCK_WAIT_MS, staleMs = RECORD_LOCK_STALE_MS, now = Date.now } = {}) {
  ...
  for (let i = 0; i < attempts; i += 1) {
    ...
    try {
      ...
      if (age > staleMs) {
        try { fs.unlinkSync(lock); } catch { /* someone else broke it first */ }
        continue;
      }
      sleepSync(waitMs);
      continue;
    ...
    try {
      try { fs.writeSync(fd, `${process.pid}\n`); } catch { /* the pid is a courtesy for a person reading the corpse */ }
...
async function runJob(jobFile) {
  ...
  const update = (patch) => {
    ...
    // landing mid-write is never renamed over (writeRecordCarryingGate).
...
module.exports = {
  ...
  terminalOutcomeBackfill, mergeTerminalOutcome, stampGateState, claimGateRecord, withRecordLock, recordLockPath, RECORD_LOCK_STALE_MS, writeRecordCarryingGate,
