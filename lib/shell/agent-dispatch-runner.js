...
//
// Held off while a SUPERVISED run's supervisor is still identifiably alive
// (isSameSupervisor — the same identity check finalizeSupervisedRun uses, not
// a bare pid probe, so a pid the kernel has since recycled to an unrelated
// process cannot hold this open forever): that process may be mid-contract
// right now, and its verified outcome is the one that should land. Once it is
// gone — or the pid demonstrably belongs to someone else now — the record is
...
function terminalOutcomeBackfill(record) {
  ...
  if (record.launch_mode === "supervised-jsonl" && isSameSupervisor(record.runner_pid, record.runner_started_ticks).reallyAlive) return null;
...
function stampGateState(home, runId, patch, { verifyAttempts = 3, readBack = readJson } = {}) {
  ...
  try {
    ...
    for (let attempt = 0; ; attempt += 1) {
      ...
      if (record.gate_state && gatesKernel.SETTLED_GATE_STATES.has(record.gate_state)) return record;
...
module.exports = {
  ...
  supervisorAliveProbe, isSameSupervisor,
