...
// wait OUTLASTS the stale window: a lock is always eventually taken — released
...
// overlap. The breaker lock is held for microseconds by a live breaker, so
// one older than the stale window is a dead breaker's and is removed
// outright; the residual is a breaker that died mid-break (its moved lock is
// left aside and the path stays empty), which costs the waiters that window,
// never a double hold of a live section. Release is checked, never blind: the
...
// release function otherwise. A breaker lock older than the stale window is
// a dead breaker's and is removed so the next breaker can take it.
function takeBreakerLock(lock, { staleMs, now }) {
  ...
  try {
    ...
    if (e && e.code === "EEXIST") {
      const age = fileAge(breaker, now);
      if (age != null && age > staleMs) {
        try { fs.unlinkSync(breaker); } catch { /* another breaker removed it */ }
      }
    }
    return null;
  }
...
function withRecordLock(file, fn, { attempts = RECORD_LOCK_ATTEMPTS, waitMs = RECORD_LOCK_WAIT_MS, staleMs = RECORD_LOCK_STALE_MS, now = Date.now } = {}) {
  ...
  for (let i = 0; i < attempts; i += 1) {
    ...
    if (breakerAge != null) {
      ...
      // A breaker lock older than the stale window is a dead breaker's: remove
      // it so the next round is not held up by it forever.
      if (breakerAge > staleMs) {
        try { fs.unlinkSync(breaker); } catch { /* another waiter removed it */ }
      }
      sleepSync(waitMs);
      continue;
    }
...
function claimGateRecord(home, runId, { workerId = null, ownerLive = () => false, now = () => new Date().toISOString(), lock = withRecordLock } = {}) {
  ...
  const held = lock(file, () => {
    ...
    if (record.gate_state && gatesKernel.SETTLED_GATE_STATES.has(record.gate_state)) {
      return { ok: false, refused: `already settled as '${record.gate_state}'${record.gate_worker ? ` by ${record.gate_worker}` : ""}`, record };
    }
    if (record.gate_state === "running" && record.gate_settle_id && record.gate_worker && record.gate_worker !== workerId && ownerLive(record.gate_worker)) {
      return { ok: false, refused: `being gated right now by worker ${record.gate_worker}`, record };
    }
    const merged = { ...record, gate_state: "running", gate_settle_id: token, gate_worker: workerId, gate_at: now() };
