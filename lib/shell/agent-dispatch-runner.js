...
const TERMINAL_SIGNATURES = Object.freeze([
  ...
  // reached your usage limit" (Codex), "quota exceeded" (generic). The
  // possessive forms are anchored on `your`/`the`, so a log line that merely
  // mentions a usage limit in prose does not read as this run hitting one —
  // and this row, not a CTA in the credit row above, is what carries Codex's
  // wording: its message LEADS with "You've hit your usage limit", and the
  // classification the pools are keyed on is identical either way
  // (environment → infrastructure → the shared retry pool).
  { signal: "usage-limit", class: "environment", re: /usage limit reached|(?:hit|reached|exceeded) (?:your|the) (?:\w+ ){0,2}usage limit|quota (?:has been )?exceeded/i },
...
//
// `expectProgress` is the COMPARE-AND-SET door for the one gate_ field that is
// not a value but an ACCUMULATOR (review finding F4). `gate_progress` holds the
// finding ledger, the fix counts, the rescue entries and the shared retry pool
// on ONE object, and its writers build the next version by reading the current
// one and rewriting the whole thing. That read happens in the CALLER, outside
// this function's own read — so a second writer landing in between (the pools
// charge racing the fix-cycle save; a resumed pipeline's first write racing the
// dying worker's last) is silently overwritten, and a lost pool charge is an
// UNRECORDED retry, which is exactly the unbounded case the pool exists to
// stop. Pass `{key, seq}` — the identity and sequence the caller merged from —
// and the write happens only if the record STILL carries that version under
// this function's own read; otherwise nothing is written, `status.conflict` is
// set, and the caller rebuilds from the winner's state. `null` seq means "I
// merged from no progress for this key", which is the first write of an
// attempt (and the correct expectation when the record carries a FOREIGN key's
// progress, since that is what the savers drop). Undefined disables the check,
// so every other caller is byte-identical.
//
// The verify-and-reapply pass extends to it for the same reason it exists for
// `gate_state`: an unlocked whole-record writer can revert a landed stamp. A
// reverted progress write is re-attempted from a fresh read, and the CAS runs
// again on that read, so a retry yields to a writer that legitimately moved the
// accumulator on rather than fighting for the last word.
//
// `status` is an out-param (the `u.claimSpoolResult` idiom): a null return
// already means "could not write", and a CAS miss is a different fact —
// retryable, and nobody's mistake — that the caller must not report as a
// failed journal write.
function stampGateState(home, runId, patch, { verifyAttempts = 3, readBack = readJson, force = false, allowSettledPatch = false, expectProgress = undefined, status = null } = {}) {
  if (!runId || !patch) return null;
  const gateOnly = {};
  for (const [k, v] of Object.entries(patch)) if (k.startsWith("gate_")) gateOnly[k] = v;
  if (!Object.keys(gateOnly).length) return null;
  const casOn = expectProgress !== undefined && gateOnly.gate_progress !== undefined;
  try {
    const file = runPaths(home, runId).record;
    for (let attempt = 0; ; attempt += 1) {
      ...
      if (casOn && !progressVersionMatches(record, expectProgress)) {
        if (status) status.conflict = true;
        return null;
      }
      const merged = { ...record, ...gateOnly };
      atomicJson(file, merged);
      // Two writes are worth verifying: `gate_state`, the one field a later
      // reader treats as a verdict, and a compare-and-set `gate_progress`,
      // whose whole point is that the version this caller merged from is the
      // one the next reader sees.
      const verifiable = !!gateOnly.gate_state || casOn;
      if (!verifiable || attempt >= verifyAttempts) return merged;
      const after = readBack(file);
      if (!after) return merged;
      if (gateOnly.gate_state && after.gate_state !== gateOnly.gate_state) continue;
      if (casOn && !progressVersionMatches(after, { key: gateOnly.gate_progress.key, seq: gateOnly.gate_progress.seq })) continue;
      return merged;
    }
  } catch {
    return null;
...
// Does `record`'s `gate_progress` still hold the exact version a caller merged
// from — same attempt key, same sequence? Progress for ANOTHER attempt's key
// reads as absent, because that is what the savers do with it: a `--regate`
// mints a new key and starts its ledger, pool and rescue state fresh, so
// finding a foreign key is not a conflict, it is the empty state (review
// finding F4, row (d) — the check reconciles against settled state rather than
// acting on a stale reading of it).
function progressVersionMatches(record, expected) {
  const want = expected && Number.isInteger(expected.seq) ? expected.seq : null;
  const cur = record && record.gate_progress && typeof record.gate_progress === "object" ? record.gate_progress : null;
  const mine = cur && expected && cur.key === expected.key ? cur : null;
  const have = mine && Number.isInteger(mine.seq) ? mine.seq : null;
  return have === want;
}
...
function stampImplState(home, runId, patch, { verifyAttempts = 3, readBack = readJson, force = false } = {}) {
  if (!runId || !patch) return null;
  const implOnly = {};
...
function stampCompletionState(home, runId, patch, { verifyAttempts = 3, readBack = readJson } = {}) {
  if (!runId || !patch) return null;
  const only = {};
...
module.exports = {
  ...
  terminalOutcomeBackfill, mergeTerminalOutcome, stampGateState, progressVersionMatches, stampImplState, stampCompletionState, isCompletionField,
