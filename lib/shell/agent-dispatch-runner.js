...
async function runJob(jobFile) {
  ...
  return new Promise((resolve) => {
    ...
    const finalize = async (code, signal) => {
      ...
      // A declared failure with no recognized environment signal in it (the
      // classifier above already read the log tail, which holds the error
      // event) is the run's own failure: the harness's wording is the reason.
