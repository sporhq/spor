...
// the job file under `home` when the caller names one. Fail-soft: no log,
// no adapter, no hook, an unreadable file → []; a non-JSON line (stderr
...
function runReportTexts(record, { home = null } = {}) {
  ...
  if (!adapter || typeof adapter.reportFromEvent !== "function") return [];
  let raw;
  try {
  ...
  for (const line of raw.split("\n")) {
    ...
    let text = null;
    try {
      text = adapter.reportFromEvent(JSON.parse(line));
    } catch {
      continue;
    }
