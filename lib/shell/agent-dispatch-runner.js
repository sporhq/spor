...
const TERMINAL_SIGNATURES = Object.freeze([
  ...
  // Codex spells the same exhaustion "You've hit your usage limit. Visit …
  // to purchase more credits or try again at <date>" (its `turn.failed`
  // error, verbatim from run 93f9ca4e); the OpenAI API's own error code for it
  // is `insufficient_quota`. Neither is "usage limit reached", so a Codex
  // review that died on credits was stamped a plain nonzero exit and the
  // review gate read the missing report as a rejection — the exact
  // misreading issue-spor-review-gate-reviewer-outage-read-as-rejection was
  // closed for, recurring one wording at a time.
  { signal: "usage-limit", class: "environment", re: /usage limit reached|hit your usage limit|quota (?:has been )?exceeded|insufficient_quota/i },
...
// (task-spor-dispatch-native-bg-terminal-detection)
//
// A finished `claude --bg` agent does NOT leave the daemon. A turn-complete
// agent routinely sits at `status: "idle"` with `state` still reading
// `"working"` — so `isRunLive` keeps its record open and a worker following it
// waits out the 24h watchdog on work that finished hours ago (the 2026-09-02
// evidence on this task: two factory pipelines stalled five hours over exactly
// this, and a `claude stop` on each session released them).
//
// Idle alone is NOT that signal — an agent waiting on a tool call reads idle
// too — so absent a `state: "done"` fast path (below) this asks for three
// independent things at once, and every one of them has to say finished:
//
//   1. every listed agent that identifies as this run reads `status: "idle"`.
//      `state` is deliberately not consulted here for anything OTHER than
//      "done": it is the field observed to lie in the other direction
//      (staying `"working"` long after the agent is actually idle), so a bare
//      `state !== "done"` proves nothing about liveness. An agent with no
//      `status` at all (an older CLI, another harness's listing shape) never
//      satisfies this — the fail-safe direction, since an unreadable listing
//      must leave the run live.
...
//
// `state: "done"` on a matched agent is a SEPARATE, independent terminal
// signal (WORKERS.md, dec-spor-native-bg-turn-complete-and-contract): the
// harness itself is declaring the run over, not this store inferring it from
// idle status, so it short-circuits straight to finished without also
// demanding the idle/transcript/quiet triple above agree — that triple exists
// to catch exactly the case `state` itself misses (stuck at `"working"`), and
// a positive `"done"` reading needs no corroboration
// (issue-spor-native-run-done-state-unreachable-and-contract-pending-invisible:
// an earlier revision had `nativeAgentEvidence` drop such an agent from the
// listing entirely, which hid it from `matchingAgents` so it could never reach
// this function to be reaped at all).
//
...
function nativeTurnComplete(record, agents, { graceMs = 60000, quietMs = NATIVE_QUIET_MS, env = process.env, stat = fs.statSync, now = () => new Date().toISOString() } = {}) {
  ...
  const doneAgent = matched.find((a) => String((a && a.state) || "").toLowerCase() === "done");
  if (doneAgent) return { agent: doneAgent, quietAt: observedActivityAt(record, stat, env), transcript: findTranscript(record, env) };
...
function stampRun(home, runId, patch, { lock = withRecordLock } = {}) {
...
function beginNativeRun(home, { harness, name, nodeId, cwd, model, runId, readOnly = false, releaseNode = null, project = null, localNodesDir = null, server = null, org = null, resolvedProfile = null, now = () => new Date().toISOString() }) {
  ...
  const record = {
    ...
    // provenance-profile-blind-to-self-routed-items) — cmdDispatch's own
    // resolveDispatchProfile verdict, which already folds an explicit
    // `--profile`, the item's own `profile:` frontmatter, and an
    // `assigned -> agent {profile:}` edge into one id. Stamped here (not just
    // on the work-loop slot, which carries no profile and must not gain one:
    // its shape is what journal/work/*.work.json publishes) so a later pin
    // can read what actually routed this run, self-routed or not.
...
function writeRecordCarryingGate(file, next, { lock = withRecordLock } = {}) {
  const held = lock(file, () => atomicJson(file, carryGateFields(file, next)));
...
// A settler's ownership token. `own` names the settled stamp the caller wrote:
// the record's `gate_settle_id` when the settler minted one (a random nonce —
// two settlers in the same millisecond cannot share it, unlike `gate_at`), or,
// for a record settled without one, its `gate_at`.
function ownsRecord(record, own) {
  if (own == null) return false;
  if (record.gate_settle_id != null) return record.gate_settle_id === own;
  return record.gate_at === own;
}

// OWN a run record's gate pipeline BEFORE running it (cross-model review,
// blocking finding 2). The settle stamp is a compare-and-swap, but a pipeline
// mutates the graph — facts, an escalation, a demotion — on its way to it, so
// two pipelines for one run (a duplicate orphan adopter, a resumed pipeline
// beside a live one) could both run and the loser leave graph state the
// winner's verdict contradicts. So the ownership nonce (`gate_settle_id`) is
// minted HERE, under the record lock, before the first gate runs: a record
// already settled, or held by another worker that is still alive, refuses the
// claim and the caller runs nothing. A dead owner's claim is taken over (that
// is what orphan resumption is). The settle then goes through the `own` door
// with this token, so it lands only while the record still carries it.
// Returns {ok, token, refused, record}: `refused` is the human reason when the
// record belongs to someone else; `ok:false` with `refused:null` means there is
// no record to own (the caller proceeds unowned, as before).
function claimGateRecord(home, runId, { workerId = null, ownerLive = () => false, now = () => new Date().toISOString(), lock = withRecordLock, reopen = null } = {}) {
  const token = crypto.randomBytes(12).toString("hex");
  if (!runId) return { ok: false, token, refused: null, record: null, reason: "no run id" };
  let file = null;
  try {
    file = runPaths(home, runId).record;
  } catch (e) {
    return { ok: false, token, refused: null, record: null, reason: (e && e.message) || String(e) };
  }
  if (!fs.existsSync(file)) return { ok: false, token, refused: null, record: null, reason: "no run record" };
  const held = lock(file, () => {
    const record = readJson(file);
    if (!record) return { ok: false, refused: null, record: null, reason: "unreadable record" };
    if (reopen && Object.values(record.gate_progress && record.gate_progress.gates || {}).some((p) => p && (p.filingIntent || (p.evidence && p.evidence.complete !== true)))) return { ok: false, refused: "flake occurrence publication is still owed for the prior judgement; resume its original attempt and graph before re-gating", record };
    if (reopen && record.gate_attestation_pending) return { ok: false, refused: "attestation publication is still owed for the prior judgement; replay its evidence through the original graph before re-gating", record };
    if (reopen && ((record.gate_settle_id || null) !== reopen.settleId || (Number(record.gate_regate_count) || 0) !== reopen.regateCount || record.gate_state !== reopen.state)) return { ok: false, refused: "the prior judgement changed before re-gate could claim it", record };
    if (record.gate_state && gatesKernel.SETTLED_GATE_STATES.has(record.gate_state) && !(reopen && ["failed", "blocked", "mismatch"].includes(record.gate_state))) {
      return { ok: false, refused: `already settled as '${record.gate_state}'${record.gate_worker ? ` by ${record.gate_worker}` : ""}`, record };
    }
    if ((record.gate_state === "running" || reopen) && record.gate_settle_id && record.gate_worker && record.gate_worker !== workerId && ownerLive(record.gate_worker)) {
      return { ok: false, refused: `being gated right now by worker ${record.gate_worker}`, record };
    }
    const merged = { ...record, gate_state: "running", gate_settle_id: token, gate_worker: workerId, gate_at: now(), ...(reopen ? { gate_regate_count: reopen.regateCount + 1, gate_regated_at: now() } : {}) };
    atomicJson(file, merged);
    const after = readJson(file);
    if (!after || after.gate_settle_id !== token) return { ok: false, refused: "the ownership stamp could not be verified on disk", record: after || merged };
    return { ok: true, refused: null, record: after };
  });
  if (!held.ok) return { ok: false, token, refused: `the run record is locked (${held.reason})`, record: null, reason: held.reason };
  return { ...held.value, token };
}

function stampGateState(home, runId, patch, { verifyAttempts = 3, readBack = readJson, force = false, allowSettledPatch = false, own = null, lock = withRecordLock } = {}) {
  if (!runId || !patch) return null;
  ...
  try {
    ...
    const held = lock(file, () => {
      ...
      for (let attempt = 0; ; attempt += 1) {
        ...
        const proposed = build(typeof patch === "function" ? structuredClone(record) : record);
        ...
        if (own != null) {
          if (!ownsRecord(record, own)) return record;
        ...
        if (own == null && !force && record.gate_settle_id && record.gate_state === "running" && (gateOnly.gate_settle_id != null || (gateOnly.gate_worker !== undefined && gateOnly.gate_worker !== record.gate_worker))) return record;
...
// The sole merging writer for an attempt's gate ledger, rescue and pools.
// `mutate` returns a partial progress patch, or null to refuse (e.g. a future
// capacity reservation). It runs synchronously against fresh owned state under
// stampGateState's lock; no dispatch or other effects may run in the callback.
// A gate row is intentionally replaced as a unit: clearing a settled debt must
// remain possible. Other rows and unknown top-level progress keys are retained.
function updateGateProgress(home, runId, mutate, { key, attempt = 1, own = null, sidePatch = {}, now = () => new Date().toISOString(), lock = withRecordLock } = {}) {
  let stamp = null;
  let applied = false;
  let reason = "the run record could not be updated";
  const record = stampGateState(home, runId, (fresh) => {
    if (gatesKernel.SETTLED_GATE_STATES.has(fresh.gate_state)) { reason = "the gate pipeline is already settled"; return null; }
    if ((own == null ? fresh.gate_settle_id != null || fresh.gate_at != null : !ownsRecord(fresh, own))) { reason = "the gate pipeline owner changed"; return null; }
    if ((Number(fresh.gate_regate_count) || 0) + 1 !== Math.max(1, Number(attempt) || 1)) { reason = "the gate pipeline attempt changed"; return null; }
    const patch = {};
    for (const [name, value] of Object.entries(sidePatch)) {
      if (name.startsWith("gate_fix_") || name.startsWith("gate_rescue_")) patch[name] = value;
    }
    if (mutate != null) {
      const prior = fresh.gate_progress && fresh.gate_progress.key === key ? fresh.gate_progress : { gates: {} };
      const delta = typeof mutate === "function" ? mutate(prior, fresh) : mutate;
      if (!delta || typeof delta !== "object" || Array.isArray(delta) || typeof delta.then === "function") { reason = "the progress update was refused"; return null; }
      stamp = { ...prior, ...delta, key, at: now(), seq: (Number.isInteger(prior.seq) ? prior.seq : 0) + 1,
        gates: { ...(prior.gates || {}), ...(delta.gates || {}) } };
      patch.gate_progress = stamp;
    }
    applied = Object.keys(patch).length > 0;
    return patch;
  }, { own, lock });
  return record && applied && (!stamp || record.gate_progress === stamp)
    ? { ok: true, record, progress: record.gate_progress || null }
    : { ok: false, record, reason };
}
...
function stampImplState(home, runId, patch, { verifyAttempts = 3, readBack = readJson, force = false, lock = withRecordLock } = {}) {
  if (!runId || !patch) return null;
  const implOnly = {};
...
function stampCompletionState(home, runId, patch, { verifyAttempts = 3, readBack = readJson, lock = withRecordLock } = {}) {
  if (!runId || !patch) return null;
  const only = {};
  ...
  try {
    ...
    for (let attempt = 0; ; attempt += 1) {
      const record = readJson(file);
      if (!record) return null;
      const merged = { ...record, ...only };
      atomicJson(file, merged);
      if (only.completion_debt === undefined || attempt >= verifyAttempts) return merged;
      const after = readBack(file);
      if (!after || after.completion_debt === only.completion_debt) return merged;
    }
    });
    return held.ok ? held.value : null;
  } catch {
...
module.exports = { judgedChildEnv,
  ...
  terminalOutcomeBackfill, mergeTerminalOutcome, stampGateState, updateGateProgress, stampImplState, stampCompletionState, isCompletionField,
  claimGateRecord, withRecordLock, breakStaleLock, recordLockPath, breakerLockPath, RECORD_LOCK_STALE_MS, RECORD_LOCK_ATTEMPTS, RECORD_LOCK_WAIT_MS, writeRecordCarryingGate,
