...
//
// A bare pid is not permanent identity, so an answering one is not believed
// forever: pid spaces recycle (32768 wide in many containers), and a recycled pid
// would otherwise hold a record `running` FOREVER — pruneRuns only ages out
// terminal records, so nothing else would ever close it. The test is SILENCE, not
// age (`lastActivityAt`): a long run that is still writing to its log is alive
// however old it is, and only one that has gone quiet past `staleMs` while its
// pid still answers is read as reuse.
function finalizeSupervisedRun(record, { alive = false, now = () => new Date().toISOString(), graceMs = 60000, staleMs = 86400000 } = {}) {
  if (!record || TERMINAL_STATES.has(record.state)) return null;
  const at = Date.parse(now());
  const created = Date.parse(record.created_at || "") || 0;
  const age = created ? at - created : 0;
  const quiet = lastActivityAt(record);
  const stale = alive && staleMs > 0 && quiet > 0 && at - quiet > staleMs;
  if (alive && !stale) return null;
  ...
  return {
    ...
    termination_signal: known ? known.signal : (stale ? "supervisor-stale" : (launched ? "supervisor-gone" : "supervisor-never-started")),
    termination_reason: known ? known.reason : trimReason(stale
...
function reconcileRuns(home, { agents = [], enumerated = true, env = process.env, now = () => new Date().toISOString(), graceMs = 60000, staleMs = 86400000 } = {}) {
  ...
  for (const record of records) {
    ...
    if (record.launch_mode === "supervised-jsonl") {
      patch = finalizeSupervisedRun(record, { alive: pidAlive(record.runner_pid), now, graceMs, staleMs });
    } else if (record.launch_mode === "native-background" && enumerated) {
      patch = finalizeRun(record, { alive: isRunLive(record, agents), env, now, graceMs });
    }
...
module.exports = {
  ...
  launchFailure, closeRun, listRuns, pruneRuns,
};
