...
function carryGateFields(file, next) {
  try {
    ...
    for (const [k, v] of Object.entries(onDisk)) {
      if (k.startsWith("gate_") || k.startsWith(candidateKernel.IMPL_FIELD_PREFIX) || isCompletionField(k)) gate[k] = v;
    }
...
//
// There is deliberately no `force` door (the gate twin has one, for `spor work
// --regate`): nothing may reopen a settled stage today, because nothing yet
// re-runs one. The step that does — the completion boundary
// (task-spor-factory-controller-completion-boundary) — adds it with its caller,
// rather than this carrying an override no code path can reach.
//
// Fail-soft, like every other write to this journal: a stamp that could not
// land leaves the pin unrecorded, and the tree is judged regardless.
function stampImplState(home, runId, patch, { verifyAttempts = 3, readBack = readJson } = {}) {
  if (!runId || !patch) return null;
  const implOnly = {};
  for (const [k, v] of Object.entries(patch)) if (k.startsWith(candidateKernel.IMPL_FIELD_PREFIX)) implOnly[k] = v;
  ...
  try {
    ...
    for (let attempt = 0; ; attempt += 1) {
      ...
      if (write.impl_state !== undefined && candidateKernel.implSettled(record.impl_state)) delete write.impl_state;
