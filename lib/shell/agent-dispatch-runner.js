...
function claimGateRecord(home, runId, { workerId = null, ownerLive = () => false, now = () => new Date().toISOString(), lock = withRecordLock, reopen = null } = {}) {
  ...
  const held = lock(file, () => {
    ...
    if (reopen && Object.values(record.gate_progress && record.gate_progress.gates || {}).some((p) => p && p.evidence && p.evidence.complete !== true)) return { ok: false, refused: "flake occurrence publication is still owed for the prior judgement; resume its original attempt and graph before re-gating", record };
