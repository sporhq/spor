...
// Only the two documented races (EACCES permission-denied, ENOENT/EPERM from
// a delete racing readdirSync on a live graph home) are isolated. Anything
// else (EIO, EMFILE/ENFILE, ENOMEM, ELOOP, ...) signals a systemic storage/fd
// problem, not a one-off bad file -- swallowing THAT would silently empty
// the graph one file at a time exactly the way dec-spor-buildgraph-per-node-
// fault-isolation already warns against for parse faults, so it rethrows and
// crashes as loudly as it used to.
const EXPECTED_READ_FAULT_CODES = new Set(["ENOENT", "EACCES", "EPERM"]);
...
function readGraphFiles(dir, { onSkip } = {}) {
  ...
  for (const f of fs.readdirSync(dir).filter((f) => f.endsWith(".md"))) {
    try {
      ...
      if (!onSkip || !EXPECTED_READ_FAULT_CODES.has(e && e.code)) throw e;
