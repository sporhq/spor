...
async function runIntegrationStage({ item, factory, deps, log = () => {} }) {
  ...
  try {
    ...
    let races = 0;
    for (;;) {
      const built = await deps.buildCandidate({ top: tree.top, head: tree.head, targetRef: integration.targetRef, strategy: integration.strategy, mode: integration.mode });
      if (!built.ok) {
