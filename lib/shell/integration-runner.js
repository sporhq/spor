...
async function runIntegrationStage({ item, factory, deps, log = () => {} }) {
  ...
  // is the BRANCH, not a tree this stage builds: the PR is opened from
  // `tree.head`, so a head that merely CONTAINS the candidate would put
  // unjudged commits into the merge. There, the head must BE the candidate's
  // tree.
  //
