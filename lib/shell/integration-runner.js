...
function buildIntegrationFact({ integration, nodeId, runId, project, verdict, detail, evidence, attempts, escalatedTo, demotion, date, factory, attempt = 0 }) {
...
async function runIntegrationStage({ item, factory, deps, log = () => {} }) {
  ...
  // The failed/blocked half. ATOMIC with the escalation, in that order, exactly
  // as runGatePipeline's refusal is (task-spor-gate-escalation-demote-atomic,
  // issue-spor-integration-settle-escalate-demote-race): with no escalation on
  // the graph there is no demotion — a rollback alone would leave the item
  // open, agent-ready, unblocked, its resolving edge standing, and this refusal
  // held only in one box's cooldown map. The status is left as the run left
  // it, the fact records the withheld rollback, and `escalation_failed` rides
  // the result so the caller marks the run record (`gate_escalation_failed`)
  // and `spor work --status` names `spor work --regate <run>` — which re-runs
  // the gates AND this stage off the run record — as the door back. The
  // verdict still SETTLES, for the reason §10.7 gives: the resume scan would
  // re-run the whole pipeline every pass with no cooldown behind it.
  const settle = async (state, { detail, evidence }) => {
    let escalatedTo = null;
    try {
      // `kind` is the escalation's own framing, derived from the verdict
      // rather than passed at every call site: a `mismatch` refusal spent no
      // fix cycle and no landing race, so an escalation body reading "its fix
      // cycles are spent (1 attempt, cap 2)" would misdescribe it to whoever
      // triages it. Every other verdict passes null and reads exactly as before.
    ...
    for (;;) {
      ...
      if (!built.ok) {
        ...
        if (gates.cycleDecision(integration, cycle) === "retry") {
          ...
          if (!refreshed.ok) return settle(refreshed.verdict, { detail: refreshed.detail });
          continue;
        }
      ...
      try {
        ...
        if (!suite.ok) {
          ...
          if (gates.cycleDecision(integration, cycle) === "retry") {
            ...
            const refreshed = await refreshTree({ cycle, runId: (fixed && fixed.runId) || null });
            if (!refreshed.ok) return await settle(refreshed.verdict, { detail: refreshed.detail });
            continue;
          }
        ...
        const landing =
          integration.mode === "propose"
            ? await deps.propose({ top: tree.top, dir: built.dir, head: integrateCommit(), sha: landSha, targetRef: integration.targetRef })
