...
async function runIntegrationStage({ item, factory, deps, log = () => {} }) {
  ...
  try {
    ...
    for (;;) {
      ...
      let landed = null;
      try {
        const forced = deps.forceProtected ? await deps.forceProtected({ dir: built.dir }) : { ok: true };
        if (!forced.ok) {
        ...
        const landing = await deps.land({ top: tree.top, dir: built.dir, sha: built.sha, expectedSha: built.expectedSha, targetRef: integration.targetRef, mode: integration.mode });
