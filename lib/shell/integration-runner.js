...
async function runIntegrationStage({ item, factory, deps, log = () => {}, gatedHead = null }) {
  ...
  //
  // And the refreshed head is, by construction, NOT the head the gates judged
  // (review finding 2): the fix cycle committed. Landing it on the strength of
  // the pre-fix verdicts is exactly the hole the equality check above closes
  // for the stage's entry, so the same rule holds here — the moved head is
  // handed back to the gate pipeline (`deps.regate`) and only a PASS at that
  // head advances `gatedHead`; a re-gate that fails, or a wiring with no
  // re-gate door at all, settles the stage failed (fail closed, §10.9). A stage
  // run with no `gatedHead` was never gate-armed (nothing to be equal to) and
    ...
    for (;;) {
      ...
      if (!built.ok) {
        ...
        if (gates.cycleDecision(integration, cycle) === "retry") {
          ...
          if (!refreshed.ok) return settle("failed", { detail: refreshed.detail, escalatedTo: refreshed.escalatedTo || null });
          continue;
        }
      ...
      try {
        ...
        const suite = await deps.runSuite({ dir: built.dir, base: built.expectedSha, head: landSha });
        ...
        if (!suite.ok) {
          ...
          if (gates.cycleDecision(integration, cycle) === "retry") {
            ...
            const refreshed = await refreshTree();
            if (!refreshed.ok) return await settle("failed", { detail: refreshed.detail, escalatedTo: refreshed.escalatedTo || null });
            continue;
          }
        ...
        const landing =
          integration.mode === "propose"
            ? await deps.propose({
                top: tree.top, dir: built.dir, head: tree.head, sha: landSha, targetRef: integration.targetRef,
                ...
                chain: { ...chain, head: tree.head, candidate: { ...chain.candidate } },
              })
