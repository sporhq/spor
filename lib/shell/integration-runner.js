...
// -> {ok, state: "open"|"closed", merged, mergedBy, mergeCommitSha, reason}.
//
...
async function checkProposal(proposal, { deps, log = () => {} }) {
  ...
  try {
    const wrote = await deps.recordFact({ id: fact.id, markdown: fact.markdown });
    if (wrote && wrote.ok) factId = fact.id;
    else log(`work: the ${verdict} outcome for ${nodeId}'s proposal could not be recorded on the graph (${(wrote && wrote.reason) || "no response"}) — the verdict still stands`);
  } catch (e) {
    log(`work: the ${verdict} outcome for ${nodeId}'s proposal could not be recorded on the graph (${(e && e.message) || e}) — the verdict still stands`);
  }
