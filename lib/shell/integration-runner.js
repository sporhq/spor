...
async function runIntegrationStage({ item, factory, deps, log = () => {}, gatedHead = null }) {
  ...
  const chain = { head: null, gatedHead: gatedHead || null, targetSha: null, landedSha: null, definition: factory.definition || null };
        ...
        const landing =
          integration.mode === "propose"
            ? await deps.propose({
                ...
                chain: { ...chain, head: tree.head, candidate: { base: built.expectedSha || null, sha: landSha, suite: "passed", command: integration.command } },
              })
