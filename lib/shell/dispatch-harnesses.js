...
function copilotArgs({ model, readOnly = null }) {
  ...
  // never prompts — so the file-writing tool is denied for the run. The shell
  // tool stays (the reviewer must run tests to demonstrate a finding); what a
  // shell command writes is bounded by the prompt, not the harness, which is
  // the honest limit of this posture and why the adapter names it.
...
const ADAPTERS = Object.freeze({
  ...
  copilot: Object.freeze({
    ...
    // The read-only posture: deny the file-writing tool at the permission
    // layer (`--deny-tool write`, which overrides `--allow-all`). See
    // copilotArgs for the limit this posture honestly has.
    readOnly: Object.freeze({ denyTools: Object.freeze(["write"]) }),
    validateOptions(options) {
