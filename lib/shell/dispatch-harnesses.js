...
function normalizeHarnessDeclaration(id, raw) {
  ...
  for (const a of raw.args || []) {
    ...
    if (a === "{model}") {
      return fail(`${declarationKey(id, "args")} entry "{model}" must be inlined into the flag that carries it (write "--model={model}") — a standalone token disappears when no model resolves, leaving the preceding flag to swallow the next argument`);
    }
...
function declaredAdapter(declaration) {
  ...
  if (!d.report || typeof d.report !== "object") return null;
