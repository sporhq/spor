...
// Codex's own declaration that the turn FAILED: a `turn.failed` event on the
// `codex exec --json` stream, whose `error.message` carries the provider's
// wording (verbatim from run 93f9ca4e: `{"type":"turn.failed","error":
// {"message":"You've hit your usage limit. …"}}`). The Claude Code analogue is
// the `is_error` result above, and the supervisor treats both the same way:
// the run is `failed` whatever the exit code says, the message is retained as
// the termination reason (and classified for an environment signal — a
// credit-dead reviewer is an outage to re-dispatch with headroom, never a
// `changes_requested`), and NO report is read — Codex writes its report
// through `--output-last-message`, and a failed turn's file, when one exists
// at all, is not a verdict. A preceding `{"type":"error"}` event is NOT read
// as the declaration: Codex may emit one for a retried request and go on to
// complete the turn, so only the terminal `turn.failed` says the turn ended.
function codexFailureFromEvent(event) {
  if (!event || typeof event !== "object" || event.type !== "turn.failed") return null;
  const err = event.error && typeof event.error === "object" ? event.error : null;
  let text = err && typeof err.message === "string" ? err.message.trim() : "";
  if (!text && typeof event.message === "string") text = event.message.trim();
  // Structured provider codes carry the same canonical terminal signature
  // even when the accompanying prose is generic or absent.
  const code = err && (typeof err.code === "string" ? err.code.trim() : typeof err.type === "string" ? err.type.trim() : "");
  const detail = [text, code].filter(Boolean).join("; ");
  return { reason: detail ? `turn.failed: ${detail}` : "turn.failed" };
}
...
const ADAPTERS = Object.freeze({
  ...
  codex: Object.freeze({
    ...
    // The harness's own declaration that the turn failed (`turn.failed`),
    // read exactly as claude-code's `is_error` result is.
...
const DECLARATION_KEYS = ["command", "args", "label", "report", "session", "posture"];
...
function normalizeHarnessDeclaration(id, raw) {
  ...
  if (raw.posture !== undefined && !POSTURE_MEANINGS.includes(raw.posture)) {
    return fail(`${declarationKey(id, "posture")} must be unattended, attended, or read-only`);
  }
  ...
  return {
    ...
    declaration: {
      ...
      session,
      ...(raw.posture === undefined ? {} : { posture: raw.posture }),
    },
  };
}
...
function declaredAdapter(declaration) {
  ...
  if (d.posture !== undefined && !POSTURE_MEANINGS.includes(d.posture)) return null;
  ...
  return Object.freeze({
    ...
    declared: true,
    declaration: d,
    // A declaration describes the fixed machine-bound argv; it never grants
    // permissions or invents flags. Expose ONLY its actual posture so callers
    // cannot translate a restrictive invocation into a more permissive run.
    // Omission preserves the legacy operator-bound warning in preflight.
    ...(d.posture === undefined ? {} : {
      [d.posture === "read-only" ? "readOnly" : d.posture]: Object.freeze({}),
      postureMeaning: () => d.posture,
    }),
    launchMode: "supervised-jsonl",
    identityMode: "env-token",
