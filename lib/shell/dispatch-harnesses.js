...
// carries no text and is skipped. An ERROR result (`is_error: true`) is NOT a
// report: its text is the harness's account of why the run stopped, not the
// agent's final message, and the terminal-state contract reads ANY report text
// as a clean `reported` outcome (report presence, not exit status, is its
// discriminator — WORKERS.md §6), so an errored session written to the report
// file would enter the gate pipeline as if it had finished
// (issue-spor-claude-supervised-error-result-read-as-report). It is handed to
// the supervisor through `claudeFailureFromEvent` below instead.
...
// The harness's own declaration that the run FAILED: a `result` event carrying
// `is_error: true` (`subtype` names the kind — `error_during_execution`,
// `error_max_turns`, … — and `result`, or on some versions an `errors` list,
// carries the text). The supervisor classifies such a run `failed` whatever
// the exit code says and writes NO report, retaining the error text as the
// termination reason; a normal result returns null and changes nothing.
function claudeFailureFromEvent(event) {
  if (!event || typeof event !== "object" || event.type !== "result" || event.is_error !== true) return null;
  let text = typeof event.result === "string" ? event.result.trim() : "";
  if (!text && Array.isArray(event.errors)) {
    text = event.errors.filter((e) => typeof e === "string" && e.trim()).map((e) => e.trim()).join("; ");
  }
  const subtype = typeof event.subtype === "string" && event.subtype ? event.subtype : "error";
  return { reason: text ? `${subtype}: ${text}` : subtype };
}
...
// is there to answer.
function opencodeArgs({ model, readOnly = null }) {
  ...
  // The read-only posture (`spor dispatch --read-only`): OpenCode's built-in
  // `plan` agent denies the `edit` permission for every path (verified against
  // opencode 1.18.0, `opencode agent list`), and `--auto` only auto-approves
  // permissions that are NOT explicitly denied — so the harness refuses every
  // file edit. Its `bash` permission, though, is `allow *` — plan mode alone
  // leaves the run able to write the implementer's live checkout through any
  // shell command (review finding 1 on the third cut of the review gate), so
  // the posture is completed in `opencodePrepareRun`, which denies `bash` for
  // the plan agent through OpenCode's config override. See there for the cost.
  if (readOnly && readOnly.agent) args.push("--agent", readOnly.agent);
  if (model) args.push("--model", model);
  return args;
}
...
function copilotArgs({ model, readOnly = null }) {
  const args = ["--output-format", "json", "--allow-all", "--no-ask-user", "--no-color", "--no-auto-update"];
  // The read-only posture: Copilot has no sandbox, but its permission layer
  // denies named tools outright — `--deny-tool` wins over `--allow-all` and
  // never prompts — so BOTH the file-writing tool and the shell tool are
  // denied for the run. Denying `write` alone left `shell` open, and a shell
  // command writes to the implementer's live checkout as freely as the write
  // tool does (review finding 4 on the second cut of the review gate): a
  // posture bounded by the prompt rather than the harness is not the
  // enforced read-only `--read-only` promises. The cost is honest and named:
  // a Copilot-routed reviewer cannot run commands, so it cannot DEMONSTRATE a
  // blocking finding — its verdicts are advisory. A review gate that needs
  // blocking power routes to a harness with a sandbox (Codex) or a
  // read-only permission mode that still runs commands (Claude, OpenCode).
  if (readOnly && Array.isArray(readOnly.denyTools)) for (const tool of readOnly.denyTools) args.push("--deny-tool", tool);
  if (model) args.push("--model", model);
  return args;
}
...
// The rest of OpenCode's read-only posture. `--agent plan` denies `edit`, but
// the plan agent's `bash` permission is `allow *` (opencode 1.18.0), and a
// shell command writes the reviewer's checkout as freely as the edit tool —
// `sed -i`, a redirect, `git checkout`. So under `--read-only` the run also
// gets `OPENCODE_CONFIG_CONTENT`, OpenCode's env-carried config layer (merged
// over the user's and the project's config, verified against 1.18.0: the plan
// agent then lists `bash: deny *` beside its `edit` denials), denying `bash`
// for the plan agent — and restating `edit: deny` so the posture does not
// rest on the built-in table staying what it is today. The cost is the same
// one Copilot's `--deny-tool shell` pays, and it is named rather than hidden:
// an OpenCode-routed reviewer cannot run commands, so it cannot DEMONSTRATE a
// blocking finding and its verdicts are advisory. A review gate that needs
// blocking power routes to a harness whose read-only posture still runs
// commands inside a sandbox (Codex) or a plan permission mode (Claude Code).
// An operator's own OPENCODE_CONFIG_CONTENT is kept and the denial merged on
// top of it (an unparseable one is replaced — the promise wins).
const OPENCODE_READ_ONLY_PERMISSION = Object.freeze({ edit: "deny", bash: "deny" });

function opencodeReadOnlyConfigContent(existing, agent = "plan") {
  let base = {};
  ...
  const agents = base.agent && typeof base.agent === "object" && !Array.isArray(base.agent) ? base.agent : {};
  const current = agents[agent] && typeof agents[agent] === "object" && !Array.isArray(agents[agent]) ? agents[agent] : {};
  ...
  return JSON.stringify({
    ...base,
    agent: { ...agents, [agent]: { ...current, permission: { ...permission, ...OPENCODE_READ_ONLY_PERMISSION } } },
  });
}

function opencodePrepareRun({ cwd, env = null, readOnly = null } = {}) {
  const out = {};
  if (cwd) out.PWD = cwd;
  if (readOnly && readOnly.agent) out.OPENCODE_CONFIG_CONTENT = opencodeReadOnlyConfigContent(env && env.OPENCODE_CONFIG_CONTENT, readOnly.agent);
  return Object.keys(out).length ? { env: out } : null;
}
...
// A Codex assistant MESSAGE, read off `codex exec --json`'s stream: an
// `item.completed` whose item is an `agent_message` carries the text. This is
// `messageFromEvent`, NOT `reportFromEvent`: Codex writes its own report
// through `--output-last-message`, and the supervisor deliberately consults
// only `reportFromEvent` when deciding what a run's report IS (byte-identical,
// see runJob) — a report hook here would have the supervisor overwrite the
// harness's own file. What this hook serves is the READERS of a finished
// run's stream: `runReportTexts` (agent-dispatch-runner.js) replays every
// assistant message so a rescue's early diagnosis block — emitted before a
// long verification and overwritten by the last message, exactly as
// `--output-last-message` semantics dictate — is still recoverable
// (issue-spor-rescue-and-fix-sessions-end-turn-waiting-on-background-job, F2:
// a Codex rescue profile lost the early block because this stream had no
// text hook at all). Verified against codex exec --json's documented shape
// (`{"type":"item.completed","item":{"type":"agent_message","text":…}}`); a
// reasoning item, a command execution, a file change carry no message text.
function codexMessageFromEvent(event) {
...
// Codex's own declaration that the run FAILED — the peer of
// `claudeFailureFromEvent`, which Codex went without
// (issue-spor-codex-usage-limit-outage-read-as-a-code-failure). Without it the
// supervisor had no `streamFailure` for a Codex run, so a provider outage was
// classified only by grepping the whole log TAIL, and an unrecognized wording
// fell through to a bare `nonzero-exit` — the review gate then read a
// credit-dead reviewer as a run that simply wrote no report, and told the
// implementer to route to a supervised harness for a run that already was one.
//
// The declaration read is `{"type":"turn.failed","error":{"message":…}}` and
// ONLY that. `codex exec --json` also emits a bare `{"type":"error","message":…}`
// (it precedes the `turn.failed` in the observed stream, carrying the same
// text), but the supervisor's loop has no way to UNSET a declaration once made
// — a later event returning null leaves the last one standing — so declaring
// on a bare `error` would file a run that hit a recoverable error and then
// finished as failed, and suppress the report of a review that actually
// passed. `turn.failed` is terminal for the turn, and `codex exec` runs one
// turn, so it cannot be contradicted by anything after it. A stream that
// somehow emits only the bare `error` is still covered, one layer down: the
// nonzero exit is classified off the log tail, where the widened usage-limit
// signature now matches Codex's wording.
//
// The declaration matters beyond its own wording: the supervisor runs
// `classifyTerminalText` over the DECLARED text first and only then over the
// log tail, so the environment signature is matched against the provider's
// error message rather than against whatever prose the session happened to
// print — and an unrecognized wording still lands as `error-result` carrying
// the provider's words, never as a nonzero exit with no reason. It also
// suppresses the report: `codex exec --output-last-message` writes its file
// whatever happened, and the terminal-state contract reads any report text as
// a clean `reported` outcome, so a turn that FAILED must not hand one back
// (the rule issue-spor-claude-supervised-error-result-read-as-report
// established for Claude, unenforced here until now).
function codexFailureFromEvent(event) {
  if (!event || typeof event !== "object" || event.type !== "turn.failed") return null;
  const err = event.error;
  const text = err && typeof err.message === "string" ? err.message.trim() : "";
  return { reason: text || "turn.failed" };
}

// The harness-specific dispatch flags, each with the option key every
// adapter's `validateOptions` reads it under, the harness whose vocabulary it
// is, and its ROLE: `posture` says HOW the box runs (unattended, read-only,
// attended), `routing` says WHO runs (a harness agent definition). This is the
// ONE membership list (issue-spor-rescue-posture-foreign-restrictive-flag-
// becomes-bypass): `rejectForeignOptions` below reads it, and the callers that
// have to split a worker's passthrough — the review's harness-neutral subset,
// the rescue's posture-vs-routing halves, the work loop's passthrough keys —
// derive theirs from it, so a new harness flag cannot silently miss one.
const HARNESS_OPTION_FLAGS = Object.freeze({
  "permission-mode": Object.freeze({ option: "permissionMode", owner: "Claude Code", role: "posture" }),
  agent: Object.freeze({ option: "agent", owner: "Claude Code", role: "routing" }),
  sandbox: Object.freeze({ option: "sandbox", owner: "Codex", role: "posture" }),
  "approval-policy": Object.freeze({ option: "approvalPolicy", owner: "Codex", role: "posture" }),
});
// The flags of one role, as `{ flag: optionKey }`.
function harnessOptionFlags(role) {
  const out = {};
  for (const [flag, spec] of Object.entries(HARNESS_OPTION_FLAGS)) if (!role || spec.role === role) out[flag] = spec.option;
  return Object.freeze(out);
}

// The flags that belong to OTHER harnesses. Shared by the adapters below that
// own none of them, because they reject the same set for the same reason:
// silently ignoring a flag the operator passed is worse than refusing it.
function rejectForeignOptions(label, options, hint) {
  const foreign = Object.entries(HARNESS_OPTION_FLAGS).find(([, spec]) => options && options[spec.option]);
  if (!foreign) return null;
  const [flag, spec] = foreign;
  return {
    message: `cannot use --${flag} with a ${label} dispatch — that flag is ${spec.owner}-specific.`,
    hint,
  };
}
...
//
// A posture is spelled in one harness's flags, but what it SAYS is harness-
// neutral: the box runs read-only, or attended (it stops to ask), or
// unattended (it never asks). A caller that has to re-express a posture in
// another harness's vocabulary — the rescue lane, whose profile routinely runs
// a different harness from the worker — reads the meaning here, from the
// adapter that OWNS the flags, and writes it back in the target adapter's own
// declarations (`readOnly` / `attended` / `unattended`).
// Ordered most restrictive first: where one posture carries flags of more
// than one meaning, the most restrictive reading wins, because widening what
// an unattended agent may do is the one mistake a translation must not make.
//
// A reading of ATTENDED is a real one, not a refusal: the rescue lane (the
// only caller that translates by meaning today) passes it straight through
// to `spor dispatch`, and cmdDispatch's worker preflight (checkWritePosture,
// lib/shell/preflight.js) lets it proceed rather than hard-refusing, because
// the runner has already made the deliberate, never-widened choice this
// posture represents — see `allowAttended` there and WORKERS.md §10.10
// (issue-spor-rescue-posture-attended-translation-hard-refuses). Nothing else
// gets that acknowledgement: an ordinary unattended dispatch with no posture
// at all, or one whose own harness genuinely resolves ATTENDED without going
// through this translation, is still refused before it claims anything.
const POSTURE_MEANINGS = Object.freeze(["read-only", "attended", "unattended"]);

// Claude Code: `plan` reads and runs but cannot edit; `bypassPermissions`
// never asks; every other mode (default/acceptEdits/…) still stops on some
// prompt, so it is attended.
function claudePostureMeaning({ permissionMode }) {
  if (!permissionMode) return null;
  if (permissionMode === "plan") return "read-only";
  if (permissionMode === "bypassPermissions") return "unattended";
  return "attended";
}

// Codex: `--sandbox read-only` is read-only whatever the approval policy; an
// approval policy other than `never` gates on prompts nobody answers, so it is
// attended; anything else (`workspace-write`, `danger-full-access`, a bare
// `--approval-policy never`) is the unattended posture Codex runs by default.
function codexPostureMeaning({ sandbox, approvalPolicy }) {
  if (!sandbox && !approvalPolicy) return null;
  if (sandbox === "read-only") return "read-only";
  if (approvalPolicy && approvalPolicy !== "never") return "attended";
  return "unattended";
}

// The meaning of a whole posture, read by the adapters that own its flags:
// the most restrictive of their readings, or null when no adapter owns any of
// the options given (an empty posture).
function postureMeaning(options) {
  let best = null;
  for (const adapter of Object.values(ADAPTERS)) {
    if (typeof adapter.postureMeaning !== "function") continue;
    const m = adapter.postureMeaning(options || {});
    if (!m) continue;
    if (best === null || POSTURE_MEANINGS.indexOf(m) < POSTURE_MEANINGS.indexOf(best)) best = m;
  }
  return best;
}
...
// The UNATTENDED posture, declared per adapter beside `readOnly` and read the
// same way (issue-spor-rescue-dispatch-drops-harness-flags): what a harness
// needs on its argv to run with nobody there to answer a prompt. Codex,
// OpenCode and Copilot need NOTHING — `--ask-for-approval never`, `--auto` and
// `--allow-all --no-ask-user` are already in their argv builders, which is why
// dispatch is unattended by definition on them — so their declaration is the
// empty posture (declared and empty, distinct from an adapter that declares
// none at all). Claude Code is the exception: without a permission mode from
// the caller it stalls on its first write. Callers that must re-express a
// posture in another harness's vocabulary (the rescue lane, which routes an
// implementer to a profile that may not share the worker's harness) read this
// rather than carrying a table of their own.
const CLAUDE_UNATTENDED = Object.freeze({ permissionMode: "bypassPermissions" });

// What `--permission-mode bypassPermissions` MEANS on Codex, and the only
// thing it ever translates to (dec-spor-codex-dispatch-bypass-translate):
// Codex's widest sandbox and a policy that never stops to ask. Spelled in
// spor's own flag vocabulary (`--approval-policy`, which codexArgs renders
// as Codex's `--ask-for-approval`), since that is what the caller can type.
const CODEX_BYPASS = Object.freeze({ sandbox: "danger-full-access", approvalPolicy: "never" });

// The ATTENDED posture, the third declaration beside `readOnly` and
// `unattended`: what a harness needs on its argv to STOP and ask before a
// write nobody has pre-approved. Claude Code needs nothing — every permission
// mode but `plan`/`bypassPermissions` asks, so attended is its default and the
// declaration is empty (declared and empty, distinct from an adapter that
// declares none at all). Codex is the mirror image: its argv builder defaults
// to `--ask-for-approval never`, so attended must be SAID — `--approval-policy
// on-request`. OpenCode and Copilot declare NO attended posture, because
// `--auto` / `--allow-all --no-ask-user` are baked into their argv builders
// and cannot be unsaid; a caller re-expressing an attended posture there has
// to narrow to `readOnly` instead (never widen to the unattended default).
const CLAUDE_ATTENDED = Object.freeze({});
...
const CLAUDE_NATIVE_BACKGROUND = Object.freeze({
  ...
  // How to STOP one of those agents, as argv the session id is appended to.
  // A finished `--bg` agent does not leave the daemon by itself, so a run
  // reconciled terminal on the turn-complete signal frees its slot with this
  // (task-spor-dispatch-native-bg-terminal-detection). Declared beside the
  // discovery it mirrors, for the same reason: what a machine executes is the
  // adapter's to say, never the run store's.
  activeStop: Object.freeze({ kind: "cli-args", args: ["stop"] }),
  buildArgs: claudeBackgroundArgs,
  // Same read-only posture as the supervised launch (`--read-only --bg`).
  readOnly: Object.freeze({ permissionMode: "plan" }),
  attended: CLAUDE_ATTENDED,
  unattended: CLAUDE_UNATTENDED,
  postureMeaning: claudePostureMeaning,
...
const ADAPTERS = Object.freeze({
  ...
  "claude-code": Object.freeze({
    ...
    // The harness's read-only posture (`spor dispatch --read-only`, the
    // review gate's launch): plan mode reads and runs but cannot edit.
    readOnly: Object.freeze({ permissionMode: "plan" }),
    // …the ATTENDED one — empty, since every mode but plan/bypass asks (see
    // CLAUDE_ATTENDED) — and the UNATTENDED one: Claude Code is the only
    // harness that needs a flag to run with nobody there to answer a prompt
    // (see CLAUDE_UNATTENDED).
    attended: CLAUDE_ATTENDED,
    unattended: CLAUDE_UNATTENDED,
    // …and what its own posture flag MEANS, for a caller re-expressing it in
    // another harness's vocabulary (see postureMeaning).
    postureMeaning: claudePostureMeaning,
    validateOptions: claudeValidateOptions,
    sessionFromEvent: claudeSessionFromEvent,
    reportFromEvent: claudeReportFromEvent,
    failureFromEvent: claudeFailureFromEvent,
  ...
  codex: Object.freeze({
    ...
    // Codex's own read-only sandbox: the filesystem is readable, commands
    // run, nothing under the tree can be written.
    readOnly: Object.freeze({ sandbox: "read-only" }),
    // Attended must be SAID: codexArgs defaults to `--ask-for-approval never`,
    // so a posture that stops to ask is an explicit `on-request` policy (see
    // CLAUDE_ATTENDED for the contrast).
    attended: Object.freeze({ approvalPolicy: "on-request" }),
    // Unattended needs NO flag: codexArgs already defaults to
    // `--ask-for-approval never --sandbox workspace-write`.
    unattended: Object.freeze({}),
    postureMeaning: codexPostureMeaning,
    validateOptions({ permissionMode, agent, sandbox, approvalPolicy }) {
      ...
      // instead of a silent behavior change. The translation is FIXED —
      // `--sandbox danger-full-access --approval-policy never`, nothing else
      // — so an explicit --sandbox/--approval-policy beside it is a
      // contradiction, not an override, and is REFUSED like every other
      // mixed-vocabulary posture (issue-spor-codex-bypass-translation-
      // validation-flaws): letting the Codex flag win would either widen or
      // narrow what "bypass" means per call while the warning still claimed
      // a translation. A caller who wants a specific Codex posture says it in
      // Codex flags alone. Every OTHER permission-mode value
      // (default/plan/acceptEdits/…) has no Codex analog and still hard-errors.
      if (permissionMode === "bypassPermissions") {
        if (sandbox || approvalPolicy) {
          const said = [sandbox && `--sandbox ${sandbox}`, approvalPolicy && `--approval-policy ${approvalPolicy}`].filter(Boolean).join(" ");
          return {
            message: `cannot combine --permission-mode bypassPermissions with ${said} on a Codex dispatch — bypassPermissions translates to exactly --sandbox ${CODEX_BYPASS.sandbox} --approval-policy ${CODEX_BYPASS.approvalPolicy}, and an explicit Codex flag beside it is a contradiction, not an override.`,
            hint: "drop --permission-mode and pass the Codex flags (--sandbox/--approval-policy) alone, or drop the Codex flags and keep the bypass translation.",
          };
        }
        return {
          translate: { ...CODEX_BYPASS },
          warning:
            `warning: --permission-mode bypassPermissions has no Codex flag; translating to ` +
            `--sandbox ${CODEX_BYPASS.sandbox} --approval-policy ${CODEX_BYPASS.approvalPolicy} for this Codex dispatch.`,
        };
      }
      return {
    ...
    // Read-only: every assistant message on the stream, for readers of a
    // finished run (runReportTexts). Deliberately NOT reportFromEvent — the
    // report stays the file `--output-last-message` writes.
    messageFromEvent: codexMessageFromEvent,
    failureFromEvent: codexFailureFromEvent,
  ...
  opencode: Object.freeze({
    ...
    // The harness's read-only posture (`spor dispatch --read-only`): the
    // built-in `plan` agent, whose permission table denies `edit` everywhere,
    // PLUS the `bash` denial `opencodePrepareRun` layers in through
    // OPENCODE_CONFIG_CONTENT — plan mode alone leaves the shell write-capable.
    readOnly: Object.freeze({ agent: "plan" }),
    // Unattended needs no flag: `--auto` is already in opencodeArgs — and
    // because it cannot be unsaid, there is NO attended posture to declare.
    unattended: Object.freeze({}),
  ...
  copilot: Object.freeze({
    ...
    // The read-only posture: deny the file-writing tool AND the shell tool at
    // the permission layer (`--deny-tool write --deny-tool shell`, which
    // override `--allow-all`). See copilotArgs for what that costs.
    readOnly: Object.freeze({ denyTools: Object.freeze(["write", "shell"]) }),
    // Unattended needs no flag: `--allow-all --no-ask-user` are in copilotArgs
    // — and because they cannot be unsaid, there is NO attended posture.
    unattended: Object.freeze({}),
    validateOptions(options) {
...
// A Windows DRIVE-RELATIVE path — `C:foo.exe`, a drive letter with no
// separator after the colon — is relative to that drive's OWN current
// directory, not a fixed location; only `C:\foo.exe` (or `C:/foo.exe`) is
// drive-ABSOLUTE. Neither `path.isAbsolute` nor a bare separator test call
// this out (it carries no `/`/`\` at all when the command has no further
// path segments), so it is checked on its own.
const WIN32_DRIVE_RELATIVE_RE = /^[A-Za-z]:(?![\\/])/;

// True for a command string that names a path (carries a `/` or `\`
// separator, either platform's, or a Windows drive prefix) but is not
// rooted — `./run.sh`, `bin/ox`, `scripts\ox.bat`, `C:foo.exe`.
// child_process resolves exactly this shape against the spawn's `cwd`,
// never against a fixed machine location, so it is the one spelling that
// lets the dispatched repo's own working tree — content any collaborator's
// commit can write — decide what executes. A bare name (no separator, no
// drive prefix) is a PATH lookup on POSIX, immune to repo content; an
// absolute path is already rooted. Checked on BOTH separators regardless of
// host platform: a declaration is hand-authored machine config that could
// be copied across machines, and `path.isAbsolute` alone would pass a
// Windows-style relative-with-backslash string through unrooted on POSIX.
//
// Residual, NOT closed by this check: on Windows, libuv's own executable
// search for a bare name (no separator at all) checks the spawned child's
// `cwd` BEFORE falling back to PATH (src/win/process.c `search_path`) — so
// even a bare-name declaration is resolvable against the dispatched repo's
// worktree there, the same hazard this function exists to close for a
// path-shaped command. Closing that fully means never handing spawn a bare
// name at all (pre-resolving it to an absolute path via a PATH-only lookup
// that does not consult cwd) across every harness launch, built-in and
// declared alike — broader than this declaration-level check and unverified
// on real Windows from this box; tracked as follow-up work, not fixed here.
function isRelativeCommandPath(cmd) {
  if (WIN32_DRIVE_RELATIVE_RE.test(cmd)) return true;
  return /[\\/]/.test(cmd) && !path.isAbsolute(cmd) && !path.win32.isAbsolute(cmd);
}

// A BUILT-IN harness's launcher override — `dispatch.bin.<id>` or its env
// spelling, resolved by explicitHarnessBin() — is the same machine-local
// trust class as a declared harness's `command` (both are stripped from a
// committable `.spor.json` by REPO_FORBIDDEN_PATHS) and is spawned with the
// identical `cwd: launchDir`, so it is refused on the same shape: a relative
// path resolves against the dispatched repo's own worktree, not this
// machine, letting an untrusted commit there choose what runs. A bare name
// (PATH lookup) or a rooted absolute path is unaffected. Only ever checks an
// EXPLICIT override — the built-in default bin names are always bare, so
// there is nothing to refuse when neither the env var nor `dispatch.bin.<id>`
// is set.
function checkHarnessBinOverride(id, { env = process.env, cfg = null } = {}) {
  const explicit = explicitHarnessBin(id, { env, cfg });
  if (!explicit || !isRelativeCommandPath(explicit.path)) return { ok: true };
  return {
    ok: false,
    error: `${explicit.source} must be an absolute path or a bare name resolved on PATH — '${explicit.path}' is a relative path, which would resolve inside the dispatched repo's own worktree and let an untrusted commit there choose what runs`,
  };
}
...
function normalizeHarnessDeclaration(id, raw) {
  ...
  // A RELATIVE path (one containing a separator but not rooted) resolves
  // against the SPAWNED CWD — the dispatched repo's own worktree, content any
  // collaborator's commit controls — not against this machine, breaking the
  // guarantee dec-spor-declarative-harness-machine-binds-execution exists to
  // give: no write another party can make chooses what runs. A bare name (no
  // separator) is safe — it resolves on PATH, which the repo cannot write —
  // and an absolute path is safe by construction, so only the relative-with-
  // separator shape is refused.
  if (isRelativeCommandPath(raw.command)) {
    return fail(`${declarationKey(id, "command")} must be an absolute path or a bare name resolved on PATH — '${raw.command}' is a relative path, which would resolve inside the dispatched repo's own worktree and let an untrusted commit there choose what runs`);
  }
...
module.exports = {
  ...
  opencodePrepareRun, opencodeReadOnlyConfigContent, OPENCODE_READ_ONLY_PERMISSION,
  harnessCommand, explicitHarnessBin, describeHarnessBin, harnessAvailable, isExecutableFile,
  checkHarnessBinOverride,
  ...
  HARNESS_OPTION_FLAGS, harnessOptionFlags, POSTURE_MEANINGS, postureMeaning,
};
