...
function harnessAvailable(id, { env = process.env, cfg = null, which = null } = {}) {
  const explicit = explicitHarnessBin(id, { env, cfg }) || declaredLauncher(id, cfg);
  if (explicit) {
...
function declaredAdapter(declaration) {
  if (!declaration || typeof declaration !== "object" || !declaration.command) return null;
  const d = declaration;
