...
function workerContract({ nodeId, factory = null, terminal = null } = {}) {
  ...
  const lines = [
    ...
    `makes the item stale — and it must be a node that EXISTS. Leave \`${id}\` where the scoping put it: it has to`,
    "be re-stamped to the repo that owns it, or superseded — resolving it is NOT enough on its own, since a",
    "resolver is what gets a run judged in the first place. Then make the FIRST line of your final message exactly",
