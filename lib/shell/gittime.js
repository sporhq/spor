...
// Env-scrubbed (gitSpawn/git-exec.js): an ambient GIT_DIR/GIT_WORK_TREE/
// GIT_COMMON_DIR beats `repoDir` in git's own precedence, so without the scrub
// a caller environment could misdirect these reads at a different repo
// (issue-spor-gittime-git-env-inheritance).
function git(repoDir, args, maxBuffer = 1 << 28) {
  const r = gitSpawn(repoDir, args, {
...
function isAncestor(repoDir, anc, desc) {
  const r = gitSpawn(repoDir, ["merge-base", "--is-ancestor", anc, desc], { stdio: "ignore" });
  return r.status === 0;
}
...
// analytics status-transitions). Fail-open: the cache is an optimization, so
// a write error (read-only home, full disk) is swallowed — the next load
// just re-folds.
function writeJsonCache(file, payload) {
  try {
    writeFileAtomic(file, JSON.stringify(payload), { mkdir: true });
  } catch { /* fail-open */ }
}
...
// cache): { head, fp, state: { id: { status, terminal, runStart, type } } }.
// HEAD-keyed like timestamps.json, plus `fp` — a fingerprint of the TERMINAL
// status vocabulary. Unlike the timestamp cache (which stores PURE,
// vocab-independent git times), the folded `state` bakes in isTerminal
// decisions, so a spor upgrade that changes the terminal set without a graph
// commit must invalidate it; `fp` captures that dependency. `state` is the
// per-node forward-walk state the fold composes onto
// (kernel.foldStatusTransitionState), not the closed-at output, so the
// OLD..NEW range fold can continue the walk. `type` is a lib/analytics.js
// addition (issue-spor-analytics-type-modification-cache-invalidation): the
// node's type as of the last fold, so a later retype — which the fold itself
// only re-evaluates on a ±status: delta — can be detected and forces a full
// rebuild instead of composing onto a stale, wrong-vocabulary entry.
