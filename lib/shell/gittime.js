...
// pure deletes (a removed node isn't tracked). Heavier than --name-only, so it
// rides a larger maxBuffer and is OFF the no-LLM prompt path (analytics only).
// Returns raw text or null (fail-open: a non-git home / oversized buffer / dead
// binary -> no completion data, never a throw). kernel.foldStatusTransitions
// parses it.
function logStatusTransitions(repoDir, nodesName) {
  return git(repoDir, [
    "log", "--reverse", "--no-renames", "--diff-filter=ACMR", "--format=%x01%ct",
    "-p", "--", `${nodesName}/`,
  ], 1 << 29);
}
...
module.exports = { gitHead, isAncestor, logTimestamps, logStatusTransitions, readCache, writeCache };
