...
function buildDeclineFinding({ nodeId, runId, harness, project, reason, reportText, state, date }) {
  ...
  const lines = [
    ...
    `> ${oneLine(reason, 400)}`,
    "",
    `The item's \`readiness: agent\` stamp was cleared and its lease released, so it returns to triage`,
    "carrying this finding instead of a gate escalation: re-brief it (or retire it) before it is",
    "dispatched again. Nothing here resolves the target.",
    "",
...
async function applyTerminalContract({
  ...
  const releaseLease = async (note) => {
    ...
    // `false` is the honest reading and it is a claim about our KNOWLEDGE, not
    // about the lease: a refusal and a lost answer are indistinguishable here,
    // and the second sits over a release the server may well have committed.
    // That costs the operator nothing, because the remedy reconciles instead of
    // assuming — `/release` is idempotent, and a claim someone else now holds
    // answers 409 naming them (API.md §3) rather than being yanked back.
