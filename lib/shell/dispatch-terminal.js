...
// (`registry.attachesResolutionHook`); the remote leg has no graph, but it does
...
// override that adds or drops a `get` hook is honored with no code change — an
// added hook no longer leaves a terminal own-status reading `resolved` with no
// edge behind it, and a dropped one no longer leaves a genuinely status-retired
// node reading as not attested. The schema fetch is best-effort
...
// (`graphLib.attachesResolutionHookOffline` / `isCompletionStatusOffline`) —
...
// has one, else the shipped SEED pack (`graphLib.attachesResolutionHookOffline`,
...
function effectiveEdgeVerified(type, liveFacts) {
  if (!type) return true;
  return liveFacts ? liveFacts.edgeVerified : graphLib.attachesResolutionHookOffline(type);
}
