...
//
// Enforcement is scoped to runs this box can judge to be OVER, against a graph
// it can reach. In v1 that meant supervised-jsonl launches alone
// (dec-spor-dispatch-terminal-states-supervised-first) — the one reason a
// native-background run was excluded is that its termination could not be
// deterministically observed. It can be now
// (task-spor-dispatch-native-bg-terminal-detection: a still-listed agent whose
// three turn-complete signals agree), so a `--bg` run reaches this door too,
// through `settleNativeContracts` rather than a supervisor. What stays
// BEST-EFFORT — stamped `terminal_enforced: false`, never `resolved` — is
// everything nothing could check: a free-text dispatch with no target node, a
// server we could not reach, a native run whose harness could not be listed or
// whose ending has no attributable transcript. Unenforced is stated on the
// record, never silently treated as covered.
//
...
// issue, question, incident — declare `resolution.verified_by: edge` and
// attach a `get()` hook that enriches `GET /v1/nodes/{id}` with `resolution`:
// a live inbound resolving edge (API.md §3), verified against the graph rather
// than trusted from the agent's word. Every other dispatchable type —
// decision, finding, capture-pending, … — declares `verified_by: status` and
// is retired by STATUS alone; its response carries no `resolution` no matter how
...
// contract, task-spor-dispatch-terminal-resolution-all-types) — and, since
// issue-spor-remote-dispatch-ignores-resident-resolution-hooks, the SAME kind
// of registry: the local leg asks the graph it just loaded
// (`registry.isEdgeVerified`); the remote leg has no graph, but it does
// have a server to ask, so it fetches the server's own live registry via
// `GET /v1/schema` (`graphLib.resolutionFactsFromSchemaSnapshot`) rather than
// reverse-engineering the shipped SEED pack. Either way, a resident schema
// override that declares the OTHER path is honored with no code change — a
// type declared edge-verified no longer leaves a terminal own-status reading
// `resolved` with no edge behind it, and one declared status-verified no
// longer leaves a genuinely status-retired node reading as not attested. Only
// a schema that declares NEITHER (one authored before the key existed) is
// still read through the legacy `get`-hook proxy, whose fragility is
// issue-spor-offline-check-get-hook-resolution-proxy: `get()` is the
// general-purpose read-time enrichment hook, so its mere presence never meant
// "edge-verified". The schema fetch is best-effort
// (`resolveLiveFacts` below): an old server with no `GET /v1/schema`, a
// network hiccup, or an unparseable body all fall back to `null`, at which
// point the caller reads the shipped seed pack exactly as before this existed
// (`graphLib.isEdgeVerifiedOffline` / `isCompletionStatusOffline`) —
// never a harder failure over an introspection endpoint being unavailable.
// For the status-only types, the registry's OWN-lifecycle `status.terminal`
// partition (decision `settled`, finding `resolved`, capture-pending
// `merged`, …) is read UNIONED with the type-blind universal completion
// words — never a hand-hardcoded per-type status table either.
...
function verifyLocalResolution(nodesDir, nodeId) {
  ...
  const edgeVerified = !type || graph.registry.isEdgeVerified(type);
  if (edgeVerified) {
  ...
  // completion words — a capture-pending `merged` has no per-type
  // declaration of its own (deliberately, see schema-capture-pending) and
  // reads terminal only via that union. Unlike the offline helper, a local
  // run already has a real loaded `graph`, so the type-blind half reads
  // `kernelResolution.terminalVocabulary(graph)` — the fallback UNIONED with
  // a resident `terminal-status` register override — instead of the bare
  // hardcoded fallback array: before this fix, a graph that grew the
  // vocabulary with its own custom universal terminal status (e.g.
  // `archived`) had that status invisible to local-mode dispatch verify even
  // though the graph carrying the override was already loaded right here
  // (issue-spor-terminal-status-validation-regressions, the local-mode twin
  // of the same gap fixed remotely in resolutionFactsFromSchemaSnapshot).
  const completionTerminal = kernelResolution.terminalVocabulary(graph).has(status) || graph.registry.terminalStatuses(type).has(status);
...
// Which of the two attestation paths applies to TYPE — a resolving edge, or
// the node's own status — read off `liveFacts` (a fetched `GET /v1/schema`
// snapshot, via `graphLib.resolutionFactsFromSchemaSnapshot`) when the caller
// has one, else the shipped SEED pack (`graphLib.isEdgeVerifiedOffline`,
// see the header comment above for what that misses). An empty TYPE (a server
// that does not echo one) is treated as edge-verified regardless of
// `liveFacts`: absent evidence of a mismatch, demanding the edge is the
// fail-safe reading.
function effectiveEdgeVerified(type, liveFacts) {
  if (!type) return true;
  return liveFacts ? liveFacts.edgeVerified : graphLib.isEdgeVerifiedOffline(type);
}

// The status-only counterpart of effectiveEdgeVerified above: is STATUS
// terminal for TYPE, read off the same source `liveFacts` came from.
function effectiveIsCompletionStatus(status, type, liveFacts) {
  return liveFacts ? liveFacts.isCompletionStatus(status) : graphLib.isCompletionStatusOffline(status, type);
}

// Fetch the live schema registry's per-type resolution facts
// (issue-spor-remote-dispatch-ignores-resident-resolution-hooks) — best-effort,
// never throws. `alreadyResolved` lets a caller skip the fetch entirely when a
// live resolving edge already answered the question (a resolved node's type
// doesn't matter); an empty `type` skips it too, matching
// effectiveEdgeVerified's own fail-safe reading for one. `fetchSchema` is the
// caller's own zero-arg transport — dispatch-terminal's own DI'd `call`, or
// bin/spor.js's `remote.get(cfg, ...)` — so this stays agnostic of which one
// is in play; it must resolve to the parsed `GET /v1/schema` body, or a
// falsy value on any failure (network error, non-2xx, unparseable body).
// Returns `null` on any failure, which every reader here (effective* above,
// resolvedOutcomeFromNode below) already treats as "fall back to the seed
// pack" — an unreachable or older server never harder-fails a dispatch over
// an introspection endpoint being unavailable. A snapshot that parsed but
// carries no `node_types` array (a mismatched server version, a proxy that
// mangled the body) is caught the same way: `resolutionFactsFromSchemaSnapshot`
// would otherwise read a MISSING `node_types` as "this type is absent from
// the registry" (edgeVerified: false) — silently the OPPOSITE reading from
// the offline fallback for a genuinely edge-verified type like `task` — so
// that shape is refused here rather than handed to it.
async function resolveLiveFacts({ type, alreadyResolved = false, fetchSchema }) {
  if (alreadyResolved || !type) return null;
  let snapshot;
  try {
    snapshot = await fetchSchema();
  } catch {
    return null;
  }
  if (!snapshot || !Array.isArray(snapshot.node_types)) return null;
  return graphLib.resolutionFactsFromSchemaSnapshot(snapshot, type);
}
...
// node JSON `GET /v1/nodes/{id}` returns (plus an optional `liveFacts` from
// `resolveLiveFacts` above) and answers with the same `resolved` patch shape
// `verifyLocalResolution` produces off a local graph, or null when the target
// is not (yet) done — the fail-safe direction, since "we could not see a
// resolution" must never read as one. `liveFacts` is optional and defaults to
// null (the offline seed-pack reading) so existing callers/tests that predate
// the live-registry fetch keep working unchanged.
function resolvedOutcomeFromNode(node, liveFacts = null) {
  const n = node || {};
  const resolution = n.resolution;
  const type = String(n.type || "").toLowerCase();
  const edgeVerified = effectiveEdgeVerified(type, liveFacts);
  ...
  if (!edgeVerified && effectiveIsCompletionStatus(n.status, type, liveFacts)) {
...
// nothing; it re-briefs. While it stays LIVE (not yet resolved/dismissed by a
// person) it also GATES re-dispatch of the same node — `spor dispatch --node`
// refuses and the work loop skips it visibly (task-spor-decline-finding-
// gates-redispatch, `dispatchDeclineFindingCheck` in bin/spor.js) — so
// filing it is not the end of the story for this node the way a bare
// `relates-to` usually is. Deterministic id per run, so a retried filing is
// one node.
...
async function httpJson({ method, url, token, body, timeoutMs = REQUEST_TIMEOUT_MS }) {
  ...
  try {
    ...
    try {
      ...
      // `res.ok` still reflects the HTTP status alone — an empty or malformed
      // body on an otherwise-2xx response is not an HTTP-layer error. But a
      // caller that needs the PARSED body (the verify leg reading
      // `resolution`, the write leg reading `results[0]`) must be able to tell
      // "the body failed to parse" apart from "the body legitimately parsed to
      // null/{}" — silently collapsing the two into `json: null` let a failed
      // verification read as a confidently-empty one
      // (issue-spor-dispatch-terminal-verification-vulnerabilities).
      jsonError = (e && e.message) || "invalid JSON body";
    }
    return { ok: res.ok, status: res.status, json, jsonError };
  } catch (e) {
...
function nodeWriteLanded(res) {
  ...
  // No per-entry verdict to read: a genuine write always comes back with a
  // `results` array (a 207 included), so a response missing one — an empty or
  // malformed body, an unexpected shape — is not evidence the node landed.
  // Falling back to the bare HTTP `ok` here was the bug: it let a 200 with no
  // parseable body read as a successful write and release the lease on a
...
async function applyTerminalContract({
  ...
  // `got.jsonError` means the response body did not parse — a 2xx with an
  // empty or malformed body is not a legitimate "no resolution" reading, it is
  // a failed read (issue-spor-dispatch-terminal-verification-vulnerabilities):
  // treating it as `{}` used to fall straight into "not resolved" and file an
  // unwarranted `reported` outcome instead of the unenforced one this actually
  // is.
  if (!got || !got.ok || got.jsonError) {
    const why = (got && (got.jsonError ? `invalid response body (${got.jsonError})` : got.error || `HTTP ${got.status}`)) || "no response";
    // `terminal_unreachable` is the one structural mark that says WHY this
    // outcome is unenforced: the graph could not be reached, not that the run
    // did something. `terminal_enforced: false` alone cannot say that — a
    // never-started run and the runner's own provisional patch are unenforced
    // too — and the execution classifier (kernel/gates.js §5.3) has to tell an
    // outage from a code failure without sniffing a note string.
    return { ...unenforcedOutcome(state, `could not re-read ${nodeId} to verify its resolution (${why}) — the outcome below is unverified`, reportText), terminal_unreachable: true };
  }
  const node = got.json || {};
  const type = String(node.type || "").toLowerCase();
  // 1b. Ask the LIVE registry which attestation path applies to this type,
  // when the answer isn't already settled by a resolving edge
  // (issue-spor-remote-dispatch-ignores-resident-resolution-hooks). Best-effort:
  // `resolveLiveFacts` falls back to `null` on any failure, at which point
  // `effectiveEdgeVerified`/`resolvedOutcomeFromNode` read the shipped seed pack
  // exactly as before this fetch existed.
  const liveFacts = await resolveLiveFacts({
    type,
    alreadyResolved: !!(node.resolution && node.resolution.by),
    fetchSchema: async () => {
      const r = await call({ method: "GET", path: "/v1/schema" });
      return r && r.ok && !r.jsonError ? r.json : null;
    },
  });
  ...
  const edgeVerified = effectiveEdgeVerified(type, liveFacts);
  const resolved = resolvedOutcomeFromNode(node, liveFacts);
  ...
  if (decline) {
    ...
    if (!nodeWriteLanded(wrote)) {
      const why = (wrote && (wrote.jsonError ? `invalid response body (${wrote.jsonError})` : wrote.error || `HTTP ${wrote.status}`)) || "no response";
      return {
        terminal_state: "declined",
        terminal_enforced: true,
        declined_reason: decline.reason,
  ...
  const fileReport = async () => {
    ...
    return { id: artifact.id, ok: nodeWriteLanded(wrote), why: (wrote && (wrote.jsonError ? `invalid response body (${wrote.jsonError})` : wrote.error || `HTTP ${wrote.status}`)) || "no response" };
  };
...
module.exports = {
  ...
  resolveLiveFacts,
  effectiveEdgeVerified,
  effectiveIsCompletionStatus,
  reportArtifactId,
  buildReportArtifact,
  parseDecline,
  declineFindingId,
