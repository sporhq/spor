...
// GIT_COMMON_DIR can't misdirect this history read at a different repo
// (issue-spor-gittime-git-env-inheritance).
function git(repoDir, args, maxBuffer = 1 << 28) {
  const r = gitSpawn(repoDir, args, { maxBuffer });
...
// twin of the server's personEmailIndex(store.graph): persons carry an `email`
  // field, so this indexes type:person nodes by lowercased email → {id,name}.
...
function personEmailIndex(graph) {
  ...
  for (const [id, n] of Object.entries((graph && graph.nodes) || {})) {
    if (n && n.type === "person" && typeof n.email === "string" && n.email) {
      idx.set(n.email.toLowerCase(), { id, name: n.name || n.title || n.email || id });
    }
  }
  return idx;
}
...
function decorateActor(rec, emailIdx) {
  ...
  const mapped = !internal && email && emailIdx ? emailIdx.get(email) || null : null;
  const person = mapped && typeof mapped === "object" ? mapped.id : mapped;
  const personName = mapped && typeof mapped === "object" ? mapped.name : null;
  return {
    sha: rec.sha || null,
...
function collectEntry({ nodesDir, id, sha, emailIdx = null } = {}) {
  ...
  // An unresolvable sha is rc != 0 -> null; a commit that didn't touch the path
  // is rc 0 with no --name-status row for it. Whether the `--format` header is
  // ALSO suppressed in that case is git-version-dependent (git <= 2.4x printed
  // nothing at all; 2.55 prints the header and an empty diff), so the name-status
  // row — the only direct evidence the commit changed the path — is what decides
  // membership. Keying on the header instead reported an untouched commit as a
  // real revision (change: null, empty patch) on newer git.
  ...
  const statusLine = lines.find((l) => /^[ACDMRTUXB]+\d*\t/.test(l));
  if (!metaLine || !statusLine) return { ok: false, code: "not_in_history" };
  const [fullSha, date, name, email, subject] = metaLine.split("\x1f");
  const change = statusLine[0]; // A | M | D | R (first letter, so a merge's combined "MM" reads as M)
...
function renderList(env) {
  ...
  for (const h of history) {
    const who = actorOf(h);
    const person = h.person ? `  [${h.person_name || h.person}]` : "";
...
function renderEntry(entry, { content = false } = {}) {
  const e = entry || {};
  const who = actorOf(e);
  const person = e.person ? `  [${e.person_name || e.person}]` : "";
