...
// A bare ISO date (YYYY-MM-DD, no time-of-day) is a git approxidate gotcha:
// without an explicit time, git anchors it to the CURRENT wall-clock time, not
// midnight — so `--since=<today's date>` compares against "today at the
// moment this process started" and silently drops everything committed
// earlier today (the reported boundary bug,
// issue-spor-changes-since-boundary-exclusive). Pin a bare date to UTC
// midnight (an explicit `Z` offset, not a bare "HH:MM:SS" — that would still
// anchor to the process's LOCAL timezone, which can shift the boundary by
// hours and reintroduce the same drop for a box not running in UTC, since
// `date` elsewhere in this envelope is UTC-normalized `%ct`) so it always
// means "from the start of that day".
...
function collect({ nodesDir, since = null, project = null, limit, keep = null } = {}) {
  ...
  if (since) {
    if (isShaLike(since)) {
      const ok = gitSpawn(repoDir, ["rev-parse", "--verify", "--quiet", `${since}^{commit}`], { stdio: "ignore" });
      if (ok.status !== 0) {
      ...
      logArgs.push(`--since=${DATE_ONLY_RE.test(since) ? `${since}T00:00:00Z` : since}`);
    }
  }
