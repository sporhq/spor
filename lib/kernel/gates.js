...
function parseReviewVerdict(text, { prior = [], cycle = 0, raised = [] } = {}) {
  ...
  for (const src of candidates.reverse()) {
    ...
    const carried = priorList.map((p) => ({
      ...
      // The rows the reviewer enumerated for a confirmed-open finding; the
      // ledger's earlier list stands when this verdict names none.
      rows: answers.has(p.id) && answers.get(p.id).rows.length ? answers.get(p.id).rows : mechanismRows(p.rows),
    }));
...
function openPriorFindings(ledger) {
  return (ledger || [])
    ...
    .map((e) => ({ id: e.id, severity: e.severity, file: e.file, summary: e.summary, evidence: e.evidence || "", opened: e.opened, rows: mechanismRows(e.rows) }));
}
...
function applyReviewToLedger(ledger, verdict, cycle) {
  ...
  for (const p of (verdict && verdict.prior) || []) {
    ...
    e.answered = cycle;
    // A confirmed-open finding keeps the rows the reviewer enumerated (the
    // fixer and the next review read them); a resolved one has no open rows.
    const rows = mechanismRows(p.rows);
    if (e.status === "resolved") e.rows = [];
    else if (rows.length) e.rows = rows;
  }
...
function renderFindings(findings, cap = 20, lineCap = 240) {
  ...
  const lines = list
    ...
    .map((f, i) => {
      ...
      return rows.length ? [head, ...rows.map((r) => `    row: ${r}`)].join("\n") : head;
    });
