...
const SETTLED_GATE_STATES = new Set(["passed", "failed", "blocked"]);
...
function parseIntegration(payload) {
  ...
  if (!INTEGRATION_MODES.includes(mode)) {
    ...
    // v1 scope (task-spor-factory-integration-step): propose mode composes
    // with a human gate over an opened PR, which is a larger surface than this
    // pass covers. Refuse loudly at load, the same place every other factory
    // authoring mistake is caught, rather than accept a declaration this
    // runner cannot honor.
    errors.push("integration.mode 'propose' is not yet implemented — declare 'local' or 'push', or drop the integration block");
  }
...
// non-empty means the worker must REFUSE rather than run ungated.
function parseFactory(body, { gateNodes = new Map(), id = null } = {}) {
