...
// How a candidate's commit is made reachable to whatever judges it (§3). Local
// mode's controller runs on the machine that holds the checkout, so a sha plus
// that machine's identity IS a sufficient reference there and `none` is the
// default; the other two exist for a controller that does not share a
// filesystem with the implementer.
const CANDIDATE_PUBLISH = Object.freeze(["none", "branch", "bundle"]);
...
function parseImplementation(payload, { gates = [] } = {}) {
  ...
  if (declared) {
    ...
    if (!isPlainObject(raw)) {
      ...
      const publish = String(strOr(candidate.publish, "none")).trim().toLowerCase();
      if (!CANDIDATE_PUBLISH.includes(publish)) {
        errors.push(`implementation.candidate.publish '${candidate.publish}' must be one of: ${CANDIDATE_PUBLISH.join(", ")}`);
      }
