...
// A carried finding's row fields from the reviewer's answer (`answer` is
// undefined when the finding went unanswered): `rows` is this review's own
// enumeration (empty when it named none, or when it cleared the finding),
// `earlierRows`/`earlierRowsCycle` the last enumeration an earlier review
// made when this one did not replace it. An answer that enumerates anew
// SUPERSEDES the earlier list — the reviewer just restated the design.
function carriedRowsOf(p, answer) {
  const rows = answer ? answer.rows : [];
  if (answer && answer.cleared) return { rows: [], earlierRows: [], earlierRowsCycle: null };
...
function rowByRowFindings(findings, cycle) {
  return (findings || []).filter(
    (f) => f && f.origin === "prior" && f.status === "open" && carriedFixCycles(f, cycle) >= ROW_BY_ROW_CARRY && mechanismRows(f.rows).length < 2
  );
}
