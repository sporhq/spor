...
function parseImplementation(payload, { gates = [] } = {}) {
  ...
  if (declared) {
    ...
    if (!isPlainObject(raw)) {
      ...
      const publish = String(candidate.publish || "none").trim().toLowerCase();
  ...
  const by = String(completionRaw.by || defaultBy).trim().toLowerCase();
  ...
  // ignored `errors` must not find `by: "nobody"` here to act on.
  const completion = {
    by: COMPLETION_BY.includes(by) ? by : defaultBy,
    after: COMPLETION_AFTER.includes(after) ? after : defaultAfter,
  };

  return { implementation: errors.length ? null : implementation, completion, errors };
}
