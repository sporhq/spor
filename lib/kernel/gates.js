...
function parseImplementation(payload, { gates = [] } = {}) {
    ...
    if (!isPlainObject(raw)) {
      ...
      implementation = {
        ...
        budget: {
          ...
          attempts: intOr(budget.attempts, GATE_DEFAULTS.implementationAttempts, { min: 1, max: 3 }),
        },
        retry: {
          ...
          attempts: intOr(retry.attempts, GATE_DEFAULTS.implementationRetryAttempts, { min: 0, max: 3 }),
          backoffMs: intOr(retry.backoff_ms, GATE_DEFAULTS.implementationRetryBackoffMs, { min: 1000, max: 600000 }),
        },
        candidate: {
  ...
  // The completion boundary. `by` defaults off the DECLARED block rather than
  // off the parsed one: the block is the opt-in, and a factory that adopted a
  // stage has asked for the stage's semantics, of which controller-written
  // completion is the load-bearing one.
  ...
  const defaultBy = declared ? "controller" : "agent";
