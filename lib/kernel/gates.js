...
// read off the graph. And the evidence is deliberately NOT "the item reads
// resolved": a live resolving edge is the PRECONDITION for being gated at all
// (`shouldGate`), so accepting it here would let one extra frontmatter key
// route the exact "resolved with nothing behind it" run the empty-diff refusal
// names in its own text. What the check demands instead is a write the run
// could not have made by doing nothing — the item re-stamped to another repo,
// the item superseded, and (for the two FINDING outcomes) a node that EXISTS
// and is named as what made the item stale. Anything unproven falls through to
// the empty-diff refusal exactly as before.
...
function verifyNoCodeOutcome({ nodeId, claim, nodes = {}, claimedRepo = null }) {
  ...
  const superseded = supersededBy && supersededBy !== resolverId ? `superseded by ${supersededBy}` : null;
  ...
  if (!moved) {
    return {
      ...
      reason:
        ...
        `${outcome === "rescoped" ? "" : `, and ${supersededBy ? `the only thing superseding it is \`${supersededBy}\`, the very node this run declared` : "nothing supersedes it"}`}` +
