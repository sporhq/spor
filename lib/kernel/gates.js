...
// something this does not do". Everything else readable is `correctness` —
// the default, and the safe one: mislabelling a defect as unmet would only
// short-circuit the budget, so the fallback must not be the short-circuit.
...
// The category a reviewer stated, or null when it stated none — the caller
// decides what unstated means (a fresh finding defaults to `correctness`; a
// prior answer that says nothing leaves the ledger entry's category alone).
function findingCategory(raw) {
  const t = String(raw === undefined || raw === null ? "" : raw).trim().toLowerCase().replace(/[\s_]+/g, "-");
  if (!t) return null;
  return UNMET_CONDITION_WORDS.has(t) ? UNMET_CONDITION : CORRECTNESS;
}
