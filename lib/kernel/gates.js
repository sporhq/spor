...
function parseReviewVerdict(text, { prior = [], cycle = 0 } = {}) {
  ...
  for (const src of candidates.reverse()) {
    ...
    const findingsRaw = Array.isArray(obj.findings) ? obj.findings.filter(isPlainObject) : null;
    ...
    const fresh = (findingsRaw || [])
      .map((f) => normalizeFinding(f, cycle))
...
function normalizeFinding(f, cycle) {
...
function withLedgerIds(findings, ledger, cycle) {
  const minted = (ledger || []).filter((e) => e.opened === cycle);
  let i = 0;
  return (findings || []).map((f) => {
    if (f.id || f.origin === "prior") return f;
    const e = minted[i++];
