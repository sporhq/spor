...
// Every repo-relative source path a suite's output names, in first-seen order.
// `dir` is the judged tree's root, so an absolute path printed by a stack frame
// comes back repo-relative and comparable with the change's own paths; a path
// outside that tree (a dependency, a system file) is dropped rather than
// guessed at, and so is every absolute path when the root is unknown — the
// direction that finds FEWER files, which is the safe one (see offDiffFailure).
...
function describeFailingFiles(cls, isolation = null) {
  ...
  const where = c.offDiff ? "none of which the change touches" : `${c.onDiff.length} of which the change touches (${c.onDiff.slice(0, 3).join(", ")})`;
  ...
  // describeFlake), so the only isolation this clause reports is one that
  // failed too — the reading that turns "off-diff" from a mitigation into
  // evidence the tree really is broken.
