...
function describeFailingFiles(cls, isolation = null) {
  ...
  if (isolation && !isolation.ok && (isolation.files || []).length) {
    ...
    s += `, and re-running ${isolation.files.join(", ")} on its own PASSED — an off-diff flake, but it could not be filed as its own issue${isolation.fileError ? ` (${isolation.fileError})` : ""}, so the failure is charged rather than passed on a record nothing keeps`;
  }
  return s;
}
