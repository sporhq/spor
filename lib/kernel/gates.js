...
function parseReviewVerdict(text, { prior = [], cycle = 0, raised = [] } = {}) {
  ...
  const unansweredPrior = (why) => priorList.map((p) => ({ ...p, origin: "prior", blocking: true, status: "open", answered: false, note: `not answered by this review (${why})`, ...carriedRowsOf(p, undefined) }));
...
function readPriorAnswers(obj, priorList) {
  ...
  const put = (id, status, note, rows) => {
    const key = String(id || "").trim();
    const s = String(status || "").trim().toLowerCase();
    if (!known.has(key)) return;
    if (PRIOR_CLEARED.has(s)) answers.set(key, { cleared: true, note: String(note || "").trim() || "cleared by the review", rows: [] });
    else if (PRIOR_OPEN.has(s)) answers.set(key, { cleared: false, note: String(note || "").trim() || "confirmed still open by the review", rows: mechanismRows(rows) });
  };
  const raw = obj.prior !== undefined ? obj.prior : obj.prior_findings;
  if (Array.isArray(raw)) {
    for (const p of raw) {
      if (isPlainObject(p)) put(p.id || p.ref, p.status || p.state || p.verdict, p.note || p.reason || p.summary, priorRowsOf(p));
    }
  } else if (isPlainObject(raw)) {
    for (const [id, v] of Object.entries(raw)) {
      if (isPlainObject(v)) put(id, v.status || v.state || v.verdict, v.note || v.reason, priorRowsOf(v));
      else put(id, v, "", null);
    }
  }
  return answers;
...
function renderFindings(findings, cap = 20, lineCap = 240) {
  ...
  const lines = list
    .slice(0, cap)
    .map((f, i) => {
      const tag = (f.blocking === false && f.note ? `${f.severity || "unspecified"}, advisory` : f.severity || "unspecified") + (f.rowByRow ? ", row-by-row" : "");
