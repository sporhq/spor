...
function verifyNoCodeOutcome({ nodeId, claim, nodes = {}, claimedRepo = null }) {
  ...
  const resolver = nodes[claim.resolver] || null;
  ...
  // The item has to have MOVED, and by a write the run could not have made by
  // doing nothing. Deliberately NOT "the item reads resolved": that is the
  // precondition for being gated (see the header above).
  const item = nodes[id] || null;
  ...
  const moved =
    repoNow && repoWas && repoNow !== repoWas
      ? `re-stamped from repo '${repoWas}' to '${repoNow}'`
      : status === "superseded"
        ? "superseded (status)"
        : item.superseded_by
          ? `superseded by ${item.superseded_by}`
          : hasEdge(resolver, ["supersedes"], id)
            ? `superseded by ${resolverId}`
            : null;
  if (!moved) {
    return {
      ok: false,
      reason:
        `${id} is unchanged: its repo still reads '${repoNow || "(none)"}'${repoWas ? ` (claimed under '${repoWas}')` : ""} and nothing supersedes it` +
        " — a no-code outcome has to re-stamp the item or supersede it, and being resolved is what got this run gated in the first place",
    };
  }
  ...
  if (outcome === "premise-stale" || outcome === "duplicate") {
    ...
    found = candidates.find((c) => nodes[c]) || null;
    if (!found) {
      return {
        ok: false,
