...
const { terminalStatuses } = require("./resolution.js");
...
// read off the graph. Anything unproven falls through to the empty-diff
...
const NO_CODE_ITEM_EDGES = Object.freeze(["resolves", "answers", "relates-to", "derived-from"]);
...
const edgeAwayFrom = (node, types, notTo) => edgesOf(node).find((e) => types.includes(String(e.type).toLowerCase()) && String(e.to) !== notTo);

// Is the work item RETIRED on the graph — a live resolving edge (read by the
// caller's own verify leg), a terminal own-status, or a supersession? Any one
// of the three means the scoping moved the item somewhere terminal.
function noCodeItemRetired({ item, resolved }) {
  if (resolved) return "a live resolving edge";
  const status = String((item && item.status) || "").toLowerCase();
  if (status === "superseded") return "status 'superseded'";
  if (status && terminalStatuses.includes(status)) return `status '${status}'`;
  if (item && item.superseded_by) return `superseded by ${item.superseded_by}`;
  return null;
}

// Does a declared no-code outcome CHECK OUT? Pure: every input has already
// been read (the claim off the run's report, the two nodes off the graph, the
// resolving edge off the caller's verify leg), and the answer is a verdict on
// them. `ok: false` always names what was missing, because that reason is what
// the fall-through refusal shows a person.
//
// `claimedRepo` is the repo stamp the item carried when this pipeline claimed
// it — the "before" value the `rescoped` branch compares against, which is
// also what keeps that branch RUN-RELATIVE: a re-stamp cannot satisfy it twice
// (the second pipeline claims the item under the repo the first moved it to).
function verifyNoCodeOutcome({ nodeId, claim, resolver, item = null, claimedRepo = null, resolved = false }) {
  ...
  if (!declared) return { ok: false, reason: `\`${resolver.id || claim.resolver}\` carries no \`outcome:\` — a no-code outcome is declared on the resolver node, not only in the report` };
  if (declared !== outcome) return { ok: false, reason: `\`${resolver.id || claim.resolver}\` declares \`outcome: ${declared}\`, but the report claims '${outcome}'` };
  if (!hasEdge(resolver, NO_CODE_ITEM_EDGES, id)) {
    return { ok: false, reason: `\`${resolver.id || claim.resolver}\` carries no ${NO_CODE_ITEM_EDGES.join("/")} edge to ${id} — it is not a resolver for this item` };
  }

  // The item has to have MOVED. Either branch is enough — they are the two
  // shapes a scoping takes: the work belongs somewhere else (re-stamped), or
  // it is over here (retired).
  const repoNow = String((item && item.repo) || "").trim();
  const repoWas = String(claimedRepo || "").trim();
  const moved = repoNow && repoWas && repoNow !== repoWas ? `re-stamped from repo '${repoWas}' to '${repoNow}'` : null;
  const retired = noCodeItemRetired({ item, resolved });
  if (!moved && !retired) {
    return {
      ok: false,
      reason:
        `${id} is unchanged: its repo still reads '${repoNow || "(none)"}'${repoWas ? ` (claimed under '${repoWas}')` : ""} and it is neither resolved nor superseded` +
        " — a no-code outcome has to leave the item somewhere new",
    };
  }

  // ...and for the two outcomes that are a FINDING about the item rather than
  // a move, the resolver has to name what it found.
  let found = null;
  if (outcome === "premise-stale" || outcome === "duplicate") {
    const e = edgeAwayFrom(resolver, NO_CODE_FOUND_EDGES, id);
    if (!e) {
      return {
        ok: false,
        reason: `\`${resolver.id || claim.resolver}\` names nothing it found: a '${outcome}' outcome needs a ${NO_CODE_FOUND_EDGES.join("/")} edge to the node that makes the item stale`,
      };
    }
    found = e.to;
  }
  ...
  return {
    ok: true,
    outcome,
    resolver: resolver.id || claim.resolver,
    found,
    detail:
      `the run declared \`${outcome}\` and it checks out: \`${resolver.id || claim.resolver}\` declares it, links ${id}` +
      `${found ? `, and names \`${found}\` as what it found` : ""}` +
      `; ${moved || `${id} is retired (${retired})`}; the branch carries no committed change, and none was owed` +
