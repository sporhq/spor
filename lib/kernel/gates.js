...
// carries evidence is an upgrade of a finding the reviewer DID raise before,
// so rule 2's introduced-by-fix floor does not apply to it.
//
...
function parseReviewVerdict(text, { prior = [], cycle = 0, raised = [] } = {}) {
  ...
  const raisedIds = new Set((Array.isArray(raised) ? raised : []).filter(isPlainObject).map((e) => String(e.id || "").trim()).filter(Boolean));
  const candidates = [];
  FENCE_RE.lastIndex = 0;
  let m;
  ...
  for (const src of candidates.reverse()) {
    ...
    if (word && !PASS_WORDS.has(word) && !FAIL_WORDS.has(word)) {
      return { ok: false, passed: false, findings: [], prior: [], verdict: word, error: `unrecognized verdict '${word}'` };
    }
    ...
    const fresh = (findingsRaw || [])
      .map((f) => normalizeFinding(f, cycle, raisedIds))
    ...
    // findings is a verdict we cannot enforce AND must not launder into a
    // pass. The downgraded findings ride along as advisory (the fixer and the
    // ledger see them, the next reviewer may demonstrate one by id).
    ...
    if (requestsChanges && !blocking.length && undemonstrated.length) {
      return {
        ...
        error:
          ...
          ` (no evidence: no command or test named) — an undemonstrated blocking finding cannot be enforced, and a request for changes it cannot back is unreadable; fails closed${stillOpen.length ? " for the prior set" : ""}`,
      };
    }
  ...
  return { ok: false, passed: false, findings: [], prior: [], verdict: null, error: "no structured verdict found in the review report" };
}
...
function normalizeFinding(f, cycle, raisedIds = new Set()) {
  const severity = String(f.severity || "").trim().toLowerCase() || "unspecified";
  const evidence = String(f.evidence || f.reproduction || f.repro || f.demonstrated_by || "").trim();
  ...
  if (!evidence) {
    out.note = "rated blocking without evidence (no command or test demonstrated it) — recorded as advisory";
    return out;
  }
  if (cycle > 0 && !introduced && !(out.id && raisedIds.has(out.id))) {
    out.note = "rated blocking on a fix cycle but not introduced by the fix — a finding available at the first review does not move the goalposts; recorded as advisory";
    return out;
  }
...
function applyReviewToLedger(ledger, verdict, cycle) {
  ...
  for (const f of (verdict && verdict.findings) || []) {
    ...
    const existing = f.id ? byId.get(f.id) : null;
...
function withLedgerIds(findings, ledger, before) {
  ...
  const known = new Set((Array.isArray(before) ? before : list).map((e) => e.id));
  let i = 0;
  return (findings || []).map((f) => {
    // A reviewer-invented id that names nothing on the ledger is not a name
    // the fixer or the next review can use — the minted one replaces it.
    if (f.origin === "prior" || (f.id && known.has(f.id))) return f;
    const e = minted[i++];
