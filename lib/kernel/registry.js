...
const SLOTS = [
  {
    ...
    render: (s, { code }) => ({
      ...
      // dec-spor-status-inert-third-partition): the declared `status.inert`,
      // or — the inheritance default — the declared `status.terminal`.
      ...
      inert:
        s.payload.status && Array.isArray(s.payload.status.inert)
          ? s.payload.status.inert
          : s.payload.status && Array.isArray(s.payload.status.terminal)
            ? s.payload.status.terminal
            : [],
      inert_inherited: !(s.payload.status && Array.isArray(s.payload.status.inert)),
      hooks: hookNames(s),
      ...prov(s),
      ...codeOf(s, code),
    }),
...
class Registry {
  ...
  inertStatuses(type) {
    ...
    const st = s && s.payload.status;
    const inert = st && (Array.isArray(st.inert) ? st.inert : st.terminal);
    if (Array.isArray(inert)) for (const v of inert) if (typeof v === "string" && v) out.add(v.toLowerCase());
    return out;
  }
