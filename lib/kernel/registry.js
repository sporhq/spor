...
function parseSchemaNode(node) {
  ...
  // Derived from SLOTS (above) rather than hand-enumerated, so a sixth kind
  // is legal the moment it gets a slot entry — nowhere else to touch. Read
  // fresh on every call (not a module-load-time constant) so appending to
  // SLOTS at runtime is immediately honored.
  const kinds = new Set(SLOTS.map((s) => s.kind));
  if (!kinds.has(node.kind)) {
    errors.push(`kind '${node.kind}' must be one of: ${[...kinds].join(", ")}`);
  }
