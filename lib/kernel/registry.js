...
class Registry {
  ...
  // Whether this node type's `get()` hook attaches read-time `resolution`
  // enrichment (GRAPH.md; task-spor-schema-get-hook-readtime-enrichment) — the
  // seed task/issue/question/incident schemas today. A type that attaches it
  // is judged by a live resolving edge (`resolves`/`answers`); every other
  // type's completion is a STATUS the node's own schema reaches (a decision's
  // `settled`, a finding's `resolved`, a capture-pending's `merged`), never an
  // edge. This is the registry-driven replacement for what used to be a
  // hand-mirrored 4-type table in dispatch-terminal.js
  // (task-spor-dispatch-terminal-resolution-all-types) — an org that adds a
  // `get` hook to another type's resident schema override is honored with no
  // code change. Distinct from requiresCompletionResolver(type) above:
  // question attaches the hook but its `answered` completion needs no live
  // edge at WRITE time (any `answers` edge from any type already satisfies the
  // door), so the two booleans genuinely diverge for that one type.
