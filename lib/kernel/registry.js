...
// live entry), how to extract that `key` and validate the payload out of a
// parsed schema node (parseSchemaNode, below), and how to render one entry
// for the snapshot() introspection view. parseSchemaNode(), the Registry
// constructor, add(), staleOverrides(), and snapshot() all walk this table
// instead of hand-enumerating the five kinds separately — adding a sixth kind
// means adding one entry here, not touching five places.
...
// snapshot()'s per-slot assembly rule: a singleton slot (queue-policy)
// unwraps to its one entry or null; the rest sort in place by their declared
// sortKey (entries is always a fresh array from .map(), so no copy is
// needed), or pass through in insertion order when a slot declares none.
function renderSlotEntries(slot, entries) {
  if (slot.singleton) return entries[0] ?? null;
  if (!slot.sortKey) return entries;
  return entries.sort((a, b) => String(slot.sortKey(a)).localeCompare(String(slot.sortKey(b))));
}
...
// How a node type's COMPLETION is attested (issue-spor-offline-check-get-hook-
// resolution-proxy): `edge` — only a live resolving edge (`resolves`/`answers`)
// attests it, the task/issue/question/incident reading — or `status` — the
// node's own terminal status retires it, the decision/finding/capture-pending
// reading. Returns null when the schema declares nothing, which is the ONE
// case the legacy `get`-hook proxy below still answers.
function resolutionVerifiedBy(payload) {
  const r = payload && payload.resolution;
  const v = r && typeof r === "object" && !Array.isArray(r) ? r.verified_by : null;
  return v === "edge" || v === "status" ? v : null;
}

// Shared predicate the SLOTS validate() hooks below reuse for every
// "field must be an array of non-empty strings" check (status partitions,
// policy `governs`, edge-schema `aliases`) instead of each hand-copying it.
function isNonEmptyStringArray(v) {
  return Array.isArray(v) && v.every((x) => typeof x === "string" && x);
}

// A node-schema payload's `status` object is shared shape across
// node/edge-schema-adjacent validation: three optional string-arrays,
// `non_resolving` (resolver semantics — does this node retire others),
// `terminal` (own-lifecycle completion — read by work-analytics,
// issue-spor-analytics-completion-ignores-schema-terminal-status), and
// `inert` (queue-liveness-dead — the per-type overlay isTerminalStatus reads;
// a schema that declares none inherits its `terminal` set,
// dec-spor-status-inert-third-partition) — plus the completion-policy trio
// below.
//
// `vocabulary` / `completion` / `resolver_required`
// (task-spor-registry-declarative-terminal-status-policy) declare, as registry
// DATA, what previously only existed inside each seed schema's sandboxed
// `validate()`/`transitions()` source: the closed status enum the door
// enforces, the single SUCCESS terminal value ("the work landed" — not the
// whole `terminal` set, which also holds the give-up outcomes `abandoned`,
// `superseded`, `rejected`), and whether reaching it additionally requires a
// live resolving resolver (task-cc-terminal-status-requires-resolver).
// Enforcement stays procedural — the hooks are still the write door, and a
// declaration alone gates nothing. This trio exists so a READER (the
// gardener's finding remedies, an agent introspecting via `spor schema`) can
// know which terminal status to name for a type without hand-mirroring hook
// source; the two are pinned together by test/seed-declarative-status-policy.
// A type that declares no `vocabulary` accepts any status; one that declares
// no `completion` has no single mechanical "close it" value (a decision's
// settled/superseded/rejected are different outcomes, not one success).
function validateStatusPartition(status) {
  const errors = [];
  if (typeof status !== "object" || Array.isArray(status)) {
    errors.push(`status must be an object`);
    return errors;
  }
  for (const field of ["non_resolving", "terminal", "inert", "vocabulary"]) {
    const v = status[field];
    if (v != null && !isNonEmptyStringArray(v)) {
      errors.push(`status.${field} must be an array of non-empty strings`);
    }
  }
  if (status.completion != null) {
    if (typeof status.completion !== "string" || !status.completion) {
      errors.push(`status.completion must be a non-empty string`);
    } else if (status.vocabulary == null) {
      // A completion without the enum it belongs to would advertise a closing
      // move for a type whose door gates no status at all — the same
      // confidently-wrong reader this surface exists to prevent.
      errors.push(`status.completion requires a status.vocabulary it belongs to`);
    } else if (isNonEmptyStringArray(status.vocabulary) &&
        !status.vocabulary.some((v) => v.toLowerCase() === status.completion.toLowerCase())) {
      errors.push(`status.completion '${status.completion}' is not in status.vocabulary`);
    }
  }
  if (status.resolver_required != null) {
    if (typeof status.resolver_required !== "boolean") {
      errors.push(`status.resolver_required must be a boolean`);
    } else if (status.resolver_required && status.completion == null) {
      errors.push(`status.resolver_required requires a status.completion to gate`);
    }
  }
  return errors;
}

// A node-schema payload's `resolution` object: the DECLARED attestation path
// for this type's completion (issue-spor-offline-check-get-hook-resolution-
// proxy). One key, `verified_by`, whose only legal values are `edge` (a live
// resolving `resolves`/`answers` edge is the only attestation) and `status`
// (the node's own terminal status retires it). It is a closed enum on
// purpose: a typo silently reading as "undeclared" would drop the type back
// onto the `get`-hook proxy this declaration exists to retire.
function validateResolutionPolicy(resolution) {
  if (typeof resolution !== "object" || resolution === null || Array.isArray(resolution)) {
    return [`resolution must be an object`];
  }
  const v = resolution.verified_by;
  if (v == null) return [`resolution.verified_by is required — 'edge' or 'status'`];
  if (v !== "edge" && v !== "status") {
    return [`resolution.verified_by must be 'edge' or 'status', got '${v}'`];
  }
  return [];
}
...
const SLOTS = [
  {
    // No sortKey: snapshot() always re-sorts node_types after merging in the
    // native `schema` type (below), so sorting it here first would be wasted
    // work.
    kind: "node-schema", field: "nodeSchemas", singleton: false, snapshotKey: "node_types",
    key: (payload) => payload.node_type,
    // node-schema disposition (dec-spor-definition-of-done-org-policy): `key`
    // is the node_type this schema defines; `status` (optional) is the
    // resolving/terminal/inert partition the kernel reads off the registry
    // instead of a hardcoded table.
    validate: (payload) => {
      const errors = [];
      const key = payload.node_type;
      ...
      if (payload.status != null) errors.push(...validateStatusPartition(payload.status));
      if (payload.resolution != null) errors.push(...validateResolutionPolicy(payload.resolution));
      return errors;
    },
    render: (s, { code }) => ({
      type: s.key,
      ...
      // The declarative completion policy (task-spor-registry-declarative-
      // terminal-status-policy): the closed status enum the type's validate()
      // door enforces, the single success terminal value a mechanical "close
      // it" remedy should name, and whether that value additionally needs a
      // live resolving decision/artifact. `[]`/`null`/`false` mean undeclared:
      // any status, no single success value, no resolver gate.
      vocabulary:
        s.payload.status && Array.isArray(s.payload.status.vocabulary)
          ? s.payload.status.vocabulary
          : [],
      completion:
        s.payload.status && typeof s.payload.status.completion === "string"
          ? s.payload.status.completion
          : null,
      resolver_required: !!(s.payload.status && s.payload.status.resolver_required === true),
      // How this type's completion is ATTESTED (issue-spor-offline-check-get-
      // hook-resolution-proxy): `edge` (only a live resolving edge attests it)
      // or `status` (its own terminal status retires it). `null` = undeclared,
      // which is what tells a remote reader to fall back to the legacy
      // `hooks` proxy (see graph.js resolutionFactsFromSchemaSnapshot) rather
      // than guess a direction.
      resolution: resolutionVerifiedBy(s.payload),
      hooks: hookNames(s),
      ...prov(s),
      ...codeOf(s, code),
    }),
  },
  {
    ...
    sortKey: (e) => e.type,
    key: (payload) => payload.edge_type,
    // write-path normalization data (API.md §1): aliases rename in place,
    // inverse_label flips onto the target. Names must be edge-ish strings and
    // may not shadow this schema's own canonical type; cross-schema
    // collisions are caught at the Registry level (aliasCollisions()).
    validate: (payload) => {
      const errors = [];
      const key = payload.edge_type;
      ...
      if (payload.aliases != null) {
        if (!isNonEmptyStringArray(payload.aliases)) {
  ...
  },
  {
    ...
    sortKey: (e) => e.schema_id,
    key: (payload, node) => node.id,
    validate: (payload, node, code) => {
      const errors = [];
  ...
  },
  {
    ...
    sortKey: (e) => e.name,
    key: (payload) => payload.register,
    validate: (payload) => {
      const errors = [];
      const key = payload.register;
...
// Live lookup by kind — reads SLOTS fresh on every call (rather than a Map
// snapshotted once at module load) so appending a slot to SLOTS is the
// entire extension: nothing else needs to be told about a new entry.
function slotFor(kind) { return SLOTS.find((s) => s.kind === kind); }

// Backing-field accessor shared by add()/slotEntries(): lazily initializes a
// registry's field for a slot it hasn't seen yet, so a slot appended to the
// live SLOTS table after a Registry was already constructed (e.g. mid-test
// extensibility pokes) still gets a real Map/null instead of crashing on an
// undefined field — construction-time init (the constructor loop below)
// remains the normal path; this is only the fallback for that edge case.
function slotField(reg, slot) {
  if (reg[slot.field] === undefined) reg[slot.field] = slot.singleton ? null : new Map();
...
function slotEntries(reg, slot) {
  const e = slotField(reg, slot);
  if (slot.singleton) return e ? [[slot.kind, e]] : [];
  return [...e];
}
...
function parseSchemaNode(node) {
  ...
  // slotFor() reads SLOTS fresh on every call (not a module-load-time
  // constant), so appending to SLOTS at runtime is immediately honored — a
  // sixth kind is legal the moment it gets a slot entry, nowhere else to
  // touch. One lookup serves both the kind-validity check and the later
  // key()/validate() routing; the full kind list is only materialized (via
  // SLOTS.map) to render the "must be one of" error text when it's missing.
  const slot = slotFor(node.kind);
  if (!slot) {
    errors.push(`kind '${node.kind}' must be one of: ${SLOTS.map((s) => s.kind).join(", ")}`);
  }
  ...
  // Per-kind key extraction + payload validation routes through the same
  // SLOTS table the Registry constructor/add()/staleOverrides()/snapshot()
  // share, instead of a hand-written if/else chain over node.kind — a sixth
  // kind supplies its own key()/validate() there and needs no edit here.
  let key = null;
  if (payload && typeof payload === "object") {
    if (slot) {
      key = slot.key(payload, node);
      if (slot.validate) errors.push(...slot.validate(payload, node, code));
    }
...
class Registry {
  constructor() {
    // One backing field per SLOTS entry — a Map keyed by the schema's `key`
    // for most kinds, or a singleton field (starts null) for the one kind
    // that has at most one live entry (queue-policy). Adding a sixth kind
    // means adding one SLOTS entry, not a constructor edit.
    for (const slot of SLOTS) {
      this[slot.field] = slot.singleton ? null : new Map();
    }
  ...
  add(schema, source) {
    ...
    const slot = slotFor(schema.kind);
    slotField(this, slot); // ensure the field exists even for a slot appended post-construction
  ...
  // Without a type, unioned across every node-schema's `status.non_resolving` (type-blind, like
  ...
  // A supplied type selects only its own declaration, for give-up semantics.
  nonResolvingStatuses(type) {
    const out = new Set();
    const schemas = type == null ? this.nodeSchemas.values() : [this.nodeSchemas.get(type)].filter(Boolean);
  ...
  // The closed status vocabulary a node type's own `validate()` door enforces,
  // declared as registry data so a reader can know it without parsing hook
  // source. Empty Set = undeclared = this type accepts any status value (norm,
  // incident, workflow-run, …). Lowercased. Enforcement is still procedural —
  // the hook is the door; this is the advertisement of what that door does,
  // pinned to it by test/seed-declarative-status-policy.js.
  statusVocabulary(type) {
    const out = new Set();
    const s = this.nodeSchemas.get(type);
    const v = s && s.payload.status && s.payload.status.vocabulary;
    if (Array.isArray(v)) for (const x of v) if (typeof x === "string" && x) out.add(x.toLowerCase());
    return out;
  }

  // The single SUCCESS terminal status for a type — the value a mechanical
  // "the work is done, make the record honest" remedy should name (task
  // `done`, issue `resolved`, question `answered`). Null when the type declares
  // none: either it has no closed lifecycle at all, or its terminal values are
  // genuinely several distinct outcomes rather than one success (a decision is
  // settled OR superseded OR rejected; a capture-pending merged OR rejected),
  // so no single status can be recommended mechanically. Distinct from
  // terminalStatuses(type), which is the FULL set including the give-up
  // outcomes (`abandoned`). Lowercased.
  completionStatus(type) {
    const s = this.nodeSchemas.get(type);
    const c = s && s.payload.status && s.payload.status.completion;
    return typeof c === "string" && c ? c.toLowerCase() : null;
  }

  // Whether reaching completionStatus(type) additionally requires a live
  // resolving `decision`/`artifact` resolver on the graph — the
  // completion-resolver invariant each such type's transitions() gate enforces
  // at write time (task-cc-terminal-status-requires-resolver,
  // dec-spor-definition-of-done-org-policy). Declared as data so read surfaces
  // (the gardener's resolved-open / terminal-without-resolver checks) stop
  // hand-mirroring which types carry the gate.
  requiresCompletionResolver(type) {
    const s = this.nodeSchemas.get(type);
    return !!(s && s.payload.status && s.payload.status.resolver_required === true);
  }

  // The DECLARED attestation path for this type's completion — `edge`,
  // `status`, or null when the schema declares neither
  // (issue-spor-offline-check-get-hook-resolution-proxy). Every seed
  // node-schema declares one; null therefore means a resident schema written
  // before the key existed, which isEdgeVerified() below reads through the
  // legacy proxy.
  resolutionVerification(t) {
    const s = this.nodeSchemas.get(t);
    return s ? resolutionVerifiedBy(s.payload) : null;
  }

  // Is this type's completion attested by a live resolving edge
  // (`resolves`/`answers`) rather than by the node's own terminal status? The
  // registry-driven replacement for what used to be a hand-mirrored 4-type
  // table in dispatch-terminal.js (task-spor-dispatch-terminal-resolution-all-
  // types), and the one question every caller should ask — never
  // attachesGetHook() below, which answers a DIFFERENT one.
  //
  // The declaration decides it (issue-spor-offline-check-get-hook-resolution-
  // proxy). A schema that declares nothing falls back to the legacy proxy —
  // "does this type attach a `get()` hook" — which is what this check was
  // BEFORE the declaration existed, and is kept only so a graph-resident
  // schema authored against the old reading is honored unchanged. It is a
  // fallback, not the contract: `get()` is the general-purpose read-time
  // enrichment hook (a question pulling its answers, a task's held-note, an
  // execution hold), so the day a status-retired type adopts one for anything
  // else, the proxy silently reclassifies it as edge-verified and a run that
  // genuinely finished the work reads as not attested. Declare the key.
  //
  // Distinct from requiresCompletionResolver(type) above: question is
  // edge-verified but its `answered` completion needs no resolving
  // decision/artifact at WRITE time (any `answers` edge satisfies the door),
  // so the two booleans genuinely diverge for that one type.
  isEdgeVerified(t) {
    const declared = this.resolutionVerification(t);
    if (declared) return declared === "edge";
    return this.attachesGetHook(t);
  }

  // Whether this node type's schema attaches a `get()` read-time enrichment
  // hook at all (GRAPH.md; task-spor-schema-get-hook-readtime-enrichment).
  // A narrow, literal fact about the schema — NOT a resolution signal, which
  // is why it is no longer named for one: the old spelling
  // (`attachesResolutionHook`) read as an answer to the question above and
  // was used as one (issue-spor-offline-check-get-hook-resolution-proxy). The
  // only resolution caller left is isEdgeVerified()'s undeclared fallback
  // above; ask that one.
  attachesGetHook(t) {
    const s = this.nodeSchemas.get(t);
    return !!(s && s.code && Object.prototype.hasOwnProperty.call(s.code, "get"));
  }

  // DEPRECATED back-compat alias, kept because `lib/` is a PUBLISHED surface
  // that the server and any sibling checkout import by name (review finding
  // F2): removing a method outright turns a rename into a TypeError in a
  // consumer that pinned an older client. It resolves to `isEdgeVerified`,
  // not to `attachesGetHook`, because the question every caller of the old
  // name was asking is the resolution one — the old doc comment said so, and
  // reading the hook was the bug (issue-spor-offline-check-get-hook-
  // resolution-proxy). For the seed pack the two answers are identical (every
  // seed schema declares `resolution.verified_by` to match its hooks, pinned
  // by test/seed-declarative-status-policy.test.js); they diverge only for a
  // resident schema whose hooks and declaration disagree, where the
  // declaration is the answer the caller wanted. A consumer that genuinely
  // wants the literal hook fact should call `attachesGetHook`.
  attachesResolutionHook(t) {
    return this.isEdgeVerified(t);
  }
  ...
  snapshot({ code = false } = {}) {
    ...
    const nativeSchema = {
      ...
      // Undeclared, spelled the way the node_types contract documents it
      // (API.md `GET /v1/schema`): every entry carries `resolution`, and
      // `null` is the value that tells a reader to fall back rather than
      // guess. Omitting the key made the NATIVE entry the one row where the
      // documented shape did not hold, so a reader distinguishing "declared
      // status" from "undeclared" saw `undefined` here (review finding F3).
      // `schema` is core and never dispatched against, so this states the
      // shape, not a policy.
      resolution: null,
      hooks: [],
      schema_id: null,
    ...
    // Fold every slot into the final view generically, keyed by its
    // snapshotKey — the same SLOTS table add()/staleOverrides() walk, so a
    // sixth kind shows up here automatically with no hand-picked key.
    // renderSlotEntries() is the per-slot assembly rule (singleton unwrap /
    // sort / pass-through).
    const result = { default_edge_weight: DEFAULT_EDGE_WEIGHT };
    for (const slot of SLOTS) {
      const entries = slotEntries(this, slot).map(([, s]) => slot.render(s, { code }));
      result[slot.snapshotKey] = renderSlotEntries(slot, entries);
    }

    // node_types additionally carries the native `schema` type, which is core
    // rather than a graph-resident slot entry — merge it in and re-sort.
    result.node_types = [nativeSchema, ...result.node_types].sort((a, b) => a.type.localeCompare(b.type));

    result.stale_overrides = this.staleOverrides();
    result.alias_collisions = this.aliasCollisions();
    return result;
  }
}

// Status-only projection of GET /v1/schema. Reuse the registry's partition
// methods without reconstructing executable schema hooks or importing seeds
// that could silently replace a resident organization's policy.
function statusRegistryFromSnapshot(snapshot) {
  if (!snapshot || !Array.isArray(snapshot.node_types) || !Array.isArray(snapshot.registers)) return null;
  const registry = new Registry();
  const stringList = (v) => Array.isArray(v) && v.every((x) => typeof x === "string" && x.length > 0);
  for (const entry of snapshot.node_types) {
    if (!entry || typeof entry.type !== "string" || !entry.type || registry.nodeSchemas.has(entry.type)) return null;
    if (!stringList(entry.non_resolving) || !stringList(entry.terminal) || !stringList(entry.inert)) return null;
    registry.nodeSchemas.set(entry.type, { payload: { status: {
      non_resolving: [...entry.non_resolving], terminal: [...entry.terminal], inert: [...entry.inert],
    } } });
  }
  for (const entry of snapshot.registers) {
    if (!entry || typeof entry.name !== "string" || !Array.isArray(entry.classes)) return null;
    if (entry.classes.some((c) => !c || typeof c.id !== "string" || !c.id)) return null;
    if (registry.registers.has(entry.name)) return null;
    registry.registers.set(entry.name, { payload: { classes: entry.classes.map((c) => ({ id: c.id })) } });
  }
  return registry;
}

module.exports = {
  statusRegistryFromSnapshot,
  parseCalVer,
  compareCalVer,
  ...
  Registry,
  DEFAULT_EDGE_WEIGHT,
  // Exposed for the extensibility conformance test (a synthetic slot pushed
  // onto SLOTS must flow through parsing/validation/construction with no
  // other edit) — not part of the public API surface otherwise.
  SLOTS,
};
