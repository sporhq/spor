...
class Registry {
  ...
  snapshot({ code = false } = {}) {
    ...
    // sixth kind shows up here automatically with no hand-picked key. A
    // singleton slot (queue-policy) unwraps to its one entry or null; the
    // rest sort by their declared sortKey (undeclared -> insertion order).
    const result = { default_edge_weight: DEFAULT_EDGE_WEIGHT };
    for (const slot of SLOTS) {
      const entries = slotEntries(this, slot).map(([, s]) => slot.render(s, { code }));
      result[slot.snapshotKey] = slot.singleton
        ? entries[0] ?? null
        : slot.sortKey
          ? [...entries].sort((a, b) => String(slot.sortKey(a)).localeCompare(String(slot.sortKey(b))))
          : entries;
    }
