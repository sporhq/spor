...
// "mismatch" is the integration stage's §4.2 M1 refusal (the branch no longer
// carries the pinned candidate): a settled, non-boundary verdict, so no
// completion is ever derived from it. It is the STAGE-level word of
// STAGE_REFUSALS below spelled for the integration stage's own stamp; the two
// are read independently (nothing writes `impl_state: "mismatch"` today — the
// refusal is recorded on `integration_state`, `gate_state` and the escalation).
const INTEGRATION_STATES = Object.freeze(["running", "landed", "parked", "failed", "refused", "mismatch"]);
...
// The execution id (§7.1, EXECUTION-STATE.md §3.2): `exec-` + the first 16 hex
// of sha256 over the tenant (the org the hosted store partitions on, or the
// literal `local`), the item, the factory and the PIPELINE ATTEMPT — one per
// (item, factory, pipeline attempt), byte-identical to the id the server
// mints (kernel/execution.js is the shared derivation; this is its spelling
// for the claim's inputs). An earlier draft keyed it on the claim TIME, which
// no hosted execution could ever reproduce; the pipeline attempt is what a
// refused item's re-run advances, and a RESUMED pipeline never recomputes it
// (the record pins it in `impl_claim.execution_id`).
function executionKey({ tenant, nodeId, factoryId, pipelineAttempt }) {
  return execution.idKey([str(tenant) || "local", str(nodeId), str(factoryId), pipelineAttempt == null ? 1 : Number(pipelineAttempt)]);
}

function executionIdFor({ tenant, nodeId, factoryId, pipelineAttempt } = {}, sha256) {
  if (typeof sha256 !== "function") throw new TypeError("executionIdFor needs a sha256(string) => hex function");
  return execution.executionIdFor(
    { tenant: str(tenant) || "local", node_id: str(nodeId), factory: str(factoryId), pipeline_attempt: pipelineAttempt == null ? 1 : Number(pipelineAttempt) },
    sha256
  );
}
