...
// Reasons are collected verbatim (deduped) for the why-line and the render.
function deriveReadiness(graph, node, held, resolvedBy) {
  const human = [];
  if (Array.isArray(node.requires) && node.requires.includes("human")) human.push("requires human");
  ...
  for (const e of node.edges ?? []) {
    ...
    const t = graph.nodes[e.to];
    if (!t) continue;
    if (t.type === "person") human.push(`assigned to ${e.to}`);
    else if (t.type === "agent") assignedAgent = assignedAgent ?? e.to;
  }
  if (held) human.push("held task awaiting triage");
  if (node.type === "question") human.push("open question awaiting an answer");
  else if (node.type === "capture-pending") human.push("pending capture awaiting triage");
  // an open (live, unanswered) question in the 1-hop neighborhood — adj carries
  // both edge directions, so a question linked either way counts once.
  const seenQ = new Set();
  for (const e of graph.adj[node.id] ?? []) {
    const nb = graph.nodes[e.to];
    if (!nb || nb.id === node.id || nb.type !== "question" || seenQ.has(nb.id)) continue;
    if (!isLive(nb, graph.supersededBy) || resolvedBy[nb.id]) continue;
    seenQ.add(nb.id);
    human.push(`open question ${nb.id} in neighborhood`);
  }
  if (human.length) return { readiness: "human", reasons: human };
  const agent = [];
  ...
  if (assignedAgent) agent.push(`assigned to agent ${assignedAgent}`);
  if (agent.length) return { readiness: "agent", reasons: agent };
...
// `counts_by_readiness` ({agent, human, untriaged}, present only when there is
// readiness signal or a readiness filter). A graph with no readiness data yields
// no readiness fields, no clause, and no count — byte-identical to before.
...
function rankQueue(graph, opts = {}) {
  ...
  // Agent-readiness (dec-spor-agent-readiness-derived-classification): the
  // derived class + reasons per pushed item, computed in the scoring loop (it
  // needs the same `held` flag) and read in the render pass for the why-line and
  // the item fields. A side Map, not a per-item field, so an untriaged item's
  // serialized shape stays byte-identical. The tallies feed the envelope's
  // counts_by_readiness (schema-approval items are outside the classification, so
  // they are neither derived nor counted).
  ...
  for (const node of Object.values(graph.nodes)) {
    ...
    // classify before scoring so the readiness filter can skip a non-matching item
    // as a hard scope filter (uncounted, like project/type) and never pays for its
    // score/attached-code. `held` is available here; heldIds is only stamped for
    // items that survive the filter.
    const rd = deriveReadiness(graph, node, held, resolvedBy);
    if (readinessFilter && !readinessFilter.has(rd.readiness)) continue;
    if (held) heldIds.add(node.id);
    readinessById.set(node.id, rd);
    readinessCounts[rd.readiness]++;
  ...
  for (const item of top) {
    ...
    // work), it just carries the classification and its reasons. Emitted ONLY when
    // decisive (agent|human); an untriaged item adds no keys, so the item is
    // byte-identical when no readiness data exists (the needed_by/cold_neighbors
    // ride-along posture). The why-line clause is added via whyLine below.
    const rd = readinessById.get(item.id);
    if (rd && rd.readiness !== "untriaged") {
  ...
  // muted/blocked, which ride along only when > 0). It describes the returned
  // queue (post every filter), so it stays consistent with `count`.
