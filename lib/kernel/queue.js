...
function whyLine(signals, node, blockerDescs, crossBlocking) {
  const parts = [];
...
// rankQueue(graph, { project?, assignee?, activity?, front?, now?, limit?, viewer?, sandboxFor? })
//   -> { items, count, muted?, dormant?, archived? }
...
function rankQueue(graph, opts = {}) {
  const { project = null, activity = null, front = null, limit = DEFAULT_LIMIT, sandboxFor = null, viewer = null, assignee = null } = opts;
  ...
  // Grouping union (task-cc-project-grouping-union-reads): when the filter names
  // a `type: project` grouping (by its id, e.g. proj-spor), scope to the union
  // of repos `grouped-under` it; a repo slug (e.g. spor) still scopes to that
  // one repo. `scope` is the member-repo Set, or null for the single-repo case.
  const scope = project ? (graph.groupingRepos?.[projectKey] ?? null) : null;
  const inProject = (n) => {
    if (!project) return true;
    const nr = rp(n.project);
    return scope ? scope.has(nr) : nr === projectKey;
  };
  ...
  const isArchived = (n) => archived.size > 0 && rp(n.project) !== projectKey && archived.has(rp(n.project));
  ...
  for (const node of Object.values(graph.nodes)) {
    ...
    const item = {
      ...
      why: whyLine(signals, node, blockerDescs, crossBlocking),
    };
