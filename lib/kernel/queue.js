...
//               same shape staleness uses. signals.front stays the raw count for
//               the queue-policy seam. The why-line states the actual window
//               (`frontDays`), not a fixed "this week".
...
// linked by any edge OTHER than resolves/answers? That is the structural mark of
// "work was recorded against this task, but nothing resolved it." Combined at the
...
// non-resolving artifact, which raises front, which re-surfaces the task, which
// triggers the next identical pass. `resolves`/`answers` inbound edges are
// excluded — a live resolving edge would already have retired the node, and a
// pending (in-review, non-resolving-status) resolver is a resolution in flight,
// not a held outcome (it keeps the task live by design, dec-spor-definition-of-
// done-org-policy). Superseded outcome nodes don't count — a withdrawn artifact
// records nothing. adj carries both directions (graph.js); the " (inbound)"
// suffix marks the reverse mirror, and `e.to` on a mirror is the edge's SOURCE.
function hasInboundOutcome(graph, id) {
  for (const e of graph.adj[id] ?? []) {
    ...
    if (src.type === "artifact" || src.type === "decision") return true;
  }
  return false;
}
...
// rankQueue(graph, { project?, assignee?, includeTypes?, excludeTypes?, activity?, front?, leases?, now?, limit?, viewer?, sandboxFor?, timestamps? })
//   -> { items, count, muted?, dormant?, archived?, leased?, reserved?, blocked? }
...
// surfaced (signal + why-line), not yet a score weight — a ranking contribution
// is a deliberate tuning change deferred to its own task.
...
function rankQueue(graph, opts = {}) {
  ...
  for (const node of Object.values(graph.nodes)) {
    ...
    // signals shape and goldens are byte-identical.
    if (timestamps) {
    ...
    // suggestion), and the render pass flips its suggestion do→triage. front>0 is
    // required (there is a runaway only when front is boosting it); no live
    // resolving edge is guaranteed (resolvedBy retired those at the top of the
    // loop); no live blocker holds in the firehose (blocked items continue'd
    // above) and is re-checked here for the steward view. signals.front keeps the
    // raw count so a queue-policy rank() can still re-weight (the seam
    // dec-cc-queue-front-from-attribution promised).
    const st = (node.status || "").toLowerCase();
    const held = signals.front > 0 && node.type === "task" && (st === "" || st === "open")
