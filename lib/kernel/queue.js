...
//               gone (excluding the node's own outbound `supersedes` edges,
//               whose target is retired by definition — not rot). High
//               staleness suggests CLOSING, not doing (§8), so it flips the
//               item's suggestion rather than boosting its score.
...
//
// A node under an OPEN execution hold (resolution.js executionHeld — a
// factory worker's `execution:` stamp, FACTORY-IMPLEMENTATION-STAGE.md §4.5)
// is live WHATEVER its status says: this is the STATUS half of the hold, the
// twin of resolutionMap's edge half. An implementer that hand-flips `done`
// under the hold has retired nothing — every `blocks` traversal, liveBlockers,
// deriveReadiness, rankQueue and the program view go through here, so its
// dependents stay blocked until the completion write clears the key. The one
// exception is a GIVE-UP status (abandoned/rejected — resolution.isGiveUpStatus):
// a person's `set_status abandoned` is the door out of an execution, and a
// hold must not keep dropped work in the queue.
function isLive(node, supersededBy, graph) {
  if (supersededBy[node.id]) return false;
  if (resolution.executionHeld(node)) return !resolution.isGiveUpStatus(node.status, node.type, graph);
...
//
// A node's own outbound `supersedes` edges are excluded from both the stale
// count and the denominator: by definition their target is retired, so
// counting them as rot would permanently inflate a consolidator's own
// staleness for doing exactly what a `supersedes` edge is for
// (issue-spor-queue-staleness-supersedes-false-positive).
function stalenessScore(graph, node, memo) {
  if (memo) { const c = memo.get(node.id); if (c !== undefined) return c; }
  const edges = (node.edges ?? []).filter((e) => e.type !== "supersedes");
  let v;
  if (!edges.length) v = 0;
  else {
    let stale = 0;
...
function rankQueue(graph, opts = {}) {
  ...
  for (const node of Object.values(graph.nodes)) {
    ...
    const item = {
      ...
      signals,
      // The item's own `profile:` frontmatter (task-spor-test-change-lane-
      // auto-routing) — set by the gate pipeline's test-change lane item, but
      // not exclusive to it: any node authored with a `profile:` pin surfaces
      // it here. Omitted (not null) when absent, so a graph with no `profile:`
      // frontmatter anywhere is byte-identical (norm-cc-byte-identical-refactor).
      // `spor work`'s candidate dispatch reads this to auto-route without a
      // manual `--profile`; the queue itself does no routing.
  ...
  for (const item of top) {
    ...
    // Every agent currently named by a live `assigned -> agent` edge
    // (task-spor-fleet-autoroute-auto-tier-consumer's client consumer,
    // issue-spor-auto-route-additive-assignment-two-assignees): unlike
    // deriveReadiness's own `assignedAgent` (first-found, for the human/agent
    // bucket only), this keeps EVERY one, because the additive auto-route bug
    // this exists to let a client detect is exactly a node carrying two of
    // them at once — the refusing box's stale edge alongside the freshly
    // routed target's. A pull worker's candidate filter reads this to skip an
    // item some OTHER agent already holds (dispatchableQueuePage's
    // `eligible`/classifyWorkItem, bin/spor.js), matching what the queue's
    // `assignee=me` view already does for a person. Omitted when empty, so a
    // node with no agent assignment is byte-identical.
    const assignedAgents = [];
    for (const e of node.edges ?? []) {
      if (e.type !== "assigned") continue;
      const t = graph.nodes[e.to];
      if (t && t.type === "agent" && !assignedAgents.includes(e.to)) assignedAgents.push(e.to);
    }
    if (assignedAgents.length) item.assigned_agents = assignedAgents;
...
module.exports = {
  ...
  // Exposed so server/dispatch-guard.js (spor-server) reuses this exact
  // parse instead of hand-duplicating it (task-spor-dispatch-guard-requires-list-consolidation).
  requiresList,
};
