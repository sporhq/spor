...
const { isTerminalStatus, terminalStatuses } = require("./resolution.js");

const SLUG_RE = /^[a-z0-9][a-z0-9-]*$/;
const arr = (v) => (Array.isArray(v) ? v : []);

// Statuses that retire a coupling norm from matching (norms rarely carry a
// status at all; an absent status is live). Shared with lib/kernel/
// resolution.js's type-blind vocabulary (issue-spor-coupling-resolution-
// terminal-status-divergence) rather than a second hardcoded list — this
// module scans node files in the hook tool loop without a loaded
// graph/registry (see header), so it reads the FALLBACK vocabulary
// (isTerminalStatus with no graph arg — the type is always "norm", coupling
// norms being the only nodes this module matches), the same one the seed
// terminal-status register reproduces byte-identically.
const isCouplingTerminal = (status) => isTerminalStatus(status, "norm");

// Back-compat static export (issue-spor-terminal-status-validation-
// regressions): this module used to export a hardcoded `TERMINAL` Set (see
// e1b3d48, which replaced it with the isCouplingTerminal() function above).
// A caller that destructured `{ TERMINAL }` off this module — downstream
// server or plugin code that predates the unification — silently got
// `undefined` after that rename and would throw the moment it tried to read
// it (`TERMINAL.has(...)`). Restore the shape as a plain Set computed from
// the SAME fallback vocabulary isCouplingTerminal reads (no case a caller
// could observe: a status is a member of TERMINAL iff isCouplingTerminal(status)
// is true), so this is a compatibility alias, never a second source of truth.
...
function isCouplingNorm(n) {
  ...
  if (n.status && isCouplingTerminal(n.status)) return false;
...
// Declared alias expansion (issue-spor-coupling-matcher-reverse-symlink-gap):
// repoRelativeCandidates only derives a symlink alias spelling when the
// INCOMING path still spells it — it can't discover an alias starting from an
// already-resolved canonical path, and a runtime REVERSE lookup (which alias
// points at this canonical path?) needs a filesystem-wide symlink scan, ruled
// out as too expensive for the edit-time hot path
// (dec-spor-dismiss-reverse-symlink-path-lookup). The settled fix is a
// config-declared map instead — `coupling.aliases` in `.spor.json`, `{
// "<alias prefix>": "<canonical prefix>" }` (both repo-root-relative, e.g.
// `{ "frontend": "packages/web" }`) — expanded in BOTH directions here at
// zero runtime cost (no scanning). A path under either side of a declared
// entry also produces the spelling under the other side; anything not
// declared is unaffected, so a graph/config with no `coupling.aliases` is
// byte-identical (norm-cc-byte-identical-refactor). Declaring nothing is the
// default posture and keeps the pre-existing limitation: a glob authored
// against an undeclared alias only matches an edit reported under that SAME
// spelling (GRAPH.md documents this).
function expandAliasCandidates(paths, aliases) {
  const entries = aliases ? Object.entries(aliases).filter(([a, c]) => a && c && a !== c) : [];
  if (!entries.length) return paths;
  const out = new Set(paths);
  for (const p of paths) {
    for (const [alias, canon] of entries) {
      for (const [from, to] of [[alias, canon], [canon, alias]]) {
...
// `aliases`, when passed, additionally expands every candidate through the
// declared alias map above (issue-spor-coupling-matcher-reverse-symlink-gap).
function couplingHit(norm, { slug, relPath, repoTags, aliases }) {
  const rawPaths = Array.isArray(relPath) ? relPath : [relPath];
  const paths = expandAliasCandidates(rawPaths, aliases);
  const matches = (glob) => {
...
module.exports = {
  parseEntry, globToRegExp, isCouplingNorm, scopeOk, couplingHit, matchCouplings, isCouplingTerminal, TERMINAL,
