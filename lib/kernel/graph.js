...
// scope); couples_when/couples_also are a coupling norm's trigger/target file
// globs, optionally repo-qualified `<slug>:<glob>` (kernel/coupling.js,
// dec-spor-coupling-norms-declared-first); skills/plugins/mcp are the orchestration-layer profile node's
...
function parseFrontmatter(raw, file) {
  ...
  for (const line of fm.split("\n")) {
    const list = line.match(/^(pin|exclude|queue_mute|commits|slugs|fingerprints|roles|tags|applies_to_tags|applies_to_repos|applies_to_projects|couples_when|couples_also|skills|plugins|mcp|requires):\s*\[([^\]]*)\]/);
