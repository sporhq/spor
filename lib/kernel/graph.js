...
// and "- {type: X, to: Y}" edges (which may carry extra flat attributes —
// "- {type: assigned, to: agent-X, profile: profile-Y}" — preserved on the
// edge object).
function parseFrontmatter(raw, file) {
  ...
  for (const line of fm.split("\n")) {
    ...
    if (list) { node[list[1]] = list[2].split(",").map((s) => s.trim()).filter(Boolean); lastKey = null; blockListKey = null; continue; }
    const kv = line.match(/^(\w[\w-]*):\s*(.*)$/);
    if (kv) {
      if (kv[1] === "edges") { lastKey = null; blockListKey = null; }
