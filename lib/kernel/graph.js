...
function parseFrontmatter(raw, file) {
  ...
  const flushEdgeBuf = () => {
    ...
    const eb = edgeBuf;
    edgeBuf = null;
    if (!eb.type || !eb.to) throw new Error(`unparseable edge entry ${JSON.stringify(eb)} in ${file}`);
    node.edges.push(eb);
  };
  ...
  for (const line of fm.split("\n")) {
    ...
    if (inEdgesBlock) {
      ...
      throw new Error(`unparseable edge entry "${line.trim()}" in ${file}`);
    }
    if (blockListKey) {
...
function buildGraph(files, { nodesDir = "", seedSchemas = [] } = {}) {
  ...
  for (const f of Object.keys(files).filter((f) => f.endsWith(".md"))) {
    ...
    try {
      ...
      skipped.push({ file: f, error: e && e.message ? e.message : String(e) });
      continue;
    }
    nodes[n.id] = n;
  }
...
function validateGraphFiles(files, seedSchemas = [], opts = {}) {
  ...
  for (const f of Object.keys(files).filter((f) => f.endsWith(".md"))) {
    ...
    // reject loudly instead becomes a warning here, not a thrown error
    // (task-spor-validate-linter-block-form-edges).
    ...
    const flushEdgeBuf = () => {
      ...
      edgeBuf = null;
      if (!eb.type || !eb.to) {
        warnings.push(`${f}: unparseable edge entry ${JSON.stringify(eb)} — the write path would reject this`);
        return;
      }
      node.edges.push(eb);
    };
    for (const line of m[1].split("\n")) {
      ...
      if (inEdgesBlock) {
        ...
        if (line.trim() === "" || line.trim().startsWith("#")) continue;
        warnings.push(`${f}: unparseable edge entry "${line.trim()}" — the write path would reject this`);
      }
    }
    flushEdgeBuf();
    for (const field of ["id", "type", "title", "summary"]) {
