...
//   buildGraph(files, {nodesDir, seedSchemas})   -> { nodes, adj, supersededBy, docs, df, N, nodesDir, skipped, registry }
...
// The server's per-node body-size cap (API.md §1: "body ≤ 8KB"). validateGraphFiles
// mirrors it as an early warning — issue-cc-accreting-log-body-cap-collision: an
// accreting running-log artifact that only discovers the cap on a rejected remote
// write has nowhere left to grow. Warn well before the wall so there's still room
// to roll over into a continuation node (GRAPH.md "Accreting running-log artifacts").
//
// The two bands are NOT the same kind of finding, so only one of them is on by
// default:
//   - OVER the cap is a defect on any node. The server re-runs validateIncoming
//     on every write, so an over-cap resident node is effectively immutable —
//     even a status flip 422s. Always warn.
//   - NEAR the cap (>= ratio) is advice, and only for a node still being
//     appended to. On a finished decision/artifact it is permanent, unresolvable
//     noise of exactly the kind opts.rootId exists to avoid below — and since
//     dec-spor-gardener-runs-graph-wide-lint the server gardener files ONE queue
//     finding per lint warning, so shipping it unconditionally would park a
//     never-clearable finding on every large terminal node in the graph. It is
//     therefore opt-in (opts.nearBodyCap), requested by `spor validate` — the
//     periodic, human-run pass GRAPH.md points a running-log's owner at.
const BODY_CAP_BYTES = 8192;
const BODY_CAP_WARN_RATIO = 0.9;
...
// The fixed allowlist of keys carrying a flat list of strings, shared between
// the inline-array form (below) and the YAML block-list form (a bare
// "key:" line followed by indented "- item" lines) — the same allowlist, two
// spellings of the same value, so both stay in lockstep with one edit.
// commits entries are repo-qualified shas ("repo@sha", task-cc-commit-
// linking); slugs/fingerprints are the project node's alias and repo-evidence
// registers (task-cc-project-identity-nodes); roles is the person node's
// role-list register the org-defined policy layer's quorum gate counts
// qualified approvals against (task-cc-policy-layer); tags is the repo node's
// free-form label register, and applies_to_* are a norm's repo/tag-scope
// selectors matched against it (task-cc-norm-ride-along-repo-tag-scope);
// couples_when/couples_also are a coupling norm's trigger/target file globs,
// optionally repo-qualified `<slug>:<glob>` (kernel/coupling.js, dec-spor-
// coupling-norms-declared-first); skills/plugins/mcp are the orchestration-
// layer profile node's runtime+capability bundle and requires is a work
// node's risk-class list (the registry-declared `requires` register,
// dec-spor-orchestration-routine-requires-threads) — all flat strings, not
// objects, by parser design.
...
// Supports simple key: value, YAML folded multi-line values (indented
// continuations), LIST_FIELDS as either an inline array (`commits:
// [repo@sha]`) or a YAML block list (`commits:` on its own line followed by
// indented `- repo@sha` lines — issue-spor-cli-put-node-block-list-
// frontmatter: the block form used to fall through to folded-scalar handling,
// silently flattening a one-element list into a string that downstream `for
// (const c of node.commits)` readers then iterated character-by-character),
// and edges as either flow form "- {type: X, to: Y}" (which may carry extra
// flat attributes — "- {type: assigned, to: agent-X, profile: profile-Y}" —
// preserved on the edge object) or YAML block form ("edges:" on its own line
// followed by "- type: X" / indented "to: Y" lines). Either form accepts
// A DELIBERATE parse fault -- the two shapes parseFrontmatter refuses (no
// frontmatter, an edge entry that resolves to no type+to). Tagged so
// buildGraph's per-node isolation can tell "this file is malformed" (skip it,
// dec-spor-buildgraph-per-node-fault-isolation) from "this parser is broken"
// (a TypeError from a future regex branch), which must still crash loudly
// rather than silently emptying every node out of the graph.
const parseFault = (message) => Object.assign(new Error(message), { sporParse: true });

// "target:" as an alias for "to:" (issue-spor-graph-frontmatter-edge-silent-
// drop: block-form edges and the "target:" key used to be silently dropped —
// no adjacency, no error, no warning. Settled scope: accept and normalize,
// never silently drop — an edge entry that still can't be understood fails
// the write loudly instead, naming the offending entry).
function parseFrontmatter(raw, file) {
  raw = String(raw).replace(/\r\n/g, "\n");
  const m = raw.match(/^---\n([\s\S]*?)\n---\n?([\s\S]*)$/);
  if (!m) throw parseFault(`no frontmatter in ${file}`);
  const [, fm, body] = m;
  const node = { edges: [], pin: [], exclude: [], body: body.trim(), file };
  let lastKey = null; // for YAML folded scalars (indented continuation lines)
  let blockListKey = null; // set while `lastKey` is a LIST_FIELD awaiting "- item" lines
  let inEdgesBlock = false; // set while under a block-form "edges:" key
  let edgeBuf = null; // an edge entry under construction across block-form "- type: X" / "to: Y" lines

  // Close out a pending block-form edge entry: `type` + (`to`/`target`,
  // normalized to `to`) are both required, or the entry is unparseable and the
  // whole write fails loudly instead of silently losing the edge.
  const flushEdgeBuf = () => {
    if (!edgeBuf) return;
    const eb = edgeBuf;
    edgeBuf = null;
    if (!eb.type || !eb.to) throw parseFault(`unparseable edge entry ${JSON.stringify(eb)} in ${file}`);
    node.edges.push(eb);
  };

  for (const line of fm.split("\n")) {
    const list = line.match(LIST_FIELD_RE);
    if (list) { flushEdgeBuf(); inEdgesBlock = false; node[list[1]] = list[2].split(",").map((s) => s.trim()).filter(Boolean); lastKey = null; blockListKey = null; continue; }
    const kv = line.match(/^(\w[\w-]*):\s*(.*)$/);
    if (kv) {
      if (kv[1] === "edges") { flushEdgeBuf(); inEdgesBlock = true; lastKey = null; blockListKey = null; }
      else if (LIST_FIELD_SET.has(kv[1]) && kv[2].trim() === "") {
        flushEdgeBuf(); inEdgesBlock = false;
        node[kv[1]] = [];
        lastKey = kv[1];
        blockListKey = kv[1];
      } else {
        flushEdgeBuf(); inEdgesBlock = false;
        node[kv[1]] = kv[2].replace(/^["']|["']$/g, "");
        lastKey = kv[1];
        blockListKey = null;
      }
      continue;
    }
    // Edges are "- {type: X, to: Y}" (or "target:" as an alias for "to:") and
    // may carry trailing flat attributes ("- {type: assigned, to: agent-X,
    // profile: profile-Y}", the per-assignment profile override,
    // dec-spor-orchestration-routine-requires-threads thread 3). The first two
    // captures stay type/to (byte-identical for the "to:" no-attribute form);
    // any "key: value" pairs after that are folded onto the edge object.
    const edge = line.match(/-\s*\{type:\s*([\w-]+),\s*(?:to|target):\s*([\w-]+)((?:\s*,\s*[\w-]+:\s*[\w-]+)*)\}/);
    if (edge) {
      flushEdgeBuf();
    ...
    if (inEdgesBlock) {
      ...
      // opens an entry, and subsequent indented "key: value" lines fold onto
      // it until the next list item or a new top-level key closes it. A line
      // that opens neither a new item nor continues one is unparseable.
      ...
      const cont2 = edgeBuf && line.match(/^\s+(\w[\w-]*):\s*(.*)$/);
      if (cont2) {
        edgeBuf[cont2[1] === "target" ? "to" : cont2[1]] = cont2[2].trim().replace(/^["']|["']$/g, "");
        continue;
      }
      if (line.trim() === "" || line.trim().startsWith("#")) continue;
      throw parseFault(`unparseable edge entry "${line.trim()}" in ${file}`);
    }
    if (blockListKey) {
      // A block list is open: `- item` lines may sit flush-left (valid YAML,
      // same indentation as the key) or indented, so leading whitespace is
      // optional. ANY other line while the list is open is ignored outright
      // — falling through to the folded-scalar branch below would `+=` onto
      // an array, coercing it (via Array.prototype.toString) back into the
      // exact corrupted string this parser exists to prevent.
      const item = line.match(/^\s*-\s+(\S.*)$/);
      if (item) node[blockListKey].push(item[1].trim().replace(/^["']|["']$/g, ""));
      continue;
    }
...
function buildGraph(files, { nodesDir = "", seedSchemas = [] } = {}) {
  ...
  // One malformed node file must not take the whole graph down: availability
  // wins for the live server, where a single corrupt node would otherwise
  // refuse the boot of a 4k-node team graph. So each file parses inside its
  // own try/catch and a fault SKIPS that node, naming it in `skipped` --
  // the loud half of the trade. The kernel is pure (no side effects), so it
  // reports rather than warns; lib/graph.js's loadGraph is the seam that
  // turns each entry into a named stderr warning, and the server reads the
  // same array. A graph with no malformed node yields an empty `skipped` and
  // is byte-identical to the pre-isolation build.
  //
  // Only a DELIBERATE parse fault (`sporParse`) is isolated. Any other throw
  // is a bug in the parser itself, and swallowing it would skip EVERY file --
  // an empty-but-valid graph, a digest of "nothing", and fail-open hooks that
  // exit 0 -- so it is rethrown and crashes as loudly as it used to.
  //
  // A skipped file is INVISIBLE, not degraded: it contributes no node, no
  // edges, no doc vector, and -- if it happened to be a `type: schema` node
  // -- no registry override either. That last case is the sharp edge of the
  // trade (the graph loads under the seed ontology instead of the resident
  // one), which is exactly why the warning names the file.
  const skipped = [];
  for (const f of Object.keys(files).filter((f) => f.endsWith(".md"))) {
    let n;
    try {
      n = parseFrontmatter(files[f], f);
      if (!n.id) throw parseFault(`missing or empty id in ${f}`);
    } catch (e) {
      if (!e || !e.sporParse) throw e; // a parser BUG, not a malformed file — crash, don't empty the graph
  ...
  // rejected schema nodes are inert data (§2.4 proposal flow). A schema node
  // that PARSED but carries an invalid schema payload is still fatal -- the
  // registry is the contract (norm-cc-registry-is-contract) and a half-built
  // one would mistype every node in the graph, so only per-file PARSE faults
  // are isolated above.
  ...
  const danglingTo = {};
  // pendingSupersedes (issue-cc-graph-supersededby-non-traversable): EVERY
  // "supersedes" edge onto a not-yet-existent target, ANY source
  // traversability, in encounter order. danglingTo itself stays traversable-
  // source-only (unchanged, so its existing markers/consumers/adjacency role
  // are untouched) — a non-traversable source never gets a danglingTo entry,
  // so without a separate index the incremental updater would have nowhere
  // to remember such a pending edge, and supersededBy would never activate
  // once the target is later created. Traversable sources are ALSO recorded
  // here (duplicating their danglingTo entry) so that when two dangling
  // sources of DIFFERENT traversability target the same missing node, the
  // later-encountered one still wins supersededBy on activation — exactly as
  // buildGraph's single ordered pass would decide once the target exists,
  // rather than "traversable sources always win" or vice versa. Keyed by the
  // missing target id -> source ids, in encounter order.
  const pendingSupersedes = {};
  for (const n of Object.values(nodes)) {
    // supersededBy indexes EVERY node's supersedes edges, source and target
    // traversability both irrelevant (issue-cc-graph-supersededby-non-
    // traversable) — a briefing superseding a briefing must be indexed the
    // same as a decision superseding a decision. The only requirement is that
    // the target exists; a dangling supersedes edge has nothing to mark yet.
    // adj/danglingTo stay gated on traversability exactly as before, so the
    // briefing/correction walk is untouched.
    for (const e of n.edges) {
      if (e.type !== "supersedes") continue;
      if (nodes[e.to]) { supersededBy[e.to] = n.id; continue; }
      (pendingSupersedes[e.to] ??= []).push(n.id);
    }
  ...
  return { nodes, adj, supersededBy, docs, df, N, postings, docIndex, danglingTo, pendingSupersedes, nodesDir,
    skipped, registry: reg, projectAliases: projectAliasMap(nodes), archivedProjects: archivedProjectSet(nodes),
...
function applyNode(graph, text, file) {
  ...
  // A write that repairs a previously-skipped file must retire its skip record
  // too (dec-spor-buildgraph-per-node-fault-isolation), or the graph keeps
  // reporting phantom corruption against a node that is now present — the same
  // "structurally identical to a rebuild" contract the maps above keep. The
  // file name is matched both as given and by the id==filename convention,
  // since a skipped file never parsed and so was recorded by name alone.
  if (graph.skipped && graph.skipped.length) {
    graph.skipped = graph.skipped.filter((s) => s.file !== file && s.file !== `${id}.md`);
  }
  ...
  // supersededBy maintenance runs regardless of traversability (issue-cc-graph-
  // supersededby-non-traversable) — removeNodeStructure/addNodeStructure/
  // activateDangling each self-gate their OWN adjacency/danglingTo portions on
  // traversability internally, matching buildGraph exactly.
  if (old) removeNodeStructure(graph, old, reg);
  graph.nodes[id] = node;
  addNodeStructure(graph, node, reg);
  activateDangling(graph, id, reg); // edges that were dangling onto this id light up (supersededBy always; adjacency only if id is traversable)
...
// Remove a node's OWN forward edges: the supersession entries they set are
// cleared regardless of traversability (issue-cc-graph-supersededby-non-
// traversable — supersededBy indexes every node), including a pending
// registration in pendingSupersedes when its supersedes edge targeted a
// still-missing node (ANY source traversability — see pendingSupersedes'
// buildGraph comment for why traversable sources are tracked here too). The
// adjacency removal, inbound mirrors, and danglingTo registrations stay
// traversable-only, exactly as buildGraph builds them. Inbound entries OTHER
// nodes placed on this node are owned by those nodes and left untouched.
function removeNodeStructure(graph, n, reg) {
  const id = n.id;
  const trav = reg.isTraversable(n.type);
  for (const e of n.edges) {
    const t = graph.nodes[e.to];
    if (e.type === "supersedes") {
      if (graph.supersededBy[e.to] === id) delete graph.supersededBy[e.to];
      if (!t) dropPendingSupersede(graph, e.to, id);
    }
    if (!trav) continue;
...
// Add a node's forward edges: supersession is set for every "supersedes" edge
// whose target exists, regardless of traversability; when the target is
// still missing, the edge is remembered in pendingSupersedes REGARDLESS of
// this node's own traversability (danglingTo stays traversable-source-only,
// unchanged) so it can activate — in true encounter order — once the target
// is created. The adjacency (with inbound mirrors) and danglingTo
// registrations stay traversable-only — the exact rules buildGraph's
// adjacency loop applies.
function addNodeStructure(graph, n, reg) {
  ...
  for (const e of n.edges) {
    const t = graph.nodes[e.to];
    if (e.type === "supersedes") {
      if (t) graph.supersededBy[e.to] = id;
      else (graph.pendingSupersedes[e.to] ??= []).push(id);
    }
    if (!trav) continue;
    const w = reg.edgeWeight(e.type);
...
// A node `id` was just created. Two independent activations:
//   - danglingTo: traversable-source edges onto `id` light up adjacency, both
//     directions, exactly as buildGraph would build it from scratch (source
//     rechecked live/traversable — it may have vanished or changed since
//     registration).
//   - pendingSupersedes: EVERY pending "supersedes" edge onto `id` (any
//     source traversability) resolves supersededBy, applied strictly in
//     encounter order so the last-registered source wins — matching
//     buildGraph's single ordered pass over Object.values(nodes) regardless
//     of which of the two sources is traversable. Splitting this out of the
//     danglingTo loop above is deliberate: draining danglingTo then
//     pendingSupersedes as two separate passes would let whichever bucket a
//     source landed in (by traversability) decide the winner instead of true
//     chronological order.
function activateDangling(graph, id, reg) {
  const pend = graph.danglingTo[id];
  if (pend) {
    const targetTrav = reg.isTraversable(graph.nodes[id].type);
    if (targetTrav) {
      for (const d of pend) {
  ...
  const pendingSup = graph.pendingSupersedes[id];
  if (pendingSup) {
    for (const from of pendingSup) if (graph.nodes[from]) graph.supersededBy[id] = from; // source vanished otherwise
    delete graph.pendingSupersedes[id];
  }
}
...
function dropPendingSupersede(graph, to, from) {
  const list = graph.pendingSupersedes[to];
  if (!list) return;
  const i = list.indexOf(from);
  if (i >= 0) list.splice(i, 1);
  if (list.length === 0) delete graph.pendingSupersedes[to];
}
...
const SCOPE_IDENTITY_TYPES = new Set(["repo", "project"]);

// Does the token name a repo/grouping identity scopeFor actually resolves — a
// `type: repo`/`type: project` node id, a slug in the alias map, or a
// member/own key of some grouping (issue-spor-kernel-project-known-accepts-
// arbitrary-nodes)? Mirrors the server's `server/project-scope.js`
// `isScopeIdentity`, which the client's own `projectKnown` used to diverge
// from: an ordinary task/issue/decision id is NOT a scope identity even
// though it is an existing node, since scopeFor only pins repo/project nodes
// and falls back to the token itself for everything else. Own-key lookups
// only — `graph.nodes`, `graph.projectAliases` and `graph.groupingRepos` are
// plain objects, so a bracket read of a prototype key (`constructor`,
// `toString`, `__proto__`, …) would otherwise hand back Object.prototype's
// member and report a valid-but-unmatched token as a registered identity.
function isScopeIdentity(graph, param) {
  const node = own(graph.nodes, param);
  if (node && SCOPE_IDENTITY_TYPES.has(node.type)) return true;
  if (own(graph.projectAliases, param)) return true;
  const gr = graph.groupingRepos || {};
  if (own(gr, param)) return true;
  for (const set of Object.values(gr)) if (set?.has?.(param)) return true;
  return false;
}
...
// is a `type: repo` or `type: project` grouping node id — what scopeFor pins
// or unions — a repo slug in the alias map, or a member/own key of some
// grouping. An UNKNOWN token is exactly scopeFor's fall-back-to-itself case
// (`new Set([param])` against a non-existent repo id) — it yields count:0 with
// no warning. This reuses the existing resolvers (projectAliases / grouping
// membership) rather than re-deriving the registry, so a bare slug that
// legitimately up-resolves to its grouping (dec-spor-queue-slug-resolves-to-
// grouping) is reported KNOWN — the deliberate semantics are untouched.
// Empty/falsy token => "known" (the global, unscoped read), so a caller that
// never passed --project never warns. An ordinary task/issue/decision id is
// NOT known (isScopeIdentity above) — it must earn "known" the same way any
// other non-identity string would.
function projectKnown(graph, param) {
  if (!param) return true;
  return isScopeIdentity(graph, param);
}
...
// NFD-decompose then strip combining marks (Unicode category M) so an
// accented letter (å/ä/ö, é, ü, …) folds to its base ASCII letter instead of
// acting as a [^a-z0-9] separator — on the nexavo (Swedish) corpus this was
// shredding 15.5% of word occurrences into junk fragments
// (issue-spor-rankagainst-tokenizer-shreds-diacritics). Covers precomposed
// base+mark letters (Swedish å/ä/ö decompose this way); does NOT cover a
// letter with no NFD decomposition (Danish/Norwegian ø/æ), which still
// separates words — out of scope here (Swedish only).
const foldDiacritics = (t) => t.normalize("NFD").replace(/\p{M}/gu, "");
const tokens = (t) => foldDiacritics(t.toLowerCase()).split(/[^a-z0-9]+/).filter((x) => x.length > 2 && !STOP.has(x));
...
// `maxHops` defaults to MAX_HOPS — every real caller. The override exists so
// scripts/rank-eval's pooling mode can generate a deliberately-wider/narrower
// walk variant (issue-spor-digest-rank-eval-retrieval-blind) without touching
// the shipped constant.
function structuralWalk(graph, seeds, maxHops = MAX_HOPS) { // seeds: {id: initialScore}
  const adj = graph.adj;
  const best = {};
  const queue = [];
...
// authored_via values written with no human review at write time — the same
// machine-vs-human taxonomy `spor changes` already surfaces (lib/changes.js
// MACHINE_VIA / trust()); kept in lockstep with it by hand since kernel/
// stays free of the shell-layer require.
const MACHINE_AUTHORED_VIA = new Set(["capture", "distill", "gardener"]);

// Inline "via capture" marker (task-cc-digest-render-authorship-marker): a
// Haiku-distilled capture used to render typographically identical to a
// human-reviewed decision in the compiled digest/briefing, hiding machine
// authorship from the ambient session-start context. Only machine-authored
// values earn a tag — a human/agent-direct write (mcp/rest/dispatch, or no
// authored_via at all) renders exactly as before.
function machineAuthorshipTag(n) {
  return MACHINE_AUTHORED_VIA.has(n.authored_via) ? `, machine·${n.authored_via}` : "";
}

// ---------- compile ----------
//
// opts: { rootId?, query?, digest?, minSim?, seedSchemas?, rerankScores? }
...
function compile(graph, opts = {}) {
  ...
  // Digest rerank (task-spor-compile-jev-rerank-stage): the server's
  // /v1/digest runs jev.rerank (per-org opt-in) over the cosine top-30 ∪ top
  // structural candidate pool BEFORE calling compile, and hands the verdicts
  // back as opts.rerankScores: { [id]: { score, noul } } — score an ordinal
  // 0-3 (irrelevant/background/directly-relevant/required), noul a 0-1
  // probability that this candidate answers or resolves the query.
  // compile() stays pure: it
  // never calls a backend itself, it only reorders the DIGEST_CAP section
  // (below) by what it's handed. Absent/empty — every existing caller, local
  // mode (no server to rerank with), and any org that hasn't opted in — is
  // byte-identical (norm-cc-byte-identical-refactor).
  const RERANK_SCORES = opts.rerankScores && Object.keys(opts.rerankScores).length ? opts.rerankScores : null;
  // Eval-only levers (scripts/rank-eval): let the ranking-eval harness generate
  // deliberately-different retrieval variants for retrieval-pooling (issue-
  // spor-digest-rank-eval-retrieval-blind) without touching the shipped
  // constants. Absent (every real caller), each is byte-identical to its
  // CONTENT_TOP_K/QUERY_SEEDS constant; opts.maxHops (below, structuralWalk)
  // is the third.
  const contentTopK = opts.contentTopK != null ? opts.contentTopK : CONTENT_TOP_K;
  const querySeeds = opts.querySeeds != null ? opts.querySeeds : QUERY_SEEDS;
  const NODES_DIR = graph.nodesDir;
  // Hoisted early (used below by the supersession "graph fixup" gates) — ride-
  ...
  if (ROOT_ID) {
    ...
    seeds = {};
    for (const [i, r] of ranked0.slice(0, querySeeds).entries()) {
  ...
  //
  // A correction also needs to be LIVE (issue-spor-corrections-no-applied-
  // lifecycle): status-less or "active" fires as always; "applied" means a
  // prior recompile already absorbed its guidance into the target's briefing
  // body, so it is retired dead weight and must not inject again. This is the
  // client-compile twin of the gate — the server's serve-time
  // correctionsForBriefing/applyBriefingCorrections must filter the same way
  // (schema-correction.md's status vocabulary is the shared contract).
  ...
  const correctionInScope = (t) =>
    ...
    seedIds.has(t);
  const correctionLive = (n) => (n.status || "active").toLowerCase() !== "applied";
  const corrections = Object.values(nodes).filter((n) =>
    n.type === "correction" && correctionLive(n) && correctionInScope(n.target));
  ...
  // ---------- assembly ----------
  const structural = structuralWalk(graph, seeds, opts.maxHops);
  ...
  // The "graph fixup" injects a superseder into the neighborhood so a reader
  // who lands on a superseded node also sees what replaced it. Gate it on the
  // superseder's OWN traversability (issue-cc-graph-supersededby-non-
  // traversable): supersededBy now indexes every node, including a briefing
  // or correction superseding an ordinary node, but those never earned a
  // rendered slot before this change (buildGraph's old gate required BOTH
  // ends traversable) — un-gated, this fixup would newly render a
  // non-traversable node's raw, unquoted body (unlike norms, which are quoted
  // as untrusted data) into the compiled output. `structural`/`contentPicks`
  // ids (`id`/`p.id`) are always traversable already (adj/docs are built
  // traversable-only), so only `w` needs the check.
  for (const id of Object.keys(structural)) {
    const w = supersededBy[id];
    if (w && reg.isTraversable(nodes[w]?.type) && !structural[w]) structural[w] = { score: structural[id].score, hops: structural[id].hops, via: `supersedes ${id} (graph fixup)` };
  }
  ...
  const contentPicks = ranked.filter((r) => r.sim >= CONTENT_MIN_SIM && !excluded.has(r.id) && !pinned.has(r.id)).slice(0, contentTopK);
  for (const p of [...contentPicks]) {
    const w = supersededBy[p.id];
    if (w && reg.isTraversable(nodes[w]?.type) && !structuralSet.has(w) && !contentPicks.some((x) => x.id === w)) {
  ...
  // A node under an open execution hold (resolution.executionHeld) is live
  ...
  function renderNorm(n, full) {
    ...
    const head = `### ${n.id} — ${n.title} (norm, ${n.date}${statusTag(n)}${crossTag(n)}${machineAuthorshipTag(n)})\n*authored by: ${author}*\n`;
  ...
  let out;
  let rerankApplied = 0; // # of candidates rerank actually reordered (meta only; digest mode only)
  if (DIGEST) {
    const lines = [];
    const seen = new Set();
    const add = (n, tag) => {
    ...
    if (RERANK_SCORES) {
      // Reranked ordering: pinned (above) -> the candidates Jev scored, keyed
      // noul desc -> score desc -> cosine-sim tiebreak (rankSim excludes the
      // structural arm by construction, so a structural candidate falls back
      // to 0 there and settles on id order — deterministic, never crashes on
      // a malformed entry) -> id for full determinism on a flat tie ->
      // remaining structural (unscored, original score-desc order) -> any
      // unscored content pick (original sim-desc order). Tags stay exactly
      // what the un-reranked path would have rendered (null for a structural
      // origin, the content-match note otherwise) — rerank changes ORDER, not
      // what's said about each node.
      const structuralIds = Object.keys(structural).filter((id) => !userOutOfScope(id));
      const structuralIdSet = new Set(structuralIds);
      const contentIds = contentPicks.filter((p) => !userOutOfScope(p.id)).map((p) => p.id);
      const scored = [...structuralIds, ...contentIds].filter((id) => RERANK_SCORES[id]);
      const unscoredStructural = structuralIds
        .filter((id) => !RERANK_SCORES[id])
        .sort((a, b) => structural[b].score - structural[a].score);
      const unscoredContent = contentIds.filter((id) => !RERANK_SCORES[id]);
      const noulOf = (r) => {
        if (typeof r.noul === "boolean") return r.noul ? 1 : 0;
        const n = Number(r.noul);
        if (!Number.isFinite(n)) return 0;
        return Math.min(1, Math.max(0, n));
      };
      scored.sort((a, b) => {
        const ra = RERANK_SCORES[a] || {};
        const rb = RERANK_SCORES[b] || {};
        const na = noulOf(ra);
        const nb = noulOf(rb);
        ...
        const ca = rankSim.get(a) ?? 0;
        const cb = rankSim.get(b) ?? 0;
        if (ca !== cb) return cb - ca;
        return a < b ? -1 : a > b ? 1 : 0;
      });
      rerankApplied = scored.length;
      const contentTag = "content match — no lineage, check for prior art";
      for (const id of scored) add(nodes[id], structuralIdSet.has(id) ? null : contentTag);
      for (const id of unscoredStructural) add(nodes[id], null);
      for (const id of unscoredContent) add(nodes[id], contentTag);
    } else {
  ...
  return {
    ...
    meta: {
      ...
      // Present only when this compile actually reordered the digest by
      // rerankScores — digest mode, a non-empty RERANK_SCORES, AND at least
      // one of its ids actually matched a candidate in this compile's pool
      // (rerankApplied > 0); a rerankScores map that matches nothing here
      // renders byte-identical to the non-rerank path, so `applied` must say
      // so rather than merely "a map was passed". The caller (the server's
      // /v1/digest handler) builds its own `rerank` response summary from
      // this rather than re-deriving it. Absent for every other call, so the
      // meta shape is byte-identical there (norm-cc-byte-identical-refactor).
      ...(DIGEST && RERANK_SCORES && rerankApplied > 0 ? { rerank: { applied: true, candidates: rerankApplied } } : {}),
    },
  };
}
...
function validateGraphFiles(files, seedSchemas = [], opts = {}) {
  ...
  const nearBodyCap = opts.nearBodyCap === true;
  const errors = [], warnings = [];
  const nodes = {};
  const fileNodes = []; // parsed files in directory order, for the warning pass

  for (const f of Object.keys(files).filter((f) => f.endsWith(".md"))) {
    const raw = String(files[f]).replace(/\r\n/g, "\n");
    const m = raw.match(/^---\n([\s\S]*?)\n---\n?([\s\S]*)$/);
    if (!m) { errors.push(`${f}: no frontmatter`); continue; }
    const node = { edges: [], body: m[2].trim(), file: f };
    // Mirrors parseFrontmatter's edge recognition (flow-form "to:"/"target:",
    // and block-form "edges:" followed by "- type: X" / indented "to:"/
    // "target:" lines — see the parser's own comment above) but stays LENIENT
    // like the rest of this re-parse: an edge entry the write parser would
    // reject throws there and is only RECORDED here
    // (task-spor-validate-linter-block-form-edges), so one bad entry can't
    // stop the lint from checking the rest of the file.
    //
    // It is recorded as an ERROR, though, not a warning: since
    // dec-spor-buildgraph-per-node-fault-isolation the loader no longer
    // refuses to boot on such a file — it SKIPS it — so this lint is the
    // gate that has to fail (exit 1). A warning here would leave a node
    // silently missing from every briefing, digest and queue with CI green.
    // The file keeps being re-parsed after the fault so a single run still
    // reports its other problems; the graph itself, of course, has neither
    // the node nor its edges.
    ...
    const flushEdgeBuf = () => {
      ...
      edgeBuf = null;
      if (!eb.type || !eb.to) {
        errors.push(`${f}: unparseable edge entry ${JSON.stringify(eb)} — this file is SKIPPED by the loader`);
        return;
      }
      node.edges.push(eb);
    };
    for (const line of m[1].split("\n")) {
      const kv = line.match(/^(\w[\w-]*):\s*(.*)$/);
      if (kv && kv[1] === "edges") { flushEdgeBuf(); inEdgesBlock = true; continue; }
      if (kv) {
        flushEdgeBuf();
        inEdgesBlock = false;
        node[kv[1]] = kv[2].replace(/^["']|["']$/g, "");
      ...
      // "target:" is accepted as an alias for "to:" (dec-spor-frontmatter-
      // edge-accept-normalize-reject-loud).
      const edge = line.match(/-\s*\{type:\s*([\w-]+),\s*(?:to|target):\s*([\w-]+)(?:\s*,\s*[\w-]+:\s*[\w-]+)*\}/);
      if (edge) { flushEdgeBuf(); node.edges.push({ type: edge[1], to: edge[2] }); continue; }
      if (inEdgesBlock) {
        const open = line.match(/^\s*-\s+(\w[\w-]*):\s*(.*)$/);
        if (open) {
          flushEdgeBuf();
        ...
        if (line.trim() === "" || line.trim().startsWith("#")) continue;
        errors.push(`${f}: unparseable edge entry "${line.trim()}" — this file is SKIPPED by the loader`);
      }
    }
    flushEdgeBuf();
    for (const field of ["id", "type", "title", "summary"]) {
  ...
  const reg = seedRegistry(seedSchemas);
  // A pristine seed-only registry, untouched by the graph-override layering
  // below — inertPartitionMismatches() needs the SEED's own terminal/inert
  // partition as its comparison baseline, which `reg` no longer holds once a
  // graph override wins a type's slot (graph beats seed wholesale).
  const seedOnly = seedRegistry(seedSchemas);
  ...
  // terminal/inert partition mismatch on a legacy resident override
  // (task-spor-validate-resident-override-inert-mismatch,
  // dec-spor-status-inert-third-partition): an override predating the inert
  // partition that declares terminal without inert silently inherits its
  // whole terminal set into queue-liveness, even where the seed schema
  // deliberately excludes some of it (decision's settled).
  warnings.push(...reg.inertPartitionMismatches(seedOnly).map((c) => `registry: ${c}`));

  for (const n of fileNodes) {
    if (n.type && !reg.isKnownType(n.type)) warnings.push(`${n.file}: unknown type '${n.type}'`);
    if (!n.date) warnings.push(`${n.file}: missing date`);
    // Body-size cap (issue-cc-accreting-log-body-cap-collision): the server
    // rejects any write over 8KB, so a node that has silently grown past the
    // wall (or is closing in on it) needs a warning while there's still room
    // to act — roll it over into a linked continuation node rather than
    // discovering the cap on a rejected write (GRAPH.md "Accreting running-log
    // artifacts"). See BODY_CAP_BYTES above for why only the over-cap band is
    // unconditional and the near-cap one is opt-in.
    const bodyBytes = Buffer.byteLength(n.body ?? "", "utf8");
    if (bodyBytes > BODY_CAP_BYTES) {
      warnings.push(`${n.file}: body is ${bodyBytes}B, over the server's ${BODY_CAP_BYTES}B cap — further growth will be rejected; roll over into a linked continuation node (GRAPH.md "Accreting running-log artifacts")`);
    } else if (nearBodyCap && bodyBytes >= BODY_CAP_BYTES * BODY_CAP_WARN_RATIO) {
      warnings.push(`${n.file}: body is ${bodyBytes}B, approaching the server's ${BODY_CAP_BYTES}B cap — plan a rollover continuation before it's rejected (GRAPH.md "Accreting running-log artifacts")`);
    }
  ...
  //
  // Selector id existence (issue-spor-norm-applies-to-unvalidated-silent-exclude):
  // `applies_to_repos`/`applies_to_projects` name graph nodes (a repo slug or
  // `repo-` id; a project grouping id). The runtime ride-along is fail-CLOSED —
  // a norm that declares any selector and matches NONE is strict-excluded — so a
  // typo'd name (`applies_to_projects: [proj-typo]`) silently drops the norm from
  // EVERY briefing with no error or warning. Warn when a named repo/project
  // resolves to nothing, exactly like a dangling edge: a forward reference to a
  // not-yet-created node is legitimate, so this stays a warning, never a
  // write-blocking error (the runtime fail-closed behavior is untouched). Tags
  // are a free-form open register with no node to check against, so they are not
  // existence-checked — only well-formedness (above) and placement. `projectKnown`
  // reuses the same resolvers the runtime ride-along does (the slug alias map +
  // grouping membership), so a bare slug that legitimately up-resolves to its
  // grouping is reported KNOWN.
  const selectorGraph = {
    nodes,
    projectAliases: Object.fromEntries(slugClaims),
    groupingRepos: groupingRepoMap(nodes),
  };
  ...
  for (const n of fileNodes) {
    ...
    for (const key of RIDE_KEYS) {
      ...
      if (n.type && reg.isKnownType(n.type) && !reg.isAlwaysOn(n.type)) {
        warnings.push(`${n.file}: ${key} on non-always_on type '${n.type}' has no effect (only always_on norms ride along)`);
        continue;
      }
      // Existence-check repo/project selectors on always_on norms (tags are an
      // open register; non-always_on/unknown types already warned or no-op).
      if (key === "applies_to_tags" || !(n.type && reg.isAlwaysOn(n.type))) continue;
      for (const v of values(key)) {
        if (!projectKnown(selectorGraph, v)) {
          warnings.push(`${n.file}: ${key} '${v}' matches no repo or project in the graph — the norm strict-excludes itself from every briefing (typo?)`);
        }
      }
    }
    // Coupling anchors (dec-spor-coupling-norms-declared-first): the
    // couples_when/couples_also pair only participates on live norm nodes,
    // both keys as inline lists — the matcher (kernel/coupling.js) treats a
    // scalar or a lone key as absent, fail-safe but silent, so surface the
    // mistake here exactly like the ride-along selectors above. The optional
    // couples_value_a/b VALUE invariant (`spor check`) is a scalar pair with
    // a `<path>#<regex>` grammar — same policy: a half-declared or malformed
    // pair is silently inert at check time, so it warns here.
    const COUPLE_KEYS = ["couples_when", "couples_also"];
    const VALUE_KEYS = ["couples_value_a", "couples_value_b"];
    if (COUPLE_KEYS.some(present) || VALUE_KEYS.some(present)) {
      if (n.type && n.type !== "norm") {
        warnings.push(`${n.file}: coupling keys (couples_*) on type '${n.type}' have no effect (coupling anchors live on norm nodes)`);
      } else {
        for (const key of COUPLE_KEYS) {
          if (present(key) && !listed(key)) {
            warnings.push(`${n.file}: ${key} must be an inline list '[a, b]' — a scalar value is ignored`);
          }
        }
        if (present("couples_when") !== present("couples_also")) {
          warnings.push(`${n.file}: ${present("couples_when") ? "couples_when without couples_also" : "couples_also without couples_when"} has no effect — a coupling norm declares both`);
        }
        if (VALUE_KEYS.some(present)) {
          if (!COUPLE_KEYS.every(present)) {
            warnings.push(`${n.file}: couples_value_* without couples_when/couples_also has no effect — the value invariant rides on a coupling norm`);
          } else if (present("couples_value_a") !== present("couples_value_b")) {
            warnings.push(`${n.file}: ${present("couples_value_a") ? "couples_value_a without couples_value_b" : "couples_value_b without couples_value_a"} — a value invariant declares both sides`);
          } else {
            const { parseValueSpec } = require("./coupling.js");
            for (const key of VALUE_KEYS) {
              if (present(key) && n[key] != null && !parseValueSpec(n[key])) {
                warnings.push(`${n.file}: ${key} '${n[key]}' is not '<path>#<regex>' — the value invariant is ignored`);
              }
            }
          }
        }
...
module.exports = {
  ...
  rankAgainst,
  authorshipLine,
  machineAuthorshipTag,
  MACHINE_AUTHORED_VIA,
  registry,
  DIGEST_CAP,
  DEFAULT_MIN_SIM,
};
