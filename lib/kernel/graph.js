...
// pin:/exclude:/queue_mute:/commits: inline lists, and "- {type: X, to: Y}"
// edges. commits entries are repo-qualified shas ("repo@sha",
// task-cc-commit-linking) — flat strings, not objects, by parser design.
function parseFrontmatter(raw, file) {
  ...
  for (const line of fm.split("\n")) {
    const list = line.match(/^(pin|exclude|queue_mute|commits):\s*\[([^\]]*)\]/);
...
function buildGraph(files, { nodesDir = "", seedSchemas = [] } = {}) {
  ...
  // Precompute each doc's tf-idf vector and norm once per load: rankAgainst
  ...
  for (const d of docs) {
    d.vec = vec(d.tf);
    let nn = 0;
    for (const x of Object.values(d.vec)) nn += x * x;
    d.norm = Math.sqrt(nn);
  }

  return { nodes, adj, supersededBy, docs, df, N, nodesDir, registry: reg };
}
...
function rankAgainst(graph, text, excludeIds) {
  ...
  // Same math as cos(rootVec, docVec), but against the vectors and norms
  // precomputed in buildGraph: sqrt(na)*sqrt(nb) === queryNorm*d.norm, and the
  // dot accumulates over Object.entries(rootVec) in the same order.
  let qn = 0;
  for (const x of Object.values(rootVec)) qn += x * x;
  const queryNorm = Math.sqrt(qn);
  return graph.docs.filter((d) => !excludeIds.has(d.id))
    .map((d) => {
      let dot = 0;
      for (const [t, x] of Object.entries(rootVec)) if (d.vec[t]) dot += x * d.vec[t];
      return { id: d.id, sim: dot / (queryNorm * d.norm || 1) };
    })
