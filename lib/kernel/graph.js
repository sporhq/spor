...
function validateGraphFiles(files, seedSchemas = []) {
  ...
  for (const n of fileNodes) {
    ...
    // mistake here exactly like the ride-along selectors above.
    const COUPLE_KEYS = ["couples_when", "couples_also"];
    if (COUPLE_KEYS.some(present)) {
      if (n.type && n.type !== "norm") {
        warnings.push(`${n.file}: couples_when/couples_also on type '${n.type}' has no effect (coupling anchors live on norm nodes)`);
      } else {
