...
// The set of repo-ids a project-scope slug covers, or null when the slug is not
// a grouping (then the caller scopes to the single resolved repo). A grouping
// is named by its node id (`proj-spor`); a repo slug (`spor`) is not a grouping.
function scopeRepos(graph, slug) {
  if (!slug) return null;
  return graph.groupingRepos?.[slug] ?? null;
}

// The set of repo-ids covered by a project-scope KEY, resolving the grouping in
// BOTH directions (task-cc-grouping-brief-digest-reads): a grouping id
// (proj-spor) -> its member repos; a repo id that is grouped-under a project ->
// that project's full member set, so a session in one repo unions its sibling
// repos; an ungrouped repo -> just itself. Unlike scopeRepos (queue, which
// unions ONLY when explicitly handed a grouping id), this expands a member repo
// up to its grouping — the read-side cross-pollination compile() wants. Returns
// a Set; the single-repo fallback keeps compile byte-identical for ungrouped
// graphs and for the project-blind callers (conformance) where key is absent.
function scopeReposFor(graph, key) {
  const gr = graph.groupingRepos || {};
  if (gr[key]) return gr[key];
  for (const set of Object.values(gr)) if (set.has(key)) return set;
  return new Set([key]);
}
...
function compile(graph, opts = {}) {
  ...
  // Grouping union (task-cc-grouping-brief-digest-reads): when the session repo
  // is grouped-under a `type: project` grouping, the boost spans EVERY repo in
  // that grouping — the repos of one product cross-pollinate context instead of
  // each treating the other as foreign. projectScope is the member-repo Set, or
  // {sessionProject} for an ungrouped repo — so an ungrouped graph and the
  // project-blind callers (conformance, sessionProject null) stay byte-identical
  // to the single-repo boost.
  const projectScope = sessionProject != null ? scopeReposFor(graph, sessionProject) : null;
