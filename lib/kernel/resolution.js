...
// No type means no evidence of giving up. A graph-less typed caller retains
// the narrow historical fallback; production completion reads supply a registry.
function isGiveUpStatus(status, type, graph) {
  ...
  const nonResolving = graph && graph.registry && typeof graph.registry.nonResolvingStatuses === "function"
    ? graph.registry.nonResolvingStatuses(type)
    : FALLBACK_NON_RESOLVING;
  return nonResolving.has((status || "").toLowerCase()) && isTerminalStatus(status, type, graph);
}
