...
function referenceRefusal(reference, { bundleStore = null, cwd = null } = {}) {
  ...
  if (proto !== "file" && proto !== "https") return `reference.locator '${proto}://' is not a reachable scheme — a candidate is fetched over file:// or https://`;
  ...
  if (proto === "file") {
    ...
    // that made them is.
    const inGitDir = /(^|\/)\.git(\/|$)/.test(locator);
    if (inGitDir) return `reference.locator '${locator}' points inside a .git directory — a repository's object store is not a store a reader can reach`;
    // A locator that has to be RESOLVED before you know where it points is not
    // self-describing, and the prefix tests below are string comparisons: a
    // `..` segment walks straight out of the declared store while still
    // starting with its characters. §3.4 requires the locator to RESOLVE under
    // the store, so anything needing resolution is refused outright rather than
    // normalized — this check also runs in READERS validating a candidate
    // object they did not mint, which is exactly where a traversal locator
    // would matter.
    //
    // The segment test runs over a SEPARATOR-FOLDED copy, because `\` is a path
    // separator for special schemes in the WHATWG URL spec on every platform,
    // not just Windows: `file:///store/..\..\etc/x` resolves to `file:///etc/x`
    // while string-starting with `file:///store/`, which is precisely the escape
    // this check exists to close. Percent-encoded separators and dots are folded
    // for the same reason — a fetcher that decodes before resolving would see
    // segments this test otherwise never gets to look at.
    ...
    if (cwd && here.startsWith(flat(`file://${cwd.endsWith("/") ? cwd : `${cwd}/`}`))) {
