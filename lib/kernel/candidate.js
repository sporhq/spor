...
function referenceRefusal(reference, { bundleStore = null, cwd = null } = {}) {
  ...
  if (proto === "file") {
    ...
    // that same fold doesn't cover: after `\` -> `/` it reads `//srv/share/run-1`,
    // a path with no drive letter, so it fell straight into the plain branch and
    // came out `file:////srv/share/run-1` — a locator no reader ever mints.
    // `pathToFileURL` (what actually stamps a published locator, in
    // shell/candidate-publish.js) treats a UNC path's SERVER as the URL's HOST,
    // not part of its path: `\\srv\share\run-1` becomes `file://srv/share/run-1`.
    // Matched here in the same pure-string terms as the drive-letter fold, so
    // the two sides are finally comparable — the host is lowercased the way
    // `domainToASCII` lowercases a plain-ASCII host when `pathToFileURL` mints
    // a real locator, so a cwd-derived prefix matches one it will actually meet.
    //
    // The UNC branch is gated on the RAW (unfolded) cwd starting with a literal
    // `\\` — never on the folded shape — because `pathToFileURL` itself only
    // takes its UNC branch that way (`isWindows && filepath.startsWith('\\\\')`).
    // Folding first and testing for a bare `//` prefix would also catch a
    // POSIX cwd that happens to start with two literal slashes (legal, if rare,
    // and left untouched by a real filesystem's `getcwd`); `pathToFileURL` never
    // promotes THAT leading segment to a URL host, so this guard must not either
    // — that would silently disarm it for exactly the shape this fix exists to
    // stop disarming, just from the other side.
    const cwdUrl = (() => {
      const raw = String(cwd);
      const folded = raw.replace(/\\/g, "/");
      if (raw.slice(0, 2) === "\\\\") {
        const unc = /^\/\/([^/]+)(\/.*)?$/.exec(folded);
        if (unc) return `file://${unc[1].toLowerCase()}${unc[2] || "/"}`;
      }
      return `file://${folded.replace(/^([a-zA-Z]:)/, "/$1")}`;
    })();
    // The cwd side is a raw path folded into URL shape, but a real locator was
    // minted by `pathToFileURL`, which percent-encodes (`~` -> `%7E` in a
    // Windows 8.3 `RUNNER~1`, a space -> `%20`, non-ASCII as UTF-8). Compare
    // the cwd against the DECODED locator, or any checkout path carrying one of
    // those characters silently disarms the guard.
    let hereDecoded = here;
    try {
      hereDecoded = flat(decodeURIComponent(here));
    } catch {
      /* malformed escapes: judge the raw form */
    }
    if (cwd && hereDecoded.startsWith(flat(cwdUrl.endsWith("/") ? cwdUrl : `${cwdUrl}/`))) {
