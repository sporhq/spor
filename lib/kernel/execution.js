...
// Stable compare-and-swap over the authoritative record, independent of read
// decorations. A same-fence renewal or accepted event changes this revision.
function releaseStateKey(record) {
  const sorted = value => Array.isArray(value) ? value.map(sorted) : value && typeof value === "object"
    ? Object.fromEntries(Object.keys(value).sort().map(key => [key, sorted(value[key])])) : value;
  const { boundary_reached, terminal, release_revision, ...state } = record;
  state.gate_results = (state.gate_results || []).map(({ on_tip, ...row }) => row);
  return JSON.stringify(sorted(state));
}

// Authority is checked by the engine, from its authenticated/constructor-bound
// identity. The pure transition never treats request actor fields as proof.
function forceRelease(record, { nodeId, person, reason, requestId, expectedRevision, revision, now }) {
  if (typeof reason !== "string" || !reason.trim() || reason.length > 2000 ||
      typeof requestId !== "string" || !/^[a-zA-Z0-9_-]{1,128}$/.test(requestId) ||
      typeof expectedRevision !== "string" || !/^[a-f0-9]{64}$/.test(expectedRevision) || !person || !Number.isFinite(Date.parse(now))) {
    return reject("invalid_event", "force release requires a person, reason (1-2000 characters), request_id and expected_revision");
  }
  if (nodeId !== record.item.node_id) return reject("conflict", "execution belongs to a different item");
  const prior = record.force_release;
  if (prior && prior.request_id === requestId) {
    if (prior.person !== person || prior.reason !== reason.trim() || prior.expected_revision !== expectedRevision) return reject("conflict", "force-release request_id was already used with different arguments");
    return { ok: true, replayed: true, record };
  }
  if (isTerminal(record)) return reject("execution_terminal", "execution is already completed or released");
  if (expectedRevision !== revision) return reject("conflict", "execution changed since it was inspected; inspect it again before force release");
  const next = cloneRecord(record);
  next.force_release = {
    request_id: requestId, person, reason: reason.trim(), at: String(now), expected_revision: expectedRevision,
    previous_owner: record.owner ? { ...record.owner } : null,
    previous_state: { stage: record.stage, seq: record.seq, candidate_id: record.candidate?.candidate_id || null },
    fence: (Number(record.owner?.fence) || 0) + 1,
  };
  next.owner = null;
  next.released_at = String(now);
  next.updated_at = String(now);
...
function rebuildFromEvents(events, { sha256 } = {}) {
  ...
  for (const ev of list) {
    ...
    if (ev.type === "ownership.changed") {
      record = { ...record, owner: ev.owner ? { ...ev.owner } : null, ...(ev.released_at ? { released_at: ev.released_at } : {}), ...(ev.force_release ? { force_release: ev.force_release } : {}), updated_at: ev.at };
      continue;
    }
