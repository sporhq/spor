...
function boundaryReached(record) {
  ...
  const gatesOk = record.gate_results.length > 0 && record.gate_results.every((g) => gateAccepted(record, g));
  ...
  const accepted = candidatePublished(record.candidate) && (record.gate_results.length === 0 || gatesOk);
...
function gateAccepted(record, g) {
  if (!(g.state === "passed" || g.state === "skipped")) return false;
  if (gateOnTip(record, g)) return true;
...
function applyExecutionEvent(record, event, { seen = new Set(), now = null, historicalReplay = false } = {}) {
  ...
  const live = ownershipLive(record, { fence: event.fence, now: now ?? at });
  if (!live.ok) return live;
...
function applyByType(next, event, at, { historicalReplay = false } = {}) {
  switch (event.type) {
    ...
    case "candidate.published": {
      ...
      if (!historicalReplay && !validVerificationTime(reference.verified_at)) return reject("invalid_event", "publication requires a valid verification timestamp");
    ...
    case "gate.started": {
      ...
      if (!candidatePublished(next.candidate)) return reject("no_candidate", "gates cannot run before a candidate is published");
      g.state = null;
      g.candidate_id = null;
    ...
    case "gate.settled": {
      ...
      if (!candidatePublished(next.candidate)) return reject("no_candidate", "a gate cannot settle before a candidate is published");
      if (chainOf(next).length > 1 && event.candidate_id == null) return reject("invalid_event", "gate.settled requires the candidate_id judged after a re-pin");
...
function pinCandidate(next, event, at, { historicalReplay = false } = {}) {
  ...
  if ((!tip || String(tip.candidate_id) !== String(c.candidate_id)) && chain.some((entry) => String(entry.candidate_id) === String(c.candidate_id))) {
...
function boundaryReachedForGates(record) {
  if (!candidatePublished(record.candidate)) return false;
  if (record.gate_results.length === 0) return true;
  return record.gate_results.every((g) => gateAccepted(record, g));
}
