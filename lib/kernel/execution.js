...
// The server kernel's own candidate-id helper, ported for parity: `cand-` +
// sha256-16 of (node_id, attempt, commit, tree). NOTE it is NOT the id a
// candidate object carries: the server stores the `candidate_id` the CLIENT
// submits verbatim (server/executions.js never calls its helper for a
// submission), and the client mints that id in kernel/candidate.js over
// `(repo, node_id, tree)` — content-addressed on what is judged, per
// FACTORY-IMPLEMENTATION-STAGE.md §3.2. This one is here only so a reader
// holding a server-minted id can recompute it.
function serverCandidateIdFor({ node_id, attempt, commit, tree }, sha256) {
  return `cand-${sha16([node_id, attempt, commit, tree], sha256)}`;
}
...
function derivedEventKey(record, event) {
  ...
  switch (event.type) {
    case "candidate.submitted":
      return event.candidate && event.candidate.candidate_id ? String(event.candidate.candidate_id) : `${x}:candidate`;
    case "gate.started":
...
// Build the pinned execution record (§3, §3.1). Every revision the pipeline
// will enforce is captured HERE and no event rewrites one — an edit to the
// factory node mid-pipeline changes nothing for the running execution.
function initExecution(spec, { sha256 } = {}) {
  ...
  const id = executionIdFor(
    { tenant: spec.tenant, node_id: spec.node_id, factory: spec.factory_node_id, pipeline_attempt: pipelineAttempt },
    sha256
  );
  return {
    ...
    attempts: [],
    // Seeded from the pinned gate list so the read surface reports every
    // declared gate, including the ones that have not started.
    gate_results: gates.map((g) => ({ id: g.id, state: null, attempt: 0, settled_at: null })),
    candidate: null,
    integration: null,
    escalations: [],
    owner: null,
...
// written? One pure predicate over the record — what the server's write gate
// consults, and what the client's completion write confirms against.
function boundaryReached(record) {
  if (!record || record.stage === "refused") return false;
  if (record.stage === "completed") return true;
  const results = Array.isArray(record.gate_results) ? record.gate_results : [];
  const gatesOk = results.length > 0 && results.every((g) => g.state === "passed" || g.state === "skipped");
  const accepted = record.candidate != null && (results.length === 0 || gatesOk);
  if (!record.completion || record.completion.boundary === "gates") return accepted;
  return accepted && !!(record.integration && record.integration.state === "landed");
}

// Terminal = nothing further will be recorded.
function isTerminal(record) {
  return !!record && (record.stage === "completed" || record.stage === "refused");
}

// Is `fence` still the live one, and is the lease unexpired at `now`? A stale
// fence means someone took over; an expired lease means the owner vanished and
// its writes must stop even before anyone takes over.
function ownershipLive(record, { fence, now }) {
  const o = record && record.owner;
  if (!o) return { ok: false, code: "not_owned", message: "execution has no live owner; claim it first" };
  if (Number(fence) !== Number(o.fence)) {
    return { ok: false, code: "fence_stale", message: `fence ${fence} is stale; the execution is held at fence ${o.fence}` };
  }
  if (now != null && o.lease_expires_at != null && Date.parse(o.lease_expires_at) <= Date.parse(now)) {
    return { ok: false, code: "lease_expired", message: `lease expired at ${o.lease_expires_at}` };
  }
  return { ok: true };
}

// The two derived fields the read surface adds (`GET /v1/executions/:id`).
function decorate(record) {
  if (!record) return null;
  return { ...record, boundary_reached: boundaryReached(record), terminal: isTerminal(record) };
}
...
// Acquire ownership (§4). A free, expired, or same-owner execution is
// claimable; an UNEXPIRED lease held by someone else is never stolen — a
// plain claim loses with `already_owned`, an explicit takeover with
// `lease_live`. The fence advances on every acquisition by a different worker
// and on every takeover, never on a same-owner re-claim.
function claim(record, { worker, machine, lease_expires_at, now, takeover = false }) {
  ...
  const sameOwner = !!o && o.worker === worker;
  if (o && !expired && !sameOwner) {
    if (takeover) {
      return { ok: false, code: "lease_live", message: "cannot take over a live lease", holder: { ...o } };
    }
    return { ok: false, code: "already_owned", message: `execution is owned by ${o.worker} until ${o.lease_expires_at}`, holder: { ...o } };
  }
...
// Heartbeat: push the expiry out without touching the fence.
...
// Hand the execution back. Only the live fence-holder may release.
...
// execution — read off the durable log by the caller, so replay suppression
// survives a restart. `event.at` is the recorded timestamp; the reducer never
// reads a clock.
function applyExecutionEvent(record, event, { seen = new Set(), now = null } = {}) {
  if (!isPlainObject(event)) return reject("invalid_event", "event must be an object");
  if (!EVENT_TYPES.includes(event.type)) return reject("invalid_event", `unknown event type '${event.type}'`);
  const at = event.at != null ? String(event.at) : now != null ? String(now) : null;

  const key = eventKey(record, event);
  if (!key) return reject("invalid_event", `event '${event.type}' has no idempotency key and none could be derived`);
  // Replay BEFORE ownership: a retry from a worker since fenced out still reads
  // as "already recorded" rather than pushing a correct client into recovery.
  if (seen.has(key)) return { ok: true, record, replayed: true, idempotency_key: key };
  // Terminal BEFORE ownership: a settled execution is settled for everyone.
  if (isTerminal(record)) return reject("execution_terminal", `execution is ${record.stage}; no further events are accepted`);

  const live = ownershipLive(record, { fence: event.fence, now: at });
  if (!live.ok) return live;

  const next = cloneRecord(record);
  const handled = applyByType(next, event, at);
...
function applyByType(next, event, at) {
  switch (event.type) {
    case "stage.started": {
    ...
    case "candidate.submitted": {
      ...
      if (!isPlainObject(c)) return reject("invalid_event", "candidate.submitted requires a candidate object");
      if (!c.commit || !c.tree) return reject("invalid_event", "a candidate must pin both commit and tree");
      if (next.candidate && next.candidate.candidate_id !== c.candidate_id) {
        return reject("candidate_conflict", `execution already holds candidate ${next.candidate.candidate_id}`);
      }
      next.candidate = { ...c };
    ...
    case "gate.started": {
      ...
      if (!next.candidate) return reject("no_candidate", "gates cannot run before a candidate is submitted");
    ...
    case "gate.settled": {
      const g = gateRow(next, event.gate_id);
      if (!g) return reject("unknown_gate", `gate '${event.gate_id}' is not in this execution's pinned gate list`);
      if (!GATE_STATES.includes(event.state)) return reject("invalid_event", `gate state must be one of ${GATE_STATES.join("|")}`);
      if (!next.candidate) return reject("no_candidate", "a gate cannot settle before a candidate is submitted");
      g.state = event.state;
    ...
    case "integration.started": {
      if (!boundaryReachedForGates(next)) {
    ...
    case "integration.settled": {
      ...
      if (!INTEGRATION_STATES.includes(state)) return reject("invalid_event", "integration state must be landed|parked|failed");
      next.integration = {
        ...next.integration,
        state,
        at,
    ...
    case "completion.written": {
      if (!boundaryReached(next)) {
        return reject("boundary_not_reached", `completion boundary '${next.completion.boundary}' has not been reached`);
      }
      next.completion = { ...next.completion, written_at: at, resolver: event.resolver != null ? String(event.resolver) : null };
...
function boundaryReachedForGates(record) {
  if (!record.candidate) return false;
  if (record.gate_results.length === 0) return true;
  return record.gate_results.every((g) => g.state === "passed" || g.state === "skipped");
}
...
function rebuildFromEvents(events, { sha256 } = {}) {
  ...
  for (const ev of list) {
    ...
    if (ev.type === "ownership.changed") {
      record = { ...record, owner: ev.owner ? { ...ev.owner } : null, updated_at: ev.at };
      continue;
    }
    const r = applyExecutionEvent(record, ev, { seen });
    if (r.ok && !r.replayed) {
      record = r.record;
