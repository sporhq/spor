...
//
// `isTerminal` throughout is `(status, id) => boolean` — the id lets a
// type-aware caller (lib/analytics.js) look up the node's OWN type and apply
// its per-type completion partition instead of one flat cross-type vocabulary
// (task-spor-analytics-type-aware-inert-partition); a caller that only cares
// about status ignores the second argument.
//
// Exports:
//   foldStatusTransitionState(logText, nodesName, isTerminal, baseState) -> state
//   statusTransitionsFromState(state)                      -> { id: closedAtISO }
...
// Shallow-clone the forward-walk state { id: { status, terminal, runStart } } so a
// seeded incremental fold never mutates the caller's cached object.
const cloneState = (s) => {
  const out = {};
  for (const id of Object.keys(s)) out[id] = { ...s[id] };
  return out;
};

// Fold `git log --reverse --no-renames --format=%x01%ct -p --diff-filter=ACMR
// -- <nodesName>/` (OLDEST commit first) into the per-node forward-walk STATE
// { id: { status, terminal, runStart } } — the cacheable, range-composable form
// behind the closed-at fold (task-spor-analytics-closed-at-cache). `runStart` is
// the committer time (ms) of the commit that BEGAN the node's current terminal
// run (null when it is not in one); statusTransitionsFromState turns terminal
// runs into closed-at times. A done→reopened→done node tracks its latest run; a
// done→merged node keeps the original run start (it never left terminal).
//
// `baseState` seeds the walk with a prior fold's state so an OLD..NEW range
// composes onto a cached base: the forward walk only ever ADVANCES/RESETS a
// node's run from where it stood, so seed-then-fold-the-range == fold-from-empty
// over the full history (the invariant the HEAD-keyed cache relies on). Passing
// no base (or null) folds from scratch.
//
...
function foldStatusTransitionState(logText, nodesName = "nodes", isTerminal = () => false, baseState = null) {
  const st = baseState ? cloneState(baseState) : {}; // id -> { status, terminal, runStart }
  if (!logText) return st;
  ...
  const flushFile = () => {
    ...
    const term = next != null && isTerminal(next, file);
...
// Project the forward-walk state into the closed-at map: a node currently in a
// terminal run (terminal && runStart != null) completed at runStart. Pure.
function statusTransitionsFromState(st) {
  const out = {};
...
// { id: closedAtISO } for every node whose FINAL committed `status:` is terminal —
// the uncached, fold-from-scratch convenience over the two primitives above. A
// node whose final status is non-terminal (or never set) is absent (not completed).
function foldStatusTransitions(logText, nodesName = "nodes", isTerminal = () => false) {
  return statusTransitionsFromState(foldStatusTransitionState(logText, nodesName, isTerminal, null));
}
...
//   resolvedBy   : { id: resolverId }        — resolutionMap targets (a live
//                                              resolves/answers edge retires the
//                                              target even while its status lags)
//   isTerminal   : (status, id) => boolean   — registry/resolution terminal test,
//                                              per-node type-aware (see file header)
...
function computeAnalytics(opts = {}) {
  const {
    nodes = {}, timestamps = null, gitClosed = {}, supersededBy = {}, resolvedBy = {},
  ...
  // Completion truth for a RETIRED node (terminal status, superseded, or retired
  // by a live resolves/answers edge), high-first:
  ...
  //   3. the resolver's created_at (a live resolves/answers edge retires the
  //      target without flipping its status, so when the status still lags open it
  //      has no transition of its own — the resolver's birth is when it was retired;
  //      task-spor-analytics-resolution-edge-completion);
  //   4. its own created_at (born terminal, or git history squashed away).
  // updated_at is deliberately never consulted. (2) and (3) only bind when the
  // node has no own status-transition; a node that DID flip terminal keeps its git
  // time, so a resolver edge never moves an already-git-dated completion.
  const completionOf = (id, node) => {
    ...
    const res = resolvedBy[id];
    if (res) {
      const rt = timestamps && timestamps[res];
      if (rt && rt.created_at) return { at: parse(rt.created_at), source: "resolved" };
    }
  ...
  for (const node of Object.values(nodes)) {
    ...
    const id = node.id;
    const retired = isTerminal(node.status, id) || !!supersededBy[id] || !!resolvedBy[id];
  ...
  let createdTot = 0, completedTot = 0, gitTransitions = 0, fallbackCompletions = 0, resolutionEdge = 0;
  const cycleDays = [];
  for (const it of considered) {
    ...
    if (it.closed != null && it.closed >= windowStart && it.closed < windowEnd) {
      ...
      // resolution-edge completions are a sub-slice of fallback (no own git
      // transition), surfaced separately so the footnote can name the status-lag.
      if (it.closedSource === "resolved") resolutionEdge++;
    }
  }
  ...
  return {
    ...
    coverage: {
      ...
      fromResolutionEdge: resolutionEdge,
...
module.exports = {
  foldStatusTransitionState, statusTransitionsFromState, foldStatusTransitions,
  computeAnalytics, isoWeekKey, weekStartUTC,
};
