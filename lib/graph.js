...
// a hand-mirrored 4-type table in dispatch-terminal.js. Reads the type's own
// DECLARED `resolution.verified_by`, falling back to the legacy `get`-hook
// proxy only for a schema that declares neither
// (issue-spor-offline-check-get-hook-resolution-proxy; Registry.isEdgeVerified
// carries the full reasoning). Seed pack only, same fallback shape as the
// other offline helpers above; a remote-only caller with no loaded graph
// can't see a graph-resident schema override that declares it differently.
function isEdgeVerifiedOffline(type) {
  try {
    return seedRegistry().isEdgeVerified(type);
  } catch {
    return false;
  }
}

// The LIVE-registry counterpart of isEdgeVerifiedOffline /
// isCompletionStatusOffline above, for a caller that fetched the live schema
// registry (`GET /v1/schema`) instead of reading the shipped seed pack
// (issue-spor-remote-dispatch-ignores-resident-resolution-hooks). A graph-
// resident schema override that declares a different attestation path — or,
// legacy, that adds or drops a type's `get()` hook — is invisible to the two
// *Offline helpers, which only ever see
// lib/seed/ — this reads the same two facts off `snapshot`, the parsed JSON
// body of `GET /v1/schema` (or a local `graph.registry.snapshot()`, e.g. in
// tests), which DOES carry resident overrides. Pure: no network, no fs.
//
// A type absent from `snapshot.node_types` (an unrecognized type, or a
// snapshot that could not be fetched and was passed through as `{}`) mirrors
// the offline helpers' own fail-safe reading for a type the seed pack
// doesn't know either: not edge-verified (falls through to the status
// check), with no per-type terminal statuses beyond the type-blind universal
// completion words `kernelResolution.terminalStatuses` already covers.
//
// That type-blind fallback is UNIONED with the snapshot's own live
// `terminal-status` register when it carries one
// (issue-spor-terminal-status-validation-regressions): `snapshot.registers`
// (Registry.snapshot()'s `register`-kind slot) reflects a graph-resident
// `schema-register-terminal-status` override the same way `node_types`
// reflects a resident node-schema override, but until this fix
// `resolutionFactsFromSchemaSnapshot` read only the hardcoded
// `kernelResolution.terminalStatuses` array and never looked at `registers`
// at all — so a remote tenant that grew the vocabulary with a custom
// universal terminal status (e.g. `archived`) had that status invisible to
// remote dispatch's terminal-state verify leg: a node retired by it would
// never read `resolved`, risking a duplicate agent re-dispatched onto
// already-finished work. Mirrors resolution.js's `terminalVocabulary(graph)`:
// UNION, never replace, so a resident register naming only its own new
// status can't silently drop the fallback's dozen values.
function resolutionFactsFromSchemaSnapshot(snapshot, type) {
  const nodeTypes = snapshot && Array.isArray(snapshot.node_types) ? snapshot.node_types : [];
  const entry = nodeTypes.find((n) => n && String(n.type || "").toLowerCase() === type);
  // The snapshot's own `resolution` declaration decides it, exactly as
  // Registry.isEdgeVerified does locally (issue-spor-offline-check-get-hook-
  // resolution-proxy). The `hooks` proxy below it is the legacy fallback for
  // the two shapes that carry no declaration: a graph-resident schema written
  // before the key existed, and an OLDER SERVER whose snapshot has no
  // `resolution` key at all — for which the proxy is still the best available
  // reading, and is byte-identical to what this function did before.
  const declared = entry && (entry.resolution === "edge" || entry.resolution === "status") ? entry.resolution : null;
  const edgeVerified = declared
    ? declared === "edge"
    : !!(entry && Array.isArray(entry.hooks) && entry.hooks.includes("get"));
  const terminal = new Set(
    entry && Array.isArray(entry.terminal)
      ? entry.terminal.filter((v) => typeof v === "string" && v).map((v) => v.toLowerCase())
      : []
  );
  const registers = snapshot && Array.isArray(snapshot.registers) ? snapshot.registers : [];
  const terminalRegister = registers.find((r) => r && r.name === "terminal-status");
  const universal = new Set(kernelResolution.terminalStatuses);
  if (terminalRegister && Array.isArray(terminalRegister.classes)) {
    for (const c of terminalRegister.classes) {
      if (c && typeof c.id === "string" && c.id) universal.add(c.id.toLowerCase());
    }
  }
  return {
    edgeVerified,
    isCompletionStatus(status) {
      const s = String(status || "").toLowerCase();
      return universal.has(s) || terminal.has(s);
    },
  };
}
...
module.exports = {
  ...
  // the type-aware, own-key identity check projectKnown is built on
  // (issue-spor-kernel-project-known-accepts-arbitrary-nodes) — exposed so a
  // caller needing just the identity half (no groupingOf-derived fallback)
  // doesn't have to re-derive it.
  isScopeIdentity: kernel.isScopeIdentity,
  ...
  isEdgeVerifiedOffline,
  // DEPRECATED back-compat alias for the name this helper carried before
  // issue-spor-offline-check-get-hook-resolution-proxy renamed it. `lib/` is a
  // PUBLISHED surface the server imports by name, so a rename that simply
  // deletes the old export is a TypeError in a consumer pinned to an older
  // client (review finding F2). It points at the SAME function, so the old
  // spelling now gets the declaration-driven answer its callers were always
  // asking for; Registry.attachesResolutionHook is the instance-method twin.
  attachesResolutionHookOffline: isEdgeVerifiedOffline,
  // the live-registry counterpart above (issue-spor-remote-dispatch-ignores-
  // resident-resolution-hooks): the same two facts, read off a fetched
  // `GET /v1/schema` snapshot instead of the seed pack.
  resolutionFactsFromSchemaSnapshot,
