...
// The LIVE-registry counterpart of attachesResolutionHookOffline /
...
// resident schema override that adds or drops a type's `get()` resolution
// hook is invisible to the two *Offline helpers, which only ever see
...
function resolutionFactsFromSchemaSnapshot(snapshot, type) {
  ...
  const edgeVerified = !!(entry && Array.isArray(entry.hooks) && entry.hooks.includes("get"));
