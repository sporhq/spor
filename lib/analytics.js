...
// The analytics terminal vocabulary: the kernel's legacy type-blind TERMINAL set
// (resolution.terminalStatuses) UNIONED with every node-schema's declared
// status.terminal partition off graph.registry (dec-cc-registry-as-data,
// issue-spor-analytics-completion-ignores-schema-terminal-status). A schema-only
// terminal status — decision `settled` — is terminal for a node's OWN lifecycle
// but deliberately absent from resolution.js TERMINAL (queue liveness / briefing
// surfacing must keep a settled decision live, dec-spor-decision-lifecycle-
// surfacing), so analytics needs the broader union or a settled decision lingers
// in WIP and the oldest-open bottleneck list forever. Empty registry partition ->
// the set is the legacy one exactly (byte-identical). Lowercased throughout.
function terminalVocab(graph) {
  const set = new Set(resolution.terminalStatuses);
  const reg = graph.registry;
  if (reg && typeof reg.terminalStatuses === "function") {
    for (const s of reg.terminalStatuses()) set.add(s);
  }
  return set;
}
...
// terminal predicate and its fingerprint, both derived from terminalVocab(graph) by
// the caller so the fold and the cache key share ONE vocabulary (the registry
// partition + the legacy set). An exact-HEAD+fp hit reuses the cached per-node fold
...
function analyze(graph, opts = {}) {
  // One terminal vocabulary drives BOTH the fold and the kernel's `retired` test:
  // the registry's status.terminal partition unioned with the legacy set, so a
  // schema-only terminal status (decision `settled`) counts as completion
  // (issue-spor-analytics-completion-ignores-schema-terminal-status). The cache
  // fingerprint keys on the SAME vocabulary, so a seed or resident terminal-vocab
  // change invalidates a cache whose folded state baked in the old set (a resident
  // change also moves HEAD; fingerprinting the union covers a code-level seed change
  // too — the issue's cache note).
  const vocab = terminalVocab(graph);
  const isTerminal = (s) => vocab.has((s || "").toLowerCase());
  const fp = [...vocab].sort().join(",");
