...
// cache), mirroring graph.js deriveTimestamps. An exact-HEAD+fp hit reuses the
// cached per-node fold state with NO `git log -p`; a cached HEAD that is an
// ancestor of current folds only OLD..NEW and composes onto the cached state (the
// forward status walk is range-composable); a history rewrite, a vocabulary
// change, or a cold/torn cache does a full rebuild. The cache lives beside
// timestamps.json under the gitignored cache/ dir and stores the per-node STATE
// (not the closed-at output) so the incremental fold can continue the walk.
// Fail-open like graph.js's gitFront: a non-git home or a failed `git log`
// returns the best available map and NEVER poisons the cache with a transient
// empty result (a torn read would otherwise stick across exact-HEAD hits).
function deriveStatusTransitions(graph) {
  ...
  if (reusable && gittime.isAncestor(repoDir, cached.head, head)) {
    ...
    state = kernel.foldStatusTransitionState(log, nodesName, resolution.isTerminalStatus, cached.state);
  } else {
    ...
    state = kernel.foldStatusTransitionState(log, nodesName, resolution.isTerminalStatus, null);
  }
...
function analyze(graph, opts = {}) {
  const gitClosed = deriveStatusTransitions(graph); // cache-backed, HEAD+fp-keyed
