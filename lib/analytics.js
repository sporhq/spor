...
// A node's cached fold entry is only re-evaluated by foldStatusTransitionState
// on a ±status: delta (see its header comment) — a commit that retypes a node
// WITHOUT touching its status line flushes as "unchanged" and leaves the
// entry's terminal/runStart exactly as they were under the OLD type. Since
// isTerminal is bound to each node's CURRENT type (graph.nodes[id].type), that
// stale entry silently diverges from a cold rebuild, which recomputes every
// status delta against the current type from scratch
// (issue-spor-analytics-type-modification-cache-invalidation). Detect it by
// snapshotting each folded node's type alongside its state and, on the next
// load, refusing to compose an incremental range onto a snapshot whose type no
// longer matches the graph — falling back to a full rebuild instead (retypes
// are rare, so this is cheap in practice). A snapshot-less entry (a cache
// written before this check existed) carries no signal and is trusted as-is.
function snapshotTypes(state, graph) {
  for (const id of Object.keys(state)) {
    const node = graph.nodes[id];
    if (node) state[id].type = node.type;
  }
  return state;
}
function hasRetypedNode(cachedState, graph) {
  return Object.keys(cachedState).some((id) => {
    const entry = cachedState[id];
    const node = graph.nodes[id];
    return node && entry.type != null && entry.type !== node.type;
  });
}
...
// state with NO `git log -p`; a cached HEAD that is an ancestor of current AND whose
// snapshotted node types still match the graph (see hasRetypedNode above) folds
// only OLD..NEW and composes onto the cached state (the forward status walk is
// range-composable); a history rewrite, a vocabulary change, a retyped node, or a
// cold/torn cache does a full rebuild. The cache lives beside timestamps.json under
// the gitignored cache/ dir and stores the per-node STATE (not the closed-at output)
// so the incremental fold can continue the walk. Fail-open like graph.js's gitFront: a
...
function deriveStatusTransitions(graph, isTerminal, fp) {
  ...
  // A vocabulary change OR a retyped node forces a full rebuild — checked here,
  // ahead of BOTH the exact-HEAD and incremental branches below, because
  // graph.nodes reads the WORKING TREE (lib/graph.js's readGraphFiles), not
  // HEAD's committed content: an uncommitted retype leaves `head` unchanged, so
  // an exact-HEAD hit would otherwise reuse cached.state verbatim with no
  // hasRetypedNode check at all, serving the OLD type's terminal classification
  // even though the graph passed in already reflects the new one.
  const reusable = !!cached && cached.fp === fp && !hasRetypedNode(cached.state, graph);
  ...
  state = snapshotTypes(state, graph);
