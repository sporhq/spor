...
// exactly the mis-attribution the flag exists to prevent by default. All
// three are dropped with a warning, the same way a repo-level token is: the
// rule these enforce is that a WRITE SOMEONE ELSE CAN MAKE never defines
// machine-local dispatch policy, of which "a graph write must never define
// what a machine executes" is the sibling half.
const REPO_FORBIDDEN_PATHS = ["dispatch.harness", "dispatch.bin", "dispatch.allowPersonToken"];
