...
// secrets, but because they name what this machine EXECUTES
// (task-spor-dispatch-declarative-custom-harness). `dispatch.harness.<id>`
// binds a harness id to a command plus a full argv template, and
// `dispatch.bin.<harness>` re-points a built-in launcher; both are documented
// as machine-local, belonging in the user `$SPOR_HOME/config.json`. Honoring
// them from a committable `.spor.json` would mean cloning a repo — or pulling
// a PR branch into one — is enough to choose the command a later `spor
// dispatch` on this box runs, and (with `dispatch.agent` set) enough to get
// that harness id auto-published to the fleet scheduler as a capability this
// box advertises. Dropped with a warning, the same way a repo-level token is:
// the rule these enforce is that a WRITE SOMEONE ELSE CAN MAKE never defines
// what a machine executes, of which "a graph write must never define what a
// machine executes" is the other half.
const REPO_FORBIDDEN_PATHS = ["dispatch.harness", "dispatch.bin"];
...
function sanitizeRepoLayer(data, file, warnings) {
  ...
  for (const dotted of REPO_FORBIDDEN_PATHS) {
    ...
    if (isPlainObject(cur) && Object.prototype.hasOwnProperty.call(cur, leaf)) {
      delete cur[leaf];
      warnings.push(`ignored '${dotted}' in committable repo config ${file} (what this machine executes is machine-local — declare it in $SPOR_HOME/config.json)`);
    }
  }
  return data;
}
