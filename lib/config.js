...
const REPO_FORBIDDEN_PATHS = [
  "dispatch.harness",
  "dispatch.bin",
  "dispatch.allowPersonToken",
  "distill.cmd",
  "nudge.cmd",
  "digest.intentCmd",
...
const KNOWN_KEYS = new Set([
  ...
  "search", "queue", "distill", "nudge", "digest", "claimNudge", "couplingNudge", "sessionLease", "inferCommits", "dispatch", "gc",
  // The pull worker's own namespace (task-spor-work-loop): accept, factory,
  // concurrency, intervalMs, maxIntervalMs, retryAfterMs, runMaxMs, project.
  // Its env twins (SPOR_WORK_ACCEPT, ...) map into `work.*` in ENV_MAP below,
  // so omitting it here warned "unknown config key 'work' ignored" about a
  // value that was in fact being honored.
  "work",
  // The attestation signing key + key id (task-spor-factory-gate-attestation):
  // `signingKey` is a SECRET (HMAC over each gate attestation, verified by a
  // CI job holding the same key) and so is stripped from a committable repo
  // `.spor.json` (REPO_FORBIDDEN_PATHS) — env, the user config, or the global
  // config only.
  "attestation",
  ...
  "briefs",
  // Coupling-matcher settings shared by BOTH consumers (the post-tool nudge
  // AND `spor check`) — distinct from `couplingNudge`, which is nudge-only
  // (enable/timeout). Currently just `coupling.aliases`, a declared
  // `{ "<alias prefix>": "<canonical prefix>" }` map the matcher expands in
  // both directions (issue-spor-coupling-matcher-reverse-symlink-gap); see
  // lib/kernel/coupling.js's expandAliasCandidates.
  "coupling",
  // The factory execution store's knobs (task-spor-client-execution-store-
  // adapter): the lease TTL a local execution or a hosted claim asks for
  // (`execution.leaseTtlMs`, default 15min, clamped to the server's [60s, 8h])
  // and the per-call bound on the `/v1/executions` round trips
  // (`execution.timeoutMs`, default 8s).
  "execution",
]);
...
const ENV_MAP = [
  ...
  ["SESSION_LEASE", "sessionLease.enabled"], // SPOR_SESSION_LEASE=0 disables the SessionEnd reserve/release branch
  ["SESSION_LEASE_TIMEOUT", "sessionLease.timeoutMs"], // bound each lease-check/reserve/release curl (ms)
  ...
  ["DISPATCH_CLAUDE_LAUNCH_MODE", "dispatch.claudeLaunchMode"], // supervised (default) | native-background: the standing twin of `spor dispatch --bg`
  ["ALLOW_PERSON_TOKEN", "dispatch.allowPersonToken"], // SPOR_ALLOW_PERSON_TOKEN=1 restores the pre-hard-fail person-scoped fallback (dec-spor-worker-strictness-split-interactive-lenient)
  ...
  ["AUTO_ROUTE", "dispatch.autoRoute"], // SPOR_AUTO_ROUTE=1 arms the autonomous FORK B handoff: an unsatisfiable-here node is re-assigned to a fleet host that satisfies THAT profile
  ["AUTO_ROUTE_MAX_AGE", "dispatch.autoRouteMaxAge"], // liveness bound on a re-route target's last_seen (30m/12h/7d/ms; default 24h, 0 disables)
  ...
  ["ATTESTATION_KEY", "attestation.signingKey"], // HMAC-SHA256 key the gate pipeline signs each run attestation with (secret; never a repo .spor.json)
  ["ATTESTATION_KEY_ID", "attestation.keyId"], // the `key_id` stamped beside the signature (default "default")
  ["WORK_ACCEPT", "work.accept"], // `spor work` acceptance policy: ready (default, explicit agent-ready only) | open
  ["WORK_RESTART_ON_LAND", "work.restartOnLand"], // SPOR_WORK_RESTART_ON_LAND=1: `spor work` exits for a restart once its loaded checkout moves past its code
  ...
  ["EXECUTION_TTL", "execution.leaseTtlMs"], // the factory execution lease TTL a claim asks for (ms, default 15min, clamped to [60s, 8h])
  ["EXECUTION_TIMEOUT", "execution.timeoutMs"], // bound each /v1/executions round trip (ms, default 8s)
];
...
// cwd in one call. Env-scrubbed (gitSpawn, shell/git-exec.js) so an ambient
// GIT_DIR/GIT_WORK_TREE/GIT_COMMON_DIR can't misdirect this worktree-root probe
// at a different repo (issue-spor-gittime-git-env-inheritance).
const _mainRootCache = new Map();
function gitMainRoot(cwd) {
  if (!cwd) return null;
  if (_mainRootCache.has(cwd)) return _mainRootCache.get(cwd);
  let result = null;
  const r = gitSpawn(cwd, ["rev-parse", "--path-format=absolute", "--git-common-dir", "--show-toplevel"]);
  if (r.status === 0) {
    // Strip a trailing \r per line, not just the trailing newline: some
    // Windows git builds emit CRLF here, and a stray \r baked into the path
    // would make mainTop !== top look true even outside a linked worktree.
    const [common, top] = (r.stdout || "").split("\n").map((l) => l.replace(/\r$/, "").trim());
    const mainTop = common ? path.dirname(common) : null;
    if (mainTop && top && mainTop !== top) result = mainTop;
  }
  _mainRootCache.set(cwd, result);
  return result;
}
...
function loadConfig(opts = {}) {
  ...
  return new Config(merged, { warnings, env, cwd, boundary, markerGraph, repoMarker, briefsBase, briefsMap, cli: opts.cli || {} });
}
...
class Config {
  ...
  // Step 1's `--org` is the one selector that REFUSES rather than falls through
  // when it matches nothing — see tenantError().
  ...
  // through to a different one: `{ kind, org, orgs }`, where `orgs` lists the
  // credentials actually stored (issue-spor-cli-unrecognized-org-fallback).
  // Both refusals come from an explicit `--org`, which is a per-invocation
  // assertion of WHICH tenant this command is for, so honoring it as a hint and
  // running against the store default instead reads the wrong graph and — worse
  ...
  // The ambient org selectors (SPOR_ORG, a repo `.spor` `org:` marker) keep
  // falling through: they also ride the fail-open hook engines, so turning them
  // into hard errors is a separate change with its own blast radius
  // (issue-spor-ambient-org-selector-silent-fallback).
  // The refusal is a REPORT, not a throw — tenant() stays a total function
  // returning null (so mode() resolves local and no wrong-tenant server/token
  // ever leaks out), and the CLI turns the report into a loud exit-1 refusal in
  // bin/spor's main(). A library caller that never sets a `--org` can never see
  // it, so hooks and the engines are byte-identical.
  tenantError() {
    this.tenant(); // force the (memoized) resolution that records it
    return this._tenantError;
  }

  _resolveTenant({ ignoreCliOrg = false } = {}) {
    const env = this._env || {};
    const cli = this._cli || {};
    ...
    if (cli.org != null && !ignoreCliOrg) {
      ...
      if (!String(cli.org).trim()) {
        this._tenantError = { kind: "empty-org", org: String(cli.org), orgs: storedOrgs(store) };
        return null;
      }
      ...
      this._tenantError = { kind: "unknown-org", org: cli.org, orgs: storedOrgs(store) };
      return null;
    }
    ...
    const markerOrg = repoMarkerOrg(this._cwd, this._boundary);
    if (markerOrg) {
  ...
  // yet" is the one legitimate use of an unresolvable `--org` (an `empty-org`
  ...
  serverForNewTenant() {
    if (!this.tenantError()) return this.server();
    const t = this._resolveTenant({ ignoreCliOrg: true });
    return t ? t.server : "";
  }
