...
class Config {
  ...
  // through to a different one: `{ kind: "unknown-org", org, orgs }`, where
  // `orgs` lists the credentials actually stored
  // (issue-spor-cli-unrecognized-org-fallback). Today the only refusal is an
  // explicit `--org` naming no stored credential: the flag is a per-invocation
  ...
  // yet" is the one legitimate use of an unresolvable `--org`, and there the
