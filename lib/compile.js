...
const NODES_DIR = path.resolve(opt("nodes", process.env.SPOR_NODES ?? path.join(home.graphHome(), "nodes")));
...
const MIN_TOP_SIM = parseFloat(opt("min-sim", "0.08")); // query-mode relevance gate

if (!ROOT_ID && !QUERY) {
  console.error("usage: compile.js [--nodes <dir>] (--root <id> | --query \"text\") [--digest] [--skeleton] [--out <file>]");
  process.exit(1);
}
