...
// Eval-only levers, never set by a real session: scripts/rank-eval's pooling
// mode (issue-spor-digest-rank-eval-retrieval-blind) generates deliberately-
// different retrieval variants by setting these CAND_* vars on the harness's
// spawned hook process, which this subprocess inherits. Absent them, each
// stays undefined and graph.compile() falls back to its shipped constant.
const envInt = (k) => (process.env[k] != null ? parseInt(process.env[k], 10) : undefined);
const CONTENT_TOP_K = envInt("CAND_CONTENT_TOP_K");
const QUERY_SEEDS = envInt("CAND_QUERY_SEEDS");
const MAX_HOPS = envInt("CAND_MAX_HOPS");
