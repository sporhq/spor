...
// on a 401/403, if the active tenant carries a refresh_token (store-based) and
// no explicit token override was given, refresh it once against the tenant's
// issuer and retry. The flat/env path has no refresh_token, so this is a no-op
// there — byte-identical.
async function request(cfg, method, apiPath, { body, timeoutMs = 6000, token: tokenOverride } = {}) {
  const r = await _attempt(cfg, method, apiPath, { body, timeoutMs, bearer: tokenOverride || token(cfg) });
  if ((r.status === 401 || r.status === 403) && !tokenOverride && cfg && typeof cfg.tenant === "function") {
    const t = cfg.tenant();
...
// Mirrors request()'s transparent refresh-once-on-401 for store-based tenants;
// a flat/env token has no refresh_token, so that branch is a no-op there.
...
async function download(cfg, apiPath, { timeoutMs = 60000, token: tokenOverride } = {}) {
  const r = await _download(cfg, apiPath, { timeoutMs, bearer: tokenOverride || token(cfg) });
  if ((r.status === 401 || r.status === 403) && !tokenOverride && cfg && typeof cfg.tenant === "function") {
    const t = cfg.tenant();
    if (t && t.refresh_token) {
      const key = t.key || auth.tenantKey(t.server, t.org);
