...
// Return a snapshot whose node/edge/policy/register/queue-policy entries are
// restricted to provenance `source` ("seed" | "graph" | "native"). Warnings and
// scalars pass through. A no-op when source is falsy.
function filterBySource(snap, source) {
  if (!source) return snap;
  const keep = (x) => x.source === source;
  return {
    ...snap,
    node_types: snap.node_types.filter(keep),
    edge_types: snap.edge_types.filter(keep),
    policies: snap.policies.filter(keep),
    registers: snap.registers.filter(keep),
    queue_policy: snap.queue_policy && keep(snap.queue_policy) ? snap.queue_policy : null,
  };
}
