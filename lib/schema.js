...
function nodeFlags(n) {
  ...
  // The declarative completion policy (task-spor-registry-declarative-terminal-
  // status-policy). The vocabulary itself is left to the per-type detail view —
  // it is the longest field and would swamp this column — but the SUCCESS value
  // and its resolver gate are what a reader needs at a glance: they are what a
  // mechanical "close it" remedy has to name.
  if (n.completion) f.push("completion:" + n.completion);
  if (n.resolver_required) f.push("resolver-required");
...
function renderType(snap, type) {
  ...
  if (n) {
    ...
    lines.push(`  statuses:    ${(n.vocabulary || []).join(", ") || "— (any status)"}`);
    // How this type's completion is ATTESTED (issue-spor-offline-check-get-hook-
    // resolution-proxy) — the fact a client verifying a dispatched run reads.
    // Spelled out here rather than in the overview flags for the same reason
    // the vocabulary is: it belongs where a reader is asking about ONE type.
    // An undeclared type says so explicitly; a reader must not mistake the
    // absence for a declared `status`.
    lines.push(`  resolution:  ${n.resolution ? `${n.resolution}-verified` : "— (undeclared — falls back to whether a get() hook is attached)"}`);
...
if (require.main === module) {
  ...
  let text = r.text;
  // Overview footer: packaged candidate schemas not in this registry
  // (task-spor-resident-schema-adoption-upgrade-path). Fail-soft — the footer
  // never breaks introspection; byte-identical when every candidate is
  // adopted (or none ship).
  if (positional == null && !has("json") && !r.stderr) {
    try {
      const candLib = require(path.join(__dirname, "candidates.js"));
      const f = candLib.footerLine(candLib.loadCandidates(), snap);
      if (f) text += "\n\n" + f;
    } catch {
      /* ignore */
    }
  }
  (r.stderr ? console.error : console.log)(text);
  process.exit(r.code);
}
