...
All graph-derived reads are viewer-scoped by the server's graph-native FGA
closure (`task-spor-graph-native-fga-read-visibility-filter`). Visibility comes
from graph relations—area stewardship/membership, direct assignment/routing/
review, authorship/initiator/asker, and agent ownership—not project claims or an
admin bit copied into the token. `stewards -> org-root` is the graph-wide read
relation. Hidden direct ids deliberately use the ordinary `404 not_found` shape;
list/search/queue/digest/lens/MCP surfaces omit them before ranking or rendering.
An edge whose target is invisible is absent from adjacency, returned
frontmatter, raw node markdown, history text, and node exports, so it cannot act
as an id/title/body oracle. An authenticated token with no resolvable person or
agent subject sees no graph content; a coarse read-only/CI flag is not a bypass.
...
| `POST /v1/queue/claim` `{ids, session?}` | `spor dispatch` claiming a working set; bulk-lease clients | **`claimAll`**: claim a whole working set in ONE round-trip instead of one `POST /v1/nodes/{id}/claim` per node (task-spor-bulk-claim-renew-apis). `ids` is REQUIRED (unlike `renewAll` below there is nothing to enumerate — a claim creates the lease it would have enumerated), bounded and deduped server-side. Each id runs through the same per-node `claim` logic, so one holder's node losing the race to another claimant lands in `failed` (`already_claimed`, naming the holder) while the rest of the batch still claims — a partial batch is reported, never rolled back → `{ok: true, status: "claimed"\|"partial"\|"refused", count, claimed: [ids], leases: [...], failed: [{node_id, code, message, holder?}]}`. `status` reads `"partial"` when some items landed and some didn't, `"refused"` when the whole batch was refused — **`ok: true` only means the call was well-formed; read `failed` for what didn't land, never `ok` alone**. The dispatch nonce and `force` (accepted on the singular `/claim`) are deliberately NOT accepted here — a dispatch tags a single agent launch at a single node, so that stays on the singular door |
| `POST /v1/queue/renew` `{ids?, session?}` | post-tool heartbeat (batched), `spor dispatch`, bulk-lease clients | **`renewAll`**: the heartbeat for a whole working set in ONE round-trip (task-spor-bulk-claim-renew-apis) — what the client-side claim heartbeat now uses instead of one `POST /v1/nodes/{id}/renew` per held node once more than one is held (task-spor-client-heartbeat-bulk-renew). Two modes: **`ids` omitted** enumerates every LIVE Tier-1 lease this caller holds (optionally narrowed to one `session`) and renews them all — the motivating case, one call per heartbeat instead of one per held node; **`ids` supplied** renews exactly that set (bounded/deduped), with `session` forwarded unchanged exactly as the singular `/renew` does. A lease that lapsed or was stolen out from under the caller lands in `failed` as `lease_lost` (naming the current holder) while the rest of the set still renews — one contested node never costs the whole working set its heartbeat → `{ok: true, status: "renewed"\|"partial"\|"refused", count, renewed: [ids], leases: [...], failed: [...], skipped_other_session?, skipped_reserved?}`. The two optional counts ride ONLY on the enumerate arm: `skipped_other_session` names live leases excluded because they're bound to a different session (a zero renewed count there means "not under this session", not "you hold nothing"), `skipped_reserved` names Tier-2 resumption reservations a blanket heartbeat deliberately leaves parked at their grace-window expiry rather than demoting to a Tier-1 horizon |
...
| `GET /v1/export` | bootstrap/offline; `spor export` | ustar tarball of the viewer-visible `nodes/` projection for seeding a local read replica (`?gzip=1` compresses); invisible files and cross-boundary edge lines are omitted. For a root steward this reproduces `nodes/` byte-for-byte. See §5 for response headers. `?history=1` instead streams an inherently unfilterable `git bundle --all` of the repo (`application/x-git-bundle`, full commit provenance, the customer data-exit path — `git clone <bundle> graph`) and is therefore admin-gated (`stewards`→root → `403` otherwise). `?auth=1` ALSO bundles `auth/*.json` so a disaster restore reproduces the credential set and has the same admin gate. The `spor export [--gzip] [--history\|--auth] [--out <file>]` CLI verb is the shell front-door (remote downloads this; `--gzip`/`--out` also build the same `nodes/` tarball locally, while `--history`/`--auth` are remote-only) |
