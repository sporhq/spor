...
| `GET /v1/queue?project=&limit=` | /spor:next, session-start | the ranked decision queue: `{items, count, questions, findings, policy?, generated_at}` — items retired by a live resolves/answers edge are excluded; `questions`/`findings` are the routed-to-me-plus-unrouted views for the authenticated identity |
