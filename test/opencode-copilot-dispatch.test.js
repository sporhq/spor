...
// `plan` agent (edit denied everywhere), Copilot's `--deny-tool write
// --deny-tool shell` (the file-writing tool AND the shell tool denied at the
// permission layer, over --allow-all — denying `write` alone left a shell
// command free to write the implementer's live checkout, review finding 4 on
// the second cut).
test("--read-only dispatches OpenCode under the plan agent and Copilot with the write and shell tools denied", () => {
  ...
  assert.match(cpRun.stdout, /copilot --output-format json --allow-all --no-ask-user --no-color --no-auto-update --deny-tool write --deny-tool shell --model profile-model/);
  ...
  assert.deepStrictEqual(dispatchHarnesses.getHarness("copilot").readOnly, { denyTools: ["write", "shell"] });
  ...
  assert.deepStrictEqual(dispatchHarnesses.getHarness("copilot").buildArgs({ model: null, readOnly: { denyTools: ["write", "shell"] } }).slice(-4), ["--deny-tool", "write", "--deny-tool", "shell"]);
